...
/**
 * Bing visual search — more reliable inside Outlook iOS in-app browser than Google Lens.
 */
export function buildBingVisualSearchUrlForImage(publicImageUrl) {
  const url = String(publicImageUrl || "").trim();
  if (!url.startsWith("http://") && !url.startsWith("https://")) return null;
...
/**
 * Force-open HTTPS URL in Safari on iOS (works from some in-app browsers including Outlook).
 */
export function buildSafariHttpsUrl(httpsUrl) {
  ...
  for (const photos of (photoGroups || new Map()).values()) {
    for (const file of photos || []) {
      ...
      try {
        ...
        // Short redirect URL — PDF readers often break long lens.google.com/?url=… links.
        const lensLaunchUrl = `${base}/api/consignment-lens/${token}`;
        file.publicImageUrl = publicImageUrl;
        file.googleLensUrl = lensLaunchUrl;
        file.googleLensDirectUrl = buildGoogleLensUrlForImage(publicImageUrl);
        hosted += 1;
      } catch (err) {
        failed += 1;
