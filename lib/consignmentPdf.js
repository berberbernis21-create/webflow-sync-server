...
export async function generateConsignmentPdf({
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.moveDown(0.8);
    doc.font("Helvetica-Bold").fontSize(13).fillColor(brandColor).text("Consignor profile", MARGIN);
    doc.moveDown(0.25);
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item?._splitChild) {
        doc
          ...
          .fillColor("#8a3b12")
          .text(
            `Split piece ${item.splitPieceIndex} of ${item.splitPieceCount} from submission Item #${item.splitFromItemNumber} (confidence ${String(item.splitConfidence || "").toUpperCase()}) — analyzed separately, not blended.`,
            MARGIN,
            doc.y,
            { width: CONTENT_WIDTH }
          );
      ...
      const contentBottom = pageContentBottom(doc);
