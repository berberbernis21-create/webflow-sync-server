...
export async function generateConsignmentPdf({
  ...
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      // Google Lens visual results for the first photo — what comps look like online
      const lensShot = photos.find((f) => f?.lensResultsScreenshot?.length)?.lensResultsScreenshot;
      if (lensShot?.length) {
        doc.moveDown(0.35);
        doc
          .font("Helvetica-Bold")
          ...
          .text("Google Lens results (first photo)", MARGIN);
        doc.moveDown(0.12);
        doc
          .font("Helvetica")
          .fontSize(9)
          .fillColor("#555")
          .text(
            "Screenshot of what Google Lens returns for this item’s main photo — use as visual context for comps.",
            MARGIN,
            doc.y,
            { width: CONTENT_WIDTH }
          );
        doc.moveDown(0.2);
        drawImageFit(doc, lensShot, CONTENT_WIDTH, Math.min(MAX_IMAGE_HEIGHT + 80, 420), "Google Lens results");
      }
