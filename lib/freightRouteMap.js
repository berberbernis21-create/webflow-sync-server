...
function payloadString({ exp, origin, destination, polyline }) {
  return `${exp}|${origin}|${destination}|${polyline || ""}`;
}
...
export function buildSignedMapImageUrl({
  ...
  // Keep signed URLs under common proxy/email length limits.
  let p = String(encodedPolyline || "").trim();
  if (p.length > 1800) p = "";
  const sig = sign(payloadString({ exp, origin: o, destination: dest, polyline: p }));
...
export function attachRouteMapFields(routeLookup, destination, origin = SHOWROOM_ORIGIN) {
  ...
  const dest = String(destination || "").trim();
  const polyline = routeLookup.encoded_polyline || "";
  return {
    ...routeLookup,
 ...
 * @returns {{ ok: true, origin: string, destination: string, polyline: string } | { ok: false, error: string, status: number }}
 */
export function verifyMapRequest(query = {}) {
  ...
  const polyline = String(query.p || "").trim();
  ...
  const expected = sign(payloadString({ exp, origin, destination, polyline }));
  ...
  return { ok: true, origin, destination, polyline };
}
...
export function buildGoogleStaticMapUrl({ origin, destination, polyline }) {
  ...
  // Markers: S = showroom, D = destination
