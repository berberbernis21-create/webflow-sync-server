...
function buildRetailAnchoredRationale({
  ...
  const parts = [
    `Intake pricing guide for ${label}: list around 30–50% of credible retail ($${range.rangeLow.toLocaleString("en-US")}–$${range.rangeHigh.toLocaleString("en-US")} from retail estimate $${retailEstimate.toLocaleString("en-US")}).`,
  ];
  ...
  if (retailEntries.length) {
    ...
    parts.push(
      "No linked online retail listings with confirmed prices — intake should confirm retail manually (consignor stated retail is shown separately and is not the research anchor)."
    );
  }
  ...
  if (resaleEntries.length) {
    ...
    const resaleAvg =
      resalePrices.length >= 2
        ...
        : resalePrices[0];
    parts.push(
      `${resaleEntries.length} linked resale listing${resaleEntries.length === 1 ? "" : "s"} included for directional context only (avg ~$${resaleAvg.toLocaleString("en-US")}) — not used as the intake pricing anchor.`
    );
  } else {
  ...
  if (consignorRetail) {
    ...
    parts.push(
      `Consignor-stated retail ($${consignorRetail.toLocaleString("en-US")}) is shown separately — if used only as consignor input, implied 30–50% intake range would be $${implied.customerImpliedAskLow.toLocaleString("en-US")}–$${implied.customerImpliedAskHigh.toLocaleString("en-US")}; do not treat as verified research.`
    );
  }
...
function applyResaleOnlyFallback(analysis, item, resaleEntries) {
  ...
  if (multi) {
    ...
    analysis.suggestedPricing.rationale = [
      `Confident multi-piece submission (${n} distinct pieces) — intake price guide withheld; do not blend into one range.`,
...
export function tightenPricingAnalysis(analysis, item, searchResults) {
  ...
  if (!resolved) {
    if (resaleEntries.length >= MIN_LINKED_RESALE_CONTEXT) {
      ...
      if (consignorRetail) {
        out.suggestedPricing.rationale = [
          out.suggestedPricing.rationale,
          `Consignor-stated retail ($${consignorRetail.toLocaleString("en-US")}) is listed separately and was not used as the intake research anchor.`,
        ]
          .filter(Boolean)
          .join(" ");
      }
    ...
    analysis.suggestedPricing.rationale = [
      multi
        ? `Confident multi-piece submission (${analysis.photoBundle?.distinctItemCount || "2+"} distinct pieces, confidence ${String(analysis.photoBundle?.mixedItemsConfidence || "medium").toUpperCase()}) — intake price guide withheld; do not blend into one range.`
        : "Insufficient verified retail comps from research — intake price guide withheld until comps are found.",
      consignorRetail
        ? `Consignor-stated retail ($${consignorRetail.toLocaleString("en-US")}) is shown separately (not used as the intake research anchor).`
        : null,
      multi
