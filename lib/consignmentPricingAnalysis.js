...
function buildLlmContext(item, visionBundle, searchResults) {
  return {
    ...
    pricingStrategy: {
      ...
      resaleCompRule:
        "comparableComps and suggested ask range must reflect actual resale/secondary market prices (average of like comps). Do NOT apply 30-50% to resale listing prices.",
      msrpRule:
        "Use 30-50% of original MSRP only when evidence is brand-new, full-price retail (pre-sale). Never use sale/clearance prices or resale comp prices as the base for a 30-50% discount.",
      showroomAskRule:
        "When comps are resale prices, suggested rangeLow/rangeHigh should align with the resale comp average (small adjustment for condition/demand), not a percentage off those comps.",
    },
  };
}

/** Snippet/search prices are treated as resale comps — ask range tracks comp average, not 30-50% off comps. */
function buildHeuristicPricingFromResalePrices(prices, searchResults) {
  prices.sort((a, b) => a - b);
  const low = prices[0];
  ...
  return {
    ...
    suggestedPricing: {
      ...
      velocityLabel: "Standard Seller",
      rationale:
        "Heuristic from resale search snippet prices (LLM unavailable). Comparable comps use resale evidence; suggested ask range is aligned to the comp average, not 30-50% of resale listings.",
    },
...
- compNotes must reflect realistic secondary/resale USD prices (what similar pieces actually sell for), not MSRP.
- If evidence is a resale listing (eBay, Facebook Marketplace, consignment shop, etc.), treat that price as a resale comp — do not plan to discount it again by 30-50%.
- priceSignals.typicalUsd should represent resale market level when resale evidence exists.
...
Rules:
- comparableComps (average, high, medium, low) must reflect realistic USD resale/secondary market prices from evidence — typically the average of like resale comps, not MSRP.
- suggestedPricing.rangeLow/rangeHigh (Lost & Found showroom consignment ask):
  • When your best evidence is resale/used comps (marketplace listings, our shop, other resale sites): anchor the ask range to the resale comp average (small adjustment for condition, size, demand). Do NOT apply 30-50% to those resale prices.
  • Use 30-50% only when you have credible brand-new, full-price MSRP (original retail before any sale). Never apply 30-50% to sale/clearance prices or to resale listing prices.
- suggestedPricing.retailEstimate: if comps are resale, set this to the resale comp anchor (e.g. average comp); if evidence is new MSRP only, set to that MSRP.
- In rationale, state whether pricing is resale-comp-anchored or MSRP-based; do not describe discounting a resale listing by 30-50%.
...
async function synthesizePricing(item, visionBundle, searchResults, photos) {
  ...
  if (!parsed) {
    ...
    if (prices.length >= 2) {
      parsed = buildHeuristicPricingFromResalePrices(prices, searchResults);
  ...
  try {
    ...
    if (prices.length >= 2) {
      analysis = normalizeAnalysisJson(
        buildHeuristicPricingFromResalePrices(prices, searchResults)
      );
