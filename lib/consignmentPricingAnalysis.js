 ...
 * - CONSIGNMENT_PRICING_TOTAL_BUDGET_MS, CONSIGNMENT_PRICING_ITEM_TIMEOUT_MS, CONSIGNMENT_PRICING_MAX_VISION_IMAGES
...
async function analyzeOneItem(item, itemNumber, photos) {
  ...
  const { analysis, modelsUsed } = await synthesizePricing(
    item,
    visionBundle,
    searchResults,
    photos
  ...
  logPricing("info", {
    ...
    modelsUsed,
    available: hasComps,
    imageCount: Math.min((photos || []).length, MAX_VISION_IMAGES),
  });
