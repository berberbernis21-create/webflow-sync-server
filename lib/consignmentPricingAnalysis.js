...
import fs from "fs";
import { buildCategorizedLinks } from "./consignmentPricingDisplay.js";
...
function buildLlmContext(item, visionBundle, searchResults) {
  return {
    ...
    pricingStrategy: {
      ...
      anchorOrder: [
        "1) RETAIL: if new/full retail is found (e.g. Wayfair $10,000), pricingAnchor=retail_30_50, rangeLow/rangeHigh=30-50% of that retail, retailEstimate=that retail.",
        "2) RESALE ONLY: if no credible retail, pricingAnchor=resale_comp_average, rangeLow/rangeHigh≈average of resale comps (retailEstimate may equal comp average for reference).",
        "3) FALLBACK: consignor originalPrice as retail, pricingAnchor=consignor_retail_30_50, rangeLow/rangeHigh=30-50% of that.",
      ],
      neverRules:
        "Never use sale/clearance as retail. Never apply 30-50% to a resale listing price.",
    },
  };
}
...
function buildHeuristicPricingFromEvidence(prices, searchResults, item) {
  ...
  if (sellerRetail) {
    return {
      ...
      suggestedPricing: {
        ...
        velocityLabel: "Standard Seller",
        rationale:
          "Heuristic (LLM unavailable). Fallback: consignor stated original retail; suggested ask 30-50% of that. Comparable comps average resale listings from search.",
      },
      sources,
    };
  }
  ...
  return {
    ...
    suggestedPricing: {
      ...
      velocityLabel: "Standard Seller",
      rationale:
        "Heuristic (LLM unavailable). No retail or consignor retail in evidence; suggested ask aligned to average resale comps. Comparable comps reflect resale search results.",
    },
    sources,
  };
}
...
- pricingAnchor and suggested ask (apply first match):
  • retail_30_50 — credible new/full RETAIL found (e.g. Wayfair lists $10,000 new). retailEstimate = that retail. rangeLow/rangeHigh = 30-50% of retail (e.g. $3,000–$5,000). Example: new item at $10k retail → suggest $3k–$5k.
  • resale_comp_average — no credible retail in evidence. rangeLow/rangeHigh ≈ average of resale comps (use comparableComps.average). Set pricingAnchor=resale_comp_average.
  • consignor_retail_30_50 — fallback: use consignor originalPrice as retailEstimate; rangeLow/rangeHigh = 30-50% of that.
- Never use sale/clearance as retail. Never apply 30-50% to a resale listing price.
- In rationale: state which anchor was used and cite resale comp average separately from the suggested ask.
