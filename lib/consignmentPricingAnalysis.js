...
function buildItemSearchQueries(item, visionBundle) {
  ...
  return uniqStrings(queries.filter(Boolean), isHandbag ? 8 : 6);
}

function collectResalePricesFromEvidence(searchResults, sources) {
  const prices = [];
  const add = (n) => {
    const v = Number(n);
    if (Number.isFinite(v) && v >= 25 && v <= 500_000) prices.push(Math.round(v));
  };

  for (const row of searchResults || []) {
    const channel = classifyLinkChannel(row.url, row.title, row.snippet);
    if (channel !== "resale") continue;
    if (row.priceHint != null) add(row.priceHint);
    for (const p of row.prices || []) add(p);
  }

  for (const s of sources || []) {
    if (String(s?.channel || "").toLowerCase() !== "resale") continue;
    if (s.price != null) add(s.price);
  }

  return [...new Set(prices)].sort((a, b) => a - b);
}

function buildMarketRecommendationRationale({ resaleCount, avg, brand, name, anchorNote }) {
  const label = [brand, name].filter(Boolean).join(" ").trim() || "this item";
  const parts = [
    `Market recommendation for ${label} is based on ${resaleCount} online resale listing price${resaleCount === 1 ? "" : "s"}.`,
    `Resale comp average: $${avg.toLocaleString("en-US")}.`,
  ];
  if (anchorNote) parts.push(anchorNote);
  parts.push("Suggested ask range reflects current secondary-market comps, condition, and brand demand.");
  return parts.join(" ");
}

/** Prefer online resale comp averages over %‑of‑retail when we have enough evidence. */
function applyMarketRecommendationPolicy(analysis, item, searchResults) {
  if (!analysis?.suggestedPricing) return analysis;

  const resalePrices = collectResalePricesFromEvidence(searchResults, analysis.sources);
  const suggested = analysis.suggestedPricing;
  const comps = analysis.comparableComps || {};

  if (resalePrices.length >= 2) {
    const avg = Math.round(resalePrices.reduce((s, p) => s + p, 0) / resalePrices.length);
    const low = resalePrices[0];
    const high = resalePrices[resalePrices.length - 1];
    const medium = resalePrices[Math.floor(resalePrices.length / 2)];
    ...
    analysis.comparableComps = {
      ...
      low,
      confidence:
        resalePrices.length >= 5 ? "medium" : resalePrices.length >= 3 ? "low" : comps.confidence || "low",
    };

    suggested.pricingAnchor = "resale_comp_average";
    suggested.rangeLow = Math.max(1, Math.round(avg * 0.9));
    suggested.rangeHigh = Math.max(suggested.rangeLow + 1, Math.round(avg * 1.1));
    if (!suggested.retailEstimate) suggested.retailEstimate = avg;

    suggested.rationale = buildMarketRecommendationRationale({
      resaleCount: resalePrices.length,
      avg,
      brand: item?.brand,
      name: item?.itemName,
      anchorNote: null,
  ...
  if (
    suggested.pricingAnchor === "consignor_retail_30_50" ||
    suggested.pricingAnchor === "retail_30_50"
  ) {
    const note =
      resalePrices.length === 1
        ? "Limited online comp data (one resale price signal); range also considers retail reference."
        : "Limited online comp matches; range uses retail reference until stronger resale comps are found.";
    suggested.rationale = `${String(suggested.rationale || "").trim()} ${note}`.trim();
  }
...
function buildLlmContext(item, visionBundle, searchResults) {
  return {
    ...
    pricingStrategy: {
      ...
      neverRules:
        "Never use sale/clearance as retail. Never apply a retail percentage formula to a resale listing price.",
    },
  };
}
...
/** Search snippets treated as resale comps; anchor: consignor retail → 30-50%, else resale average. */
function buildHeuristicPricingFromEvidence(prices, searchResults, item) {
  prices.sort((a, b) => a - b);
  const low = prices[0];
  const high = prices[prices.length - 1];
  const avg = Math.round(prices.reduce((s, p) => s + p, 0) / prices.length);
  const medium = prices[Math.floor(prices.length / 2)];
  const comps = {
    average: avg,
    high,
    medium,
  ...
  const sellerRetail = parseSellerRetailUsd(item);
  if (sellerRetail) {
    return {
      comparableComps: comps,
      suggestedPricing: {
        rangeLow: Math.max(1, Math.round(sellerRetail * 0.3)),
        rangeHigh: Math.max(1, Math.round(sellerRetail * 0.5)),
        retailEstimate: sellerRetail,
        pricingAnchor: "consignor_retail_30_50",
        velocityLabel: "Standard Seller",
        rationale:
          "Heuristic (LLM unavailable). Market recommendation uses consignor stated retail as reference; online resale comp average noted separately where available.",
      },
      sources,
    };
  }
  ...
  return {
    comparableComps: comps,
    suggestedPricing: {
      rangeLow: Math.max(1, Math.round(avg * 0.92)),
      rangeHigh: Math.max(1, Math.round(avg * 1.08)),
      retailEstimate: avg,
      pricingAnchor: "resale_comp_average",
      velocityLabel: "Standard Seller",
      rationale:
        "Heuristic (LLM unavailable). Market recommendation aligned to average of online resale comps from search results.",
    },
    sources,
  };
}
...
- comparableComps: always the average/spread of RESALE/secondary-market prices for like items.
...
async function synthesizePricing(item, visionBundle, searchResults, photos) {
  ...
  if (!parsed) {
    ...
    if (prices.length >= 2) {
      parsed = buildHeuristicPricingFromEvidence(prices, searchResults, item);
...
async function analyzeOneItem(item, itemNumber, photos, itemDeadlineMs) {
  ...
  try {
    ...
    if (prices.length >= 2) {
      analysis = normalizeAnalysisJson(
        buildHeuristicPricingFromEvidence(prices, searchResults, item)
      );
  ...
  if (analysis && typeof analysis === "object") {
    analysis = applyMarketRecommendationPolicy(analysis, item, searchResults);
