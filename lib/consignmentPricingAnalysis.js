...
const PER_ITEM_TIMEOUT_MS = Math.max(
  15000,
  parseInt(process.env.CONSIGNMENT_PRICING_ITEM_TIMEOUT_MS || "45000", 10) || 45000
);
...
async function fetchSearchResults(item, visionBundle) {
  const queries = buildItemSearchQueries(item, visionBundle);
  const all = [];
  for (const q of queries.slice(0, 4)) {
    try {
      const hits = await googleCustomSearch(q);
      for (const hit of hits) {
        all.push({ ...hit, query: q });
      }
    } catch (err) {
...
async function synthesizePricing(item, visionBundle, searchResults) {
  ...
  try {
    ...
    logPricing("warn", { event: "gemini.failed", message: err?.message });
  }
  ...
  if (!parsed) {
    try {
      ...
      logPricing("warn", { event: "openai.failed", message: err?.message });
    }
  }
...
function pricingConfigured() {
  const hasVision =
    ...
    Boolean(parseJsonEnv("GOOGLE_VISION_CREDENTIALS_JSON")) ||
    Boolean(parseJsonEnv("GOOGLE_SEARCH_CREDENTIALS_JSON")) ||
  ...
  return hasVision && hasSearch && hasLlm;
}
...
async function analyzeOneItem(item, itemNumber, photos) {
  ...
  const hasComps =
    analysis.comparableComps.average != null || analysis.comparableComps.low != null;
...
export async function analyzeConsignmentItemsPricing({ items, photoGroups }) {
  if (!pricingConfigured()) {
    logPricing("info", { event: "skipped", reason: "not_configured" });
    return { results: [], modelsUsed: [], configured: false };
  }
