...
import { tightenPricingAnalysis, collectVerifiedResaleComps, collectVerifiedRetailComps, computeCompStats, computeRetailAnchoredRange, computeRetailCompStats } from "./consignmentCompTuning.js";
...
function buildLlmContext(item, visionBundle, searchResults) {
  return {
    ...
    pricingStrategy: {
      ...
      comparableCompsRule:
        "comparableComps = average/spread of credible NEW/FULL RETAIL prices for like items (stores, brands, Wayfair, etc.). Include consignor stated retail in retailEstimate when no online retail is found.",
      anchorOrder: [
        "1) RETAIL ANCHOR (preferred): find credible new/full retail prices online or use consignor originalPrice. pricingAnchor=retail_30_50 or consignor_retail_30_50. rangeLow/rangeHigh = 30–50% of retailEstimate.",
        "2) RESALE CONTEXT: gather resale marketplace prices (Fashionphile, eBay, Poshmark, etc.) as supporting sources only — tag channel=resale, do NOT use resale average as pricingAnchor.",
        "3) LAST RESORT: if no retail reference exists but resale listings do, pricingAnchor=resale_comp_average with a note that retail must be confirmed manually.",
      ],
      customerLanguage:
        'In rationale, explain the market recommendation as 30–50% of credible retail. Mention resale comps only as directional context.',
      neverRules:
        "Never use sale/clearance/outlet prices as retail comps. Resale prices belong in sources with channel=resale only — not in comparableComps. Exclude unrelated brands/models.",
    },
  };
}
...
function buildHeuristicPricingFromEvidence(searchResults, item) {
  const consignorRetail = parseSellerRetailUsd(item);
  ...
  if (consignorRetail && !retailPrices.length) {
    retailPrices.push(consignorRetail);
  }
  ...
  const retailStats = computeRetailCompStats(retailPrices);
  const retailEstimate =
    retailEntries.length >= 2
      ? retailStats.average
      : consignorRetail && retailEntries.length === 1
        ? Math.round((retailEntries[0].price + consignorRetail) / 2)
        : retailPrices[0];
  ...
  return {
    ...
    suggestedPricing: {
      ...
      retailEstimate,
      pricingAnchor: retailEntries.length ? "retail_30_50" : "consignor_retail_30_50",
      velocityLabel: "Standard Seller",
      rationale:
        "Heuristic from linked search results (LLM unavailable). Ask anchored at 30–50% of retail estimate; resale links are directional only.",
    },
    sources,
  };
}
...
- comparableComps: average/spread of credible NEW/FULL RETAIL prices for like items (same brand/model when possible). Consignor originalPrice may inform retailEstimate when online retail is thin.
- Resale listings go in sources with channel=resale only — directional context, not comparableComps.
- pricingAnchor and suggested ask (apply first match):
  • retail_30_50 — PREFERRED when credible online retail exists. retailEstimate = credible retail reference. rangeLow/rangeHigh = 30–50% of retailEstimate.
  • consignor_retail_30_50 — consignor originalPrice when no online retail; rangeLow/rangeHigh = 30–50% of that retail.
  • resale_comp_average — LAST RESORT only when no retail reference exists; note that retail must be confirmed manually.
- In rationale: lead with the 30–50% of retail market recommendation. Cite retail comp average in comparableComps. Mention resale only as supporting context.
- velocityLabel: use "High Seller / Fast Seller" when resale context shows strong demand; otherwise "Standard Seller".
