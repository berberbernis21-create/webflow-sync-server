 ...
 * - CONSIGNMENT_OPENAI_MAX_IMAGES — GPT vision only; unset/0 = all photos per item (size skips still apply)
...
/** 0 = unlimited — send every photo for the item to OpenAI (subject to resize / 1.8MB skip). */
...
function photosForOpenAi(photos) {
  const all = photos || [];
  return OPENAI_MAX_IMAGES > 0 ? all.slice(0, OPENAI_MAX_IMAGES) : all;
}
...
function perItemTimeoutMs(photoCount) {
  ...
  return Math.min(base + extraMs, TOTAL_BUDGET_MS);
}
  ...
  const results = await mapPool(jobs, MAX_CONCURRENT_ITEMS, async (job) => {
    ...
    try {
      const result = await withTimeout(
        ...
        perItemTimeoutMs(photos.length),
        `item_${itemNumber}`
      );
