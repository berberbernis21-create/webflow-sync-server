...
function buildItemSearchQueries(item, visionBundle) {
  ...
  if (isHandbag) {
    ...
    // No brand submitted — do not invent one. Name/category + visual clues only.
    if (productBits) {
      queries.push(`${productBits} retail price`.trim());
      queries.push(`${productBits} chairish OR 1stdibs`.trim());
      queries.push(`${productBits} resale consignment`.trim());
    }
    // Vision entities as *visual clue* search terms only (not treated as brand).
    for (const entity of (visionBundle?.webEntities || []).slice(0, 2)) {
      const e = String(entity || "").trim();
      if (e.length < 4) continue;
      queries.push(`${e} ${name || category} furniture`.trim());
    }
  }
