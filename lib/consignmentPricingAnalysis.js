...
function perItemTimeoutMs(photoCount) {
  const base = PER_ITEM_TIMEOUT_MS;
  const n = Number(photoCount) || 0;
  if (n <= 3) return base;
  const extraMs = Math.min((n - 3) * 1500, 12_000);
  return Math.min(base + extraMs, submissionPricingBudgetMs(MAX_PRICING_ITEMS));
}

/** Scale wall-clock budget so up to MAX_PRICING_ITEMS can complete (Vision → Search → Gemini → OpenAI). */
function submissionPricingBudgetMs(itemCount) {
...
async function synthesizePricing(item, visionBundle, searchResults, photos) {
  ...
  try {
    geminiBrief = await callGeminiIntermediate(context);
    if (geminiBrief) modelsUsed.push(`gemini:${geminiModel}`);
  } catch (err) {
    logLlmApiError("gemini", err);
  }

  const imageDataUrls = await preparePhotoDataUrls(photos);
...
export async function analyzeConsignmentItemsPricingWithBudget({
  ...
  const effectiveBudgetMs = budgetMs ?? submissionPricingBudgetMs(analyzeCount);
...
async function analyzeOneItem(item, itemNumber, photos) {
  ...
  const { analysis, modelsUsed, openaiImageCount } = await synthesizePricing(
    item,
    visionBundle,
    searchResults,
    photos
...
export async function analyzeConsignmentItemsPricing({ items, photoGroups, deadlineMs } = {}) {
  const configStatus = getPricingConfigStatus();
  const deadline =
    typeof deadlineMs === "number" && Number.isFinite(deadlineMs)
      ? deadlineMs
      : Date.now() + submissionPricingBudgetMs(Math.min(items?.length ?? 1, MAX_PRICING_ITEMS));
  ...
  const results = await mapPool(jobs, MAX_CONCURRENT_ITEMS, async (job) => {
    ...
    const itemTimeout = Math.min(perItemTimeoutMs(photos.length), remainingMs);
