...
/** Map brand / vision text → manufacturer or retailer site: filters. */
function brandSiteHints(item, visionBundle) {
  const blob = [
    item?.brand,
    item?.itemName,
    ...(visionBundle?.logos || []),
    ...(visionBundle?.webEntities || []).slice(0, 6),
    ...(visionBundle?.detectedText || []).slice(0, 4),
  ]
  ...
  if (/century furniture|centuryfurniture/.test(blob)) push("centuryfurniture.com");
...
function buildItemSearchQueries(item, visionBundle) {
  ...
  const topEntity = String(visionBundle?.webEntities?.[0] || "").trim();
  const idHint = [brand, topEntity, name].filter(Boolean).join(" ").slice(0, 90);
  const queries = [];
  ...
  if (isHandbag) {
    ...
    // Trusted-site first — more reliable than open web noise.
    const trustedSites = [
      ...brandSiteHints(item, visionBundle),
      "wayfair.com",
      "chairish.com",
      "1stdibs.com",
      "crateandbarrel.com",
    ...
    for (const site of trustedSites) {
      queries.push(`site:${site} ${idHint || core}`.trim());
    }
    queries.push(`${idHint || core} new retail price buy`.trim());
    queries.push(`${brand} ${name} manufacturer retail price`.trim());
    queries.push(`${idHint || core} msrp`.trim());
    if (/sofa|sectional|chair|table|furniture|dining|wingback/i.test(`${category} ${name}`)) {
      queries.push(`${brand} ${name} dining chair retail`.trim());
      queries.push(`${idHint || core} furniture retailer price`.trim());
      queries.push(`${core} site:wayfair.com OR site:dwr.com OR site:rh.com`.trim());
    }
    queries.push(`${idHint || core} chairish OR 1stdibs used`.trim());
    queries.push(`${core} consignment resale price`.trim());
  }

  for (const logo of (visionBundle.logos || []).slice(0, 2)) {
    queries.push(`${logo} ${name || category} retail price`.trim());
    if (brand) queries.push(`site:wayfair.com ${brand} ${logo}`.trim());
  }

  // Vision web entities are the best text clue from multi-image reverse lookup.
  for (const entity of (visionBundle.webEntities || []).slice(0, 3)) {
    queries.push(`${entity} furniture retail price`.trim());
    queries.push(`"${entity}" manufacturer buy`.trim());
  }
  for (const label of (visionBundle.labels || []).slice(0, 2)) {
    if (!/furniture|chair|table|sofa|indoor|wood|fabric/i.test(label)) continue;
    queries.push(`${brand} ${label} ${category} retail`.trim());
  }
  for (const textLine of (visionBundle.detectedText || []).slice(0, 2)) {
    if (textLine.length < 4) continue;
    queries.push(`"${textLine}" ${brand || "furniture"} retail`.trim());
  }
  return uniqStrings(queries.filter(Boolean), isHandbag ? 10 : 10);
}
...
function searchRowsFromVisionPages(visionBundle) {
...
async function fetchSearchResults(item, visionBundle) {
  ...
  all.push(...searchRowsFromVisionPages(visionBundle));
...
- Identify brand, model family, material, color, configuration (e.g. L-shape sectional vs sofa + chairs). Call out labels/tags/text in photos.
...
- sources: ONLY URLs that appear in the payload searchResults / visionPages. Never invent or guess links. Tag channel retail|resale|customer. Strong visual matches → matchStrength "strong". Prefer preferred retailers / manufacturer product pages with a clear price.
    ...
    analysis.sources = (analysis.sources || [])
      ...
      .map((s) => {
        ...
        return {
          ...
          strongVisionMatch: Boolean(row?.strongVisionMatch || s.matchStrength === "strong"),
        };
      });
