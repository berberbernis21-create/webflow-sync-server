...
export function palletizeItems(items = [], { allowTitleDimFallback = true, preferClientPallet = false } = {}) {
  return (Array.isArray(items) ? items : []).map((item, index) => {
    ...
    // Webflow summary shows this exact pallet — emails/API must match it, not reinvent.
    let pallet = result.pallet;
    const clientPallet = item.pallet && typeof item.pallet === "object" ? item.pallet : null;
    if (preferClientPallet && clientPallet) {
      const cw = num(clientPallet.width);
      const cd = num(clientPallet.depth);
      const ch = num(clientPallet.height);
      const cwt = num(clientPallet.weight);
      if (cw && cd && ch && cwt) {
        pallet = {
          width: cw,
          depth: cd,
          ...
          packaging_type: String(clientPallet.packaging_type || "Pallet(s)"),
          ...
          pallet_count: 1,
          packing_notes: ["Matches Webflow calculator summary pallet entry."],
        };
      }
    }
