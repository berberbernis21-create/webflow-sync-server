...
/**
 * Loaded delivery truck is a bit slower than a car ETA.
 * Example: 15 min car route → 17 min truck drive time (factor 17/15).
 */
export const LOCAL_AZ_TRUCK_DRIVE_FACTOR = 17 / 15;

/** Scale car/route minutes up slightly for truck pace (ceil). */
export function applyLocalTruckDriveMinutes(driveMinutes) {
  const m = Number(driveMinutes);
  if (!Number.isFinite(m) || m <= 0) return null;
  return Math.max(1, Math.ceil(m * LOCAL_AZ_TRUCK_DRIVE_FACTOR));
}
...
/** Heavy OR tall/wide items force the 3-person ($130/hr) estimate and need confirmation. */
export const LOCAL_OVERSIZE_WEIGHT_LB = 299;
export const LOCAL_OVERSIZE_HEIGHT_IN = 72;
export const LOCAL_OVERSIZE_WIDTH_IN = 72;
...
/** Product dims: 299+ lb OR taller than 72" OR wider than 72". */
export function isLocalOversizeConfirmItem(item = {}) {
  const weight = Number(item?.weight);
  const height = Number(item?.height);
  const width = Number(item?.width);
  const heavy = Number.isFinite(weight) && weight >= LOCAL_OVERSIZE_WEIGHT_LB;
  const tall = Number.isFinite(height) && height > LOCAL_OVERSIZE_HEIGHT_IN;
  const wide = Number.isFinite(width) && width > LOCAL_OVERSIZE_WIDTH_IN;
  return heavy || tall || wide;
}
