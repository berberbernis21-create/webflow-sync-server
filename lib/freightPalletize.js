...
export function inferFreightClass(item = {}) {
  if (item.freight_class != null && Number(item.freight_class) > 0) {
...
export function palletizeItem(item = {}) {
  ...
  const freightClass = inferFreightClass({ ...item, title: item.title });
  ...
  if (width > STANDARD_PALLET_W) freightW = roundUpInches(width);
  if (depth > STANDARD_PALLET_D) freightD = roundUpInches(depth);
