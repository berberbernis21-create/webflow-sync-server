...
export const LOCAL_AZ_STAIR_FEE_PER_EXTRA_FLIGHT = 75;
/** Heavy + tall/wide items force the 3-person ($130/hr) estimate and need confirmation. */
...
/** Product dims: 299+ lb and taller than 72" and wider than 72". */
export function isLocalOversizeConfirmItem(item = {}) {
  const weight = Number(item?.weight);
  const height = Number(item?.height);
  const width = Number(item?.width);
  if (![weight, height, width].every((n) => Number.isFinite(n) && n > 0)) return false;
  return (
    weight >= LOCAL_OVERSIZE_WEIGHT_LB &&
    height > LOCAL_OVERSIZE_HEIGHT_IN &&
    width > LOCAL_OVERSIZE_WIDTH_IN
  );
}
...
/** Stair fee: first flight free, then $75 per additional flight. */
