...
export function validateFreightQuoteRequest(body = {}, { requireItemsComplete = false } = {}) {
  ...
  if (deliveryPath === "local_az" || deliveryPath === "pickup_az") {
    if (!(access.assembly_required === "yes" || access.assembly_required === "no" || access.assembly_required === "idk")) {
      return {
        ok: false,
        error: "Please answer whether disassembly and reassembly are required to move safely (Yes / No / idk).",
        status: 400,
      };
    }
    if (access.assembly_required === "yes") {
      if (!(access.assembly_over_15 === "yes" || access.assembly_over_15 === "no" || access.assembly_over_15 === "idk")) {
        return {
          ok: false,
          error: "Please say whether disassembly & reassembly will take longer than 15 minutes (Yes / No / idk).",
          status: 400,
        };
      }
      if (access.assembly_over_15 === "yes") {
        if (!(access.assembly_extra_minutes >= 1)) {
          return {
            ok: false,
            error: "Enter how many more minutes beyond 15 for disassembly & reassembly (numbers only).",
            status: 400,
          };
        }
      } else {
