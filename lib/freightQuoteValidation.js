...
export function validateFreightQuoteRequest(body = {}, { requireItemsComplete = false } = {}) {
  ...
  const items = rawItems
    .map((it) => {
      let title = str(it.title || it.itemName || it.name, 300);
      if (!title) return null;
      const isPickup = deliveryPath === "pickup_az";
      let productUrl = isPickup ? "" : str(it.product_url || it.productUrl, 500);
      const titleIsUrl =
        /^https?:\/\//i.test(title) ||
        /lostandfoundresale\.com\//i.test(title) ||
        /\/products?\//i.test(title);
      if (!isPickup && titleIsUrl) {
        if (!productUrl) productUrl = title;
        // Keep a readable interim title until enrichSubmissionItemMedia replaces it.
        try {
          const href = /^https?:\/\//i.test(title) ? title : `https://${title.replace(/^\/+/, "")}`;
          const parts = new URL(href).pathname.split("/").filter(Boolean);
          const pi = parts.findIndex((p) => p === "product" || p === "products");
          const slug = decodeURIComponent(
            parts[pi >= 0 && parts[pi + 1] ? pi + 1 : parts.length - 1] || ""
          )
            .replace(/-\d{4,}$/g, "")
            .replace(/[-_+]+/g, " ")
            .trim();
