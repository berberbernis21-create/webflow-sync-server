...
export function validateFreightQuoteRequest(body = {}, { requireItemsComplete = false } = {}) {
  ...
  for (const it of items) {
    ...
    // Per-item assembly questionnaire is local / pickup only.
    if (deliveryPath === "local_az" || deliveryPath === "pickup_az") {
      if (!(it.assembly_required === "yes" || it.assembly_required === "no" || it.assembly_required === "idk")) {
        return {
          ok: false,
          error: `For "${it.title}", please answer whether disassembly and reassembly are required to move it safely (Yes / No / idk).`,
          status: 400,
        };
      }
      if (it.assembly_required === "yes") {
        if (!(it.assembly_over_15 === "yes" || it.assembly_over_15 === "no" || it.assembly_over_15 === "idk")) {
          return {
            ok: false,
            error: `For "${it.title}", please say whether disassembly & reassembly will take longer than 15 minutes (Yes / No / idk).`,
            status: 400,
          };
        }
        if (it.assembly_over_15 === "yes") {
          if (!(it.assembly_extra_minutes >= 1)) {
            return {
              ok: false,
              error: `For "${it.title}", enter how many more minutes beyond 15 (numbers only).`,
              status: 400,
            };
          }
        } else {
        ...
        // no / idk — treated the same for pricing; keep answer for Q/A display
        it.assembly_over_15 = "";
        it.assembly_extra_minutes = 0;
      }
    } else {
  ...
  if (
    ...
    return {
      ok: false,
      error: "Please choose 1 extra person or 2 extra people (two trucks).",
      status: 400,
    };
  }
      ...
      multi_item_note:
        palletized.length > 1
          ...
          : deliveryPath !== "nationwide" && totalQty > 1
            ? `Local/pickup quantity total is ${totalQty} (counts toward multi-item handling). No flip/stack confirmation needed.`
            : "",
    },
  };
}
