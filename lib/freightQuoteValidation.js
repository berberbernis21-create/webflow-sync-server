...
function normalizeAccess(body = {}) {
  ...
  // SOP: Lost & Found has no dock — liftgate pickup always yes.
  const liftgatePickup = true;
  // Liftgate delivery yes unless confirmed dock or forklift.
  let liftgateDelivery = bool(a.liftgate_delivery ?? a.liftgateDelivery, true);
  if (dock || forklift) liftgateDelivery = bool(a.liftgate_delivery, false);
  ...
  return {
    ...
    inside_delivery: bool(a.inside_delivery ?? a.insideDeliveryRequested, false),
    room_placement: bool(a.room_placement, false),
...
export function validateFreightQuoteRequest(body = {}, { requireItemsComplete = false } = {}) {
  ...
  const customerName = str(body.customer_name || body.customerName || body.name, 120);
  const customerEmail = str(body.customer_email || body.customerEmail || body.email, 200).toLowerCase();
  ...
  if (deliveryPath !== "local_az" && deliveryPath !== "nationwide") {
  ...
  if (deliveryPath === "local_az" && !isArizonaStateOrZip({ state, zip })) {
    return {
      ok: false,
      error:
  ...
  const items = rawItems
    .map((it) => {
      ...
      return {
        ...
        freight_class: numOrNull(it.freight_class ?? it.freightClass),
  ...
  const access = normalizeAccess({ ...body, destination_type: destinationType });
