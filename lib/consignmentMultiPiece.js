...
/** Same-SKU furniture plurals — keep as one item unless clearly unrelated/art. */
const MATCHING_FURNITURE_PLURAL =
  /\b(chairs?|stools?|bar\s*stools?|nightstands?|ottomans?|benches?)\b/i;
    ...
    if (count && count >= 2) {
      ...
      if (MATCHING_SET_HINT.test(blob) && !DISTINCT_ART_HINT.test(blob) && !UNRELATED_HINT.test(blob)) {
        return {
          ...
          confidence: "low",
          reason: `Title suggests ${count} pieces that look like a matching set — keep as one analysis item unless photos show unrelated pieces.`,
        };
      }
      // "2 wingback dining chairs" = matching pair, not two unrelated SKUs.
      if (
        MATCHING_FURNITURE_PLURAL.test(blob) &&
        !DISTINCT_ART_HINT.test(blob) &&
        !UNRELATED_HINT.test(blob) &&
        !/\band\b.{0,40}\b(table|sofa|sectional|painting|art)\b/i.test(blob)
      ) {
        return {
          distinctItemCount: count,
          confidence: "low",
          reason: `Title describes ${count} matching furniture pieces (likely one SKU/set) — do not split unless photos show unrelated items.`,
        };
      }
