...
export function buildInternalEmailPricingHtml(pricing) {
  ...
  const parts = [
    ...
    `<p style="margin:0 0 10px;font-size:12px;color:#777;">See pricing in the attached PDF.</p>`,
    ...
    `<tr><td style="padding:4px 12px 4px 0;color:#555;">Suggested consignment ask range</td><td style="padding:4px 0;font-weight:600;">${blurPriceHtml(`${formatUsd(suggested.rangeLow)} - ${formatUsd(suggested.rangeHigh)}`)}</td></tr>`,
    `<tr><td style="padding:4px 12px 4px 0;color:#555;">Resale comp anchor (avg)</td><td style="padding:4px 0;">${blurPriceHtml(formatUsd(suggested.retailEstimate))}</td></tr>`,
...
export function buildInternalEmailPricingText(pricing) {
  ...
  const lines = [
    "See pricing in the attached PDF.",
...
export function drawPdfPricingSection(doc, pricing, { brandColor, margin, contentWidth, contentBottom }) {
  ...
  doc.moveDown(0.15);
  drawRow(
    "Suggested consignment ask range",
    `${formatUsd(suggested.rangeLow)} - ${formatUsd(suggested.rangeHigh)}`
  );
  drawRow("Resale comp anchor (avg)", formatUsd(suggested.retailEstimate));
