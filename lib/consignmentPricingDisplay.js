...
export const INTERNAL_EMAIL_PRICING_NOTICE =
  "Full research, links, and pricing detail are in the attached PDF.";
...
export function buildFinalPricingSummary({ suggested, item, reviewerGuidance, analysis } = {}) {
  ...
  const suggest =
    suggested?.rangeLow != null && suggested?.rangeHigh != null
      ? `We would price at ${formatUsd(suggested.rangeLow)}–${formatUsd(suggested.rangeHigh)} (${labels.rangeLabel}).`
      : guidance.impliedAskLow != null && guidance.impliedAskHigh != null
        ? `No verified retail comps yet — directional starting ask from category research: ${formatUsd(guidance.impliedAskLow)}–${formatUsd(guidance.impliedAskHigh)} (confirm manually).`
        : "Research-based ask is pending until verified comps are confirmed.";
  ...
  return [
    ...
    suggest,
    customerBit,
    "Approach: price from verified retail at ~30–50% when available; customer input is a separate check.",
  ].join(" ");
}
...
/**
 * Slim email block: suggested ask + evidence found/not found + see PDF.
 */
export function buildInternalEmailPricingHtml(pricing, item = null) {
  if (!pricing?.available || !pricing.analysis) {
    ...
    return `<div style="margin-top:12px;padding:12px;background:#f7f7f7;border:1px solid #e5e5e5;"><p style="margin:0;font-size:13px;color:#555;"><strong>Pricing:</strong> ${escapeHtml(headline)}. Please refer to the attached PDF.</p></div>`;
  }

  const { suggestedPricing: suggested, photoBundle } = pricing.analysis;
  const labels = getSuggestedPricingLabels(suggested);
  const evidence = hasResearchEvidence(pricing.analysis);
  ...
  const parts = [`<div style="margin-top:12px;padding:12px;background:#f7f7f7;border:1px solid #e5e5e5;">`];
  if (showMulti) {
    parts.push(
      `<p style="margin:0 0 8px;font-size:12px;color:#8a3b12;"><strong>Note:</strong> Multiple distinct pieces flagged — see PDF.</p>`
    );
  }
  parts.push(
    `<p style="margin:0 0 4px;font-size:11px;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;color:#1a3c34;">Suggested ask</p>`,
    `<p style="margin:0 0 2px;font-size:12px;color:#666;">${escapeHtml(labels.rangeLabel)}</p>`,
    `<p style="margin:0 0 10px;font-size:20px;font-weight:700;color:#1a3c34;">${
      hasRange ? blurPriceHtml(rangeText) : escapeHtml(rangeText)
    }</p>`,
    `<p style="margin:0;font-size:13px;color:#444;line-height:1.45;">`,
    evidence
      ? `<strong>Evidence:</strong> Research links/context found — please refer to the attached PDF for comps, reasons, and links.`
      : `<strong>Evidence:</strong> No verified online comps found yet — please refer to the attached PDF for category notes and next steps.`,
    `</p>`,
    `<p style="margin:8px 0 0;font-size:12px;color:#777;">Full analysis, links, and customer-input detail are in the PDF.</p>`,
    `</div>`
  );
  return parts.join("");
}
...
export function buildInternalEmailPricingText(pricing) {
  if (!pricing?.available || !pricing.analysis) {
    return `${pricingUnavailableHeadline(pricing)}. See attached PDF.`;
  }
  const suggested = pricing.analysis.suggestedPricing;
  const evidence = hasResearchEvidence(pricing.analysis);
  const range =
    suggested?.rangeLow != null && suggested?.rangeHigh != null
      ? "Suggested ask: [see PDF]"
      : "Suggested ask: pending — see PDF";
  return [
    range,
    evidence
      ? "Evidence: research links/context found — see attached PDF."
      : "Evidence: no verified online comps found yet — see attached PDF.",
  ].join("\n");
}
...
export function buildCategorizedLinks({ item, analysis }) {
  ...
  for (const entry of analysis?.verifiedRetailCompEntries || []) {
    ...
    buckets.retail.push({
      ...
      note: entry.strongVisionMatch
        ? "Strong image match + verified live listing — pricing anchor"
        : "Verified live listing — used in retail comp average / pricing anchor",
  ...
  for (const entry of analysis?.verifiedResaleCompEntries || []) {
    ...
    buckets.resale.push({
      ...
      price: entry.price,
      channel: "resale",
      note: "Verified live listing — directional resale only (not pricing anchor)",
      matchStrength: "weak",
      linkVerified: true,
    });
  }
...
export function drawPdfPricingSection(doc, pricing, { brandColor, margin, contentWidth, contentBottom, item }) {
  ...
  const directionalAsk =
    reviewerGuidance?.impliedAskLow != null && reviewerGuidance?.impliedAskHigh != null
      ? `${formatUsd(reviewerGuidance.impliedAskLow)} - ${formatUsd(reviewerGuidance.impliedAskHigh)} (directional)`
      : null;
  const rangeText =
    suggested.rangeLow != null && suggested.rangeHigh != null
      ? `${formatUsd(suggested.rangeLow)} - ${formatUsd(suggested.rangeHigh)}`
      : directionalAsk || "Pending research comps";
  ...
  doc.text("SUGGESTED ASK", margin + 10, boxY + 7, { width: contentWidth - 20 });
  ...
  // Short why
  doc.font("Helvetica-Bold").fontSize(10).fillColor(brandColor).text("Why this price", margin);
  doc.moveDown(0.1);
  ...
  doc.moveDown(0.2);
  doc.font("Helvetica-Bold").fontSize(10).fillColor(brandColor).text("What the customer said", margin);
  doc.moveDown(0.1);
