...
/**
 * Internal email: Suggested Pricing box with research + reviewer guidance.
...
export function buildInternalEmailPricingHtml(pricing, item = null) {
  ...
  const rationale = sanitizeEmailSummaryText(
    formatPricingRationaleForDisplay(suggested?.rationale) ||
  ...
  for (const row of researchRows) {
    ...
    parts.push(
      `<tr><td style="padding:4px 12px 4px 0;color:#555;">${escapeHtml(row.label)}</td><td style="padding:4px 0;">${
        looksMoney ? blurPriceHtml(display) : escapeHtml(display)
      }</td></tr>`
    );
  }
  ...
  parts.push(
    ...
    `<tr><td style="padding:4px 12px 4px 0;color:#555;">Image match</td><td style="padding:4px 0;">${escapeHtml(`${match.label} — ${match.detail}`)}</td></tr>`,
    `</table>`
  );
  ...
  parts.push(
    `<p style="margin:12px 0 0;font-size:11px;color:#999;">Full comps and source links are in the attached PDF. Analysis: ${escapeHtml(EMAIL_ANALYSIS_LABEL)}</p>`,
    `</div>`
  );
...
export function buildInternalEmailPricingText(pricing, item = null) {
  ...
  const { suggestedPricing: suggested, comparableComps: comps, reviewerGuidance } = pricing.analysis;
  const labels = getSuggestedPricingLabels(suggested);
  const match = getImageMatchConfidence(pricing.analysis);
  const guidanceNotes = sanitizeEmailSummaryText(formatReviewerGuidanceNotes(reviewerGuidance));
  const rationale = sanitizeEmailSummaryText(
  ...
  const lines = [
    ...
    `  ${labels.rangeLabel}: ${rangeText}`,
    `  Seller velocity: ${suggested.velocityLabel || "Standard Seller"}`,
    `  Comp confidence: ${capitalizeConfidence(comps?.confidence)}`,
    `  Image match: ${match.label} — ${match.detail}`,
    guidanceNotes ? `  Reviewer guidance: ${guidanceNotes}` : "",
    rationale ? `  Summary: ${rationale}` : "",
    "  Full comps and source links: see attached PDF",
  ];
