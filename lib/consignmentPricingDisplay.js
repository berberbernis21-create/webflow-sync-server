...
export function getPricingDetailRows(suggested, comps, item) {
  const rows = [];
  const retail =
    parseConsignorRetailUsd(item) ||
    (suggested?.pricingAnchor !== "resale_comp_average" ? suggested?.retailEstimate : null);

  if (comps?.average) {
    rows.push({
      label: "Resale comp average (verified listings)",
      value: comps.average,
    });
  }
  if (retail) {
...
export function buildInternalEmailPricingHtml(pricing, item = null) {
  ...
  const detailRows = getPricingDetailRows(suggested, comps, item);
...
export function buildCategorizedLinks({ item, analysis, searchResults, visionBundle }) {
  ...
  for (const entry of analysis?.verifiedCompEntries || []) {
    add({
      ...
      price: entry.price,
      channel: "resale",
      note: "Verified resale comp used in average",
...
export function drawPdfPricingSection(doc, pricing, { brandColor, margin, contentWidth, contentBottom, item }) {
  ...
  for (const row of getPricingDetailRows(suggested, comps, item)) {
