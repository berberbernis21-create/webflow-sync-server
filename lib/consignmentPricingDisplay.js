...
export function buildCategorizedLinks({ item, analysis, searchResults, visionBundle }) {
  ...
  for (const r of searchResults || []) {
    add({
      title: r.title || r.url,
      url: r.url,
      price: r.priceHint ?? null,
      channel: classifyLinkChannel(r.url, r.title, r.snippet),
      note: r.query ? `Search: ${r.query}` : "Custom Search",
    });
  }

  for (const p of visionBundle?.visionPages || []) {
    if (!p?.url) continue;
    add({
      title: p.title || "Vision web match",
      url: p.url,
      price: null,
      channel: classifyLinkChannel(p.url, p.title),
      note: p.kind ? `Vision: ${p.kind}` : "Google Vision",
    });
  }
  ...
  for (const entry of byUrl.values()) {
    const ch = entry.channel === "retail" ? "retail" : "resale";
...
function drawPdfLinkGroup(doc, title, links, { brandColor, margin, contentWidth, contentBottom }) {
  ...
  for (const link of links) {
    ...
    const lines = [link.title ? `• ${link.title}${pricePart}` : `• ${formatUsd(link.price)}`];
    if (link.url) lines.push(`  ${link.url}`);
    if (link.note) lines.push(`  ${link.note}`);
    doc.text(lines.join("\n"), margin, doc.y, { width: contentWidth, lineGap: 1, link: link.url || undefined });
    doc.moveDown(0.12);
  }
}
