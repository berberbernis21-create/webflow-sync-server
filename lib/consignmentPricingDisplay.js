...
export function getSuggestedPricingLabels(suggested) {
  ...
  if (anchor === "resale_comp_average") {
    return {
      rangeLabel: "Suggested list price from resale comps — confirm retail",
      anchorLabel: "Resale context only — confirm retail",
    };
  }
  if (anchor === "retail_30_50") {
    return {
      rangeLabel: "Suggested list price — 30–50% of retail from what we found",
      anchorLabel: "Retail estimate from research (linked listings)",
    };
  }
  if (anchor === "consignor_retail_30_50") {
    return {
      rangeLabel: "Suggested list price from consignor-stated retail only",
      anchorLabel: "Consignor-stated retail (not research)",
    };
  }
  return {
    rangeLabel: "Suggested list price based on these factors and what we found",
...
export function getReviewerGuidanceRows(reviewerGuidance) {
  ...
  if (g.noExactRetailComps || (g.typicalRetailLow && g.typicalRetailHigh) || g.notes) {
    ...
    if (g.impliedAskLow != null && g.impliedAskHigh != null) {
      rows.push({
        label: "Suggested list price from that retail band (30–50%)",
...
export function buildFinalPricingSummary({ suggested, item, reviewerGuidance, analysis } = {}) {
  ...
  const suggest =
    suggested?.rangeLow != null && suggested?.rangeHigh != null
      ? `Suggested list price: ${formatUsd(suggested.rangeLow)}–${formatUsd(suggested.rangeHigh)}, based on these factors and what we found (our 30–50% of retail pricing).`
      : guidance.impliedAskLow != null && guidance.impliedAskHigh != null
        ? `Suggested list price from the typical retail band (30–50%): ${formatUsd(guidance.impliedAskLow)}–${formatUsd(guidance.impliedAskHigh)}. Confirm with better comps if you can.`
        : "Not enough verified comps yet for a suggested list price — review the factors and links below.";
