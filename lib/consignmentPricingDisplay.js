...
/** Labels for the consignment reviewer — research to support our 30–50% of retail pricing. */
export function getSuggestedPricingLabels(suggested) {
  ...
  if (anchor === "resale_comp_average") {
    return {
      rangeLabel: "Based on resale comps only — confirm retail before finalizing",
      anchorLabel: "Resale context only — confirm retail",
    };
  }
  if (anchor === "retail_30_50") {
    return {
      rangeLabel: "Based on our pricing of 30–50% of retail",
      anchorLabel: "Retail estimate from research (linked listings)",
    };
  }
  if (anchor === "consignor_retail_30_50") {
    return {
      rangeLabel: "Based on consignor-stated retail only (not research)",
      anchorLabel: "Consignor-stated retail (not research)",
    };
  }
  return {
    rangeLabel: "Based on our pricing of 30–50% of retail (research)",
...
export function getReviewerGuidanceRows(reviewerGuidance) {
  ...
  if (g.noExactRetailComps || (g.typicalRetailLow && g.typicalRetailHigh) || g.notes) {
    if (g.basis) {
      rows.push({ label: "Category / brand basis", valueText: String(g.basis), value: null });
    }
    ...
    if (g.impliedAskLow != null && g.impliedAskHigh != null) {
      rows.push({
        label: "30–50% of that retail band (starting point)",
...
export function buildFinalPricingSummary({ suggested, item, reviewerGuidance, analysis } = {}) {
  ...
  const suggest =
    suggested?.rangeLow != null && suggested?.rangeHigh != null
      ? `Based on our pricing of 30–50% of retail, we think this should be about ${formatUsd(suggested.rangeLow)}–${formatUsd(suggested.rangeHigh)}.`
      : guidance.impliedAskLow != null && guidance.impliedAskHigh != null
        ? `No verified retail comps yet — from the typical retail band above, 30–50% lands around ${formatUsd(guidance.impliedAskLow)}–${formatUsd(guidance.impliedAskHigh)}. Confirm before finalizing.`
        : "Not enough verified comps yet to recommend a price — review the research below.";
...
export function drawPdfPricingSection(doc, pricing, { brandColor, margin, contentWidth, contentBottom, item }) {
  ...
  doc.text("RESEARCHED PRICE", margin + 10, boxY + 7, { width: contentWidth - 20 });
  ...
  // Short why — for the person reviewing this consignment submission
  doc.font("Helvetica-Bold").fontSize(10).fillColor(brandColor).text("Why we think this price", margin);
  doc.moveDown(0.1);
