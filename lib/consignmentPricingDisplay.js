...
/** Customer-facing labels — avoid internal %‑of‑retail jargon in email/PDF. */
export function getSuggestedPricingLabels(suggested) {
  const anchor = String(suggested?.pricingAnchor || "resale_comp_average").toLowerCase();
  if (anchor === "resale_comp_average") {
    return {
      rangeLabel: "Market recommendation",
      anchorLabel: "Resale comp average (verified listings)",
    };
  }
  if (anchor === "consignor_retail_30_50") {
    return {
      rangeLabel: "Market recommendation",
      anchorLabel: "Consignor stated retail (reference only)",
    };
  }
  if (anchor === "retail_30_50") {
    return {
      rangeLabel: "Market recommendation",
      anchorLabel: "Est. original retail (reference only)",
    };
  }
  return {
    rangeLabel: "Market recommendation",
...
/** Secondary pricing rows for email/PDF — never show consignor retail as comp average. */
export function getPricingDetailRows(suggested, comps, item, compEvidence) {
  const rows = [];
  const retail = parseConsignorRetailUsd(item);
  const linkedMode = compEvidence?.mode === "linked_resale_listings";

  if (linkedMode && comps?.average) {
    rows.push({
      label: "Resale comp average (linked listings)",
      value: comps.average,
    });
  }
  if (retail) {
...
/** Rewrite internal anchor/ratio language for team-facing summary text. */
export function formatPricingRationaleForDisplay(rationale, suggested) {
  let s = String(rationale || "").trim();
  if (!s) return s;
  s = s.replace(/\bconsignor_retail_30_50\b/gi, "consignor retail reference");
  s = s.replace(/\bretail_30_50\b/gi, "original retail reference");
  s = s.replace(/\bresale_comp_average\b/gi, "online resale comp average");
  s = s.replace(/30[–-]50%\s+of\s+(?:consignor\s+)?(?:stated\s+)?retail/gi, "market recommendation band");
  s = s.replace(/30[–-]50%\s+yields/gi, "market recommendation range");
  s = s.replace(/suggested ask 30[–-]50% of that/gi, "market recommendation from consignor retail reference");
  const anchor = String(suggested?.pricingAnchor || "").toLowerCase();
  if (anchor === "resale_comp_average" && !/market recommendation/i.test(s)) {
    s = `Market recommendation is based on ${s.charAt(0).toLowerCase()}${s.slice(1)}`;
  }
...
function sanitizeEmailSummaryText(text) {
  ...
  s = s.replace(
    ...
    "Market recommendation is based on online resale comps; see attached PDF. "
  );
...
export function buildCategorizedLinks({ item, analysis }) {
  ...
  for (const entry of analysis?.verifiedCompEntries || []) {
    const url = safeUrl(entry?.url);
    if (!url) continue;
    buckets.resale.push({
      ...
      channel: "resale",
      note:
        analysis?.compEvidence?.mode === "linked_resale_listings"
          ? "Used in resale comp average"
          : "Directional only — not averaged (insufficient linked comps)",
  ...
  if (retail) {
    buckets.customer.push({
      ...
      price: retail,
      channel: "customer",
      note: "Reference only — not a resale comp",
    });
  }
...
function drawPdfCategorizedLinks(doc, categorizedLinks, opts) {
  ...
  const total = (groups.resale?.length || 0) + (groups.customer?.length || 0);
  if (!total) return;
  ...
  doc.font("Helvetica-Bold").fontSize(9.5).fillColor(brandColor).text("Resale comps used in pricing", margin);
  doc.moveDown(0.1);
...
export function drawPdfPricingSection(doc, pricing, { brandColor, margin, contentWidth, contentBottom, item }) {
  ...
  doc.font("Helvetica-Bold").fontSize(10).fillColor(brandColor).text("Comparable Comps (linked resale only)", margin);
  doc.moveDown(0.15);
  if (linkedMode && comps?.average) {
    drawRow("Linked listings used", String(compEvidence?.linkedListingCount ?? "—"));
    ...
    doc
      ...
      .fillColor("#666")
      .text(
        `No resale average — fewer than ${compEvidence?.minRequired ?? 2} linked listings with confirmed prices. See summary and URLs below.`,
        margin,
        doc.y,
