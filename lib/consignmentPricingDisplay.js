 ...
 * Customer-stated retail / implied ask are always separate from research market rec.
 */
export function getPricingDetailRows(suggested, comps, item, compEvidence) {
  ...
  if (Number.isFinite(consignorRetail) && consignorRetail > 0) {
    rows.push({
      label: "Customer-stated retail (input only — not research)",
    ...
    if (impliedLow > 0 && impliedHigh > 0) {
      rows.push({
        label: "Based on customer input — where we would price (30–50%)",
...
export function buildInternalEmailPricingHtml(pricing, item = null) {
  ...
  const guidanceNotes = sanitizeEmailSummaryText(formatReviewerGuidanceNotes(reviewerGuidance));
  ...
  if (mixedNote) {
    ...
    if (Array.isArray(photoBundle?.pieces) && photoBundle.pieces.length) {
      parts.push(`<ul style="margin:0 0 10px;padding-left:18px;font-size:13px;color:#444;">`);
      for (const piece of photoBundle.pieces.slice(0, 6)) {
        const line = [piece.label, piece.description].filter(Boolean).join(" — ");
        if (line) parts.push(`<li style="margin:0 0 4px;">${escapeHtml(line)}</li>`);
      }
      parts.push(`</ul>`);
    }
  }
  parts.push(
    ...
    suggested.rangeLow != null && suggested.rangeHigh != null
      ...
      : `<tr><td style="padding:4px 12px 4px 0;color:#555;">${escapeHtml(labels.rangeLabel)}</td><td style="padding:4px 0;font-weight:600;color:#777;">Pending research comps</td></tr>`
  );
  for (const row of detailRows) {
    const display =
      row.valueText != null && String(row.valueText).trim()
        ...
        : formatUsd(row.value);
    parts.push(
      `<tr><td style="padding:4px 12px 4px 0;color:#555;">${escapeHtml(row.label)}</td><td style="padding:4px 0;">${blurPriceHtml(display)}</td></tr>`
    );
  }
  parts.push(
  ...
  if (guidanceRows.length || guidanceNotes) {
    parts.push(
      `<p style="margin:14px 0 6px;font-size:13px;font-weight:700;color:#1a3c34;text-transform:uppercase;letter-spacing:0.04em;">Reviewer guidance</p>`,
      `<p style="margin:0 0 8px;font-size:12px;color:#777;">Not verified comps — brand/category directional only for the team.</p>`,
      `<table style="border-collapse:collapse;width:100%;font-size:13px;">`
    );
    for (const row of guidanceRows) {
      const display =
        row.valueText != null && String(row.valueText).trim()
          ? String(row.valueText)
          : formatUsd(row.value);
      const blur = /\$|\d/.test(display) && row.label !== "Reviewer guidance basis";
      parts.push(
        `<tr><td style="padding:4px 12px 4px 0;color:#555;">${escapeHtml(row.label)}</td><td style="padding:4px 0;">${
          blur ? blurPriceHtml(display) : escapeHtml(display)
        }</td></tr>`
      );
    }
    parts.push(`</table>`);
    if (guidanceNotes) {
      parts.push(
        `<p style="margin:8px 0 0;font-size:13px;color:#444;line-height:1.55;">${escapeHtml(guidanceNotes)}</p>`
      );
    }
  }
...
export function drawPdfPricingSection(doc, pricing, { brandColor, margin, contentWidth, contentBottom, item }) {
  ...
  const { comparableComps: comps, suggestedPricing: suggested, compEvidence, photoBundle, reviewerGuidance } =
    pricing.analysis;
  const rationale = formatPricingRationaleForDisplay(suggested.rationale, suggested);
  const retailAnchored = compEvidence?.mode === "linked_retail_listings";
  const guidanceRows = getReviewerGuidanceRows(reviewerGuidance);
  const guidanceNotes = formatReviewerGuidanceNotes(reviewerGuidance);
  ...
  if (shouldShowMultiPieceCallout(photoBundle)) {
    ...
    doc.moveDown(0.1);
    if (photoBundle.photoObservations) {
      doc
        .font("Helvetica")
        .fontSize(9.5)
        .fillColor("#333")
        .text(photoBundle.photoObservations, margin, doc.y, { width: contentWidth, lineGap: 2 });
      doc.moveDown(0.15);
    }
    if (Array.isArray(photoBundle.pieces) && photoBundle.pieces.length) {
      doc.font("Helvetica-Bold").fontSize(9.5).fillColor("#333").text("Per-piece notes", margin);
      doc.moveDown(0.08);
      for (const piece of photoBundle.pieces.slice(0, 8)) {
        const line = [piece.label, piece.description, piece.pricingNote]
          .filter(Boolean)
          .join(" — ");
        if (!line) continue;
        doc.font("Helvetica").fontSize(9.5).fillColor("#333").text(`• ${line}`, margin, doc.y, {
          width: contentWidth,
          lineGap: 2,
        });
        doc.moveDown(0.08);
  ...
  if (suggested.rangeLow != null && suggested.rangeHigh != null) {
    ...
    drawRow(labels.rangeLabel, "Pending research comps");
  }
  ...
  if (guidanceRows.length || guidanceNotes) {
    ...
    doc
      ...
      .text("Reviewer guidance (not verified comps)", margin);
    doc.moveDown(0.1);
    doc
      .font("Helvetica-Oblique")
      .fontSize(9)
      .fillColor("#666")
      .text(
        "Brand/category directional range for the team when exact linked retail comps are missing or thin.",
        margin,
        doc.y,
        { width: contentWidth, lineGap: 2 }
      );
    ...
    if (guidanceNotes) {
      doc.moveDown(0.1);
      doc.font("Helvetica").fontSize(9.5).fillColor("#333").text(guidanceNotes, margin, doc.y, {
        width: contentWidth,
        lineGap: 3,
      });
    }
