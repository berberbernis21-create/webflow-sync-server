...
/** Internal intake-reviewer labels — what Lost & Found should list this at. */
export function getSuggestedPricingLabels(suggested) {
  ...
  if (anchor === "resale_comp_average") {
    return {
      rangeLabel: "Intake guide — resale context only (confirm retail before locking)",
      anchorLabel: "Resale context only — confirm retail",
    };
  }
  if (anchor === "retail_30_50") {
    return {
      rangeLabel: "Intake guide — ~30–50% of verified research retail",
      anchorLabel: "Retail estimate from research (linked listings)",
    };
  }
  if (anchor === "consignor_retail_30_50") {
    return {
      rangeLabel: "Intake guide — customer input only (not research)",
      anchorLabel: "Consignor-stated retail (not research)",
    };
  }
  return {
    rangeLabel: "Intake guide — research starting point for the team",
...
export function getCustomerInputRows(suggested, item) {
  ...
  if (impliedLow > 0 && impliedHigh > 0) {
    rows.push({
      label: "If intake used consignor input only (30–50%)",
...
export function getReviewerGuidanceRows(reviewerGuidance) {
  ...
  if (g.noExactRetailComps || (g.typicalRetailLow && g.typicalRetailHigh) || g.notes) {
    if (g.basis) {
      rows.push({ label: "Intake category basis", valueText: String(g.basis), value: null });
    }
    ...
    if (g.impliedAskLow != null && g.impliedAskHigh != null) {
      rows.push({
        label: "Directional intake price from that band (starting point)",
...
export function buildFinalPricingSummary({ suggested, item, reviewerGuidance, analysis } = {}) {
  ...
  const suggest =
    suggested?.rangeLow != null && suggested?.rangeHigh != null
      ? `For intake, price this around ${formatUsd(suggested.rangeLow)}–${formatUsd(suggested.rangeHigh)} (${labels.rangeLabel}).`
      : guidance.impliedAskLow != null && guidance.impliedAskHigh != null
        ? `No verified retail comps yet — directional intake starting range from category research: ${formatUsd(guidance.impliedAskLow)}–${formatUsd(guidance.impliedAskHigh)} (confirm before listing).`
        : "Intake pricing pending until verified comps are confirmed.";

  const customerBit =
    customerRetail?.value != null
      ? `Consignor stated ${formatUsd(customerRetail.value)} (input only — not the research anchor).`
  ...
  return [
    ...
    suggest,
    customerBit,
    "Intake approach: list from verified retail at ~30–50% when available; consignor input is a separate check.",
  ].join(" ");
}
...
/**
 * Slim email block: Evidence only (pricing detail lives in the PDF).
 */
export function buildInternalEmailPricingHtml(pricing, item = null) {
  if (!pricing?.available || !pricing.analysis) {
    return `<p style="margin:8px 0 0;font-size:13px;color:#555;"><strong>Evidence:</strong> Pending — see attached PDF.</p>`;
  }

  const evidence = hasResearchEvidence(pricing.analysis);
  const showMulti = shouldShowMultiPieceCallout(pricing.analysis.photoBundle);
  const parts = [`<p style="margin:8px 0 0;font-size:13px;color:#444;line-height:1.45;">`];
  if (showMulti) {
    parts.push(`<span style="color:#8a3b12;">Multiple pieces flagged. </span>`);
  }
  parts.push(
    evidence
      ? `<strong>Evidence:</strong> Found — see attached PDF.`
      : `<strong>Evidence:</strong> Not found — see attached PDF.`,
    `</p>`
  );
  return parts.join("");
}
...
export function buildInternalEmailPricingText(pricing) {
  if (!pricing?.available || !pricing.analysis) {
    return "Evidence: Pending — see attached PDF.";
  }
  const evidence = hasResearchEvidence(pricing.analysis);
  return evidence
    ? "Evidence: Found — see attached PDF."
    : "Evidence: Not found — see attached PDF.";
}
...
export function isJunkCompSource(url, title = "", snippet = "") {
  ...
  if (
    /keyword\.php|\/keyword\/|\/keywords\/|\/search\?|\/catalogsearch\/|\/filter\?|\/collections\/all\?/.test(
      u
    )
  ) {
    return true;
...
export function buildCategorizedLinks({ item, analysis }) {
  ...
  for (const entry of analysis?.verifiedRetailCompEntries || []) {
    ...
    buckets.retail.push({
      ...
      note: entry.strongVisionMatch
        ? "Strong image match + verified live listing — intake pricing anchor"
        : "Verified live listing — used in retail average / intake pricing guide",
  ...
  for (const entry of analysis?.verifiedResaleCompEntries || []) {
    ...
    buckets.resale.push({
      ...
      price: entry.price,
      channel: "resale",
      note: "Verified live listing — directional resale only (not intake pricing anchor)",
      matchStrength: "weak",
      linkVerified: true,
    });
  }
  ...
  if (retail) {
    buckets.customer.push({
      ...
      price: retail,
      channel: "customer",
      note: "Input only — not used as the intake pricing research anchor",
    });
  }
...
export function drawPdfPricingSection(doc, pricing, { brandColor, margin, contentWidth, contentBottom, item }) {
  ...
  doc.text("INTAKE PRICE GUIDE", margin + 10, boxY + 7, { width: contentWidth - 20 });
  ...
  doc.text(
    `Velocity note for intake: ${suggested.velocityLabel || "Standard Seller"}`,
    margin,
    doc.y,
    { width: contentWidth }
  );
  ...
  // Short why — for the Lost & Found intake reviewer
  doc
    .font("Helvetica-Bold")
    .fontSize(10)
    .fillColor(brandColor)
    .text("Why price it here for intake", margin);
  doc.moveDown(0.1);
