...
export function buildManualReviewReasons(
  ...
  if (oversize.length) {
    const titles = oversize
      ...
      .join(", ");
    reasons.push(
      `Oversized item — size-based 3-person / $130/hr best guess (${titles}: 299+ lb and over 72" H, or over 550 lb). Final may be lower if two people are enough (~$95/hr). Confirm with delivery team (estimate only)`
    );
  }
