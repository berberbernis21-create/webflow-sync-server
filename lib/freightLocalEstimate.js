...
export async function lookupDriveTimeMinutes(destinationAddress) {
  ...
  try {
    ...
    const truckMinutes = applyLocalTruckDriveMinutes(carMinutes) || carMinutes;

    return attachRouteMapFields(
      {
        ok: true,
        drive_minutes: truckMinutes,
...
export function buildManualReviewReasons(
  ...
  if (oversize.length) {
    const titles = oversize
      ...
      .join(", ");
    reasons.push(
      `Oversized item needs confirmation (${titles}: 299+ lb and over 72" H, or over 550 lb) — priced as 3-person / $130/hr best guess (estimate only)`
    );
  }
