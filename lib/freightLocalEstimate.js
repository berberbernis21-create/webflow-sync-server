...
/**
 * Google Distance Matrix (server-side only). Env: GOOGLE_MAPS_API_KEY
 * @returns {{ ok: boolean, drive_minutes?: number, distance_miles?: number, error?: string, provider?: string }}
 */
export async function lookupDriveTimeMinutes(destinationAddress) {
  ...
  const key = String(process.env.GOOGLE_MAPS_API_KEY || process.env.GOOGLE_DISTANCE_MATRIX_API_KEY || "").trim();
  if (!key) {
    return { ok: false, error: "maps_not_configured", provider: "none" };
  }

  const url = new URL("https://maps.googleapis.com/maps/api/distancematrix/json");
  url.searchParams.set("origins", SHOWROOM_ORIGIN);
  ...
  try {
    ...
    if (!res.ok) {
      return { ok: false, error: `maps_http_${res.status}`, provider: "google_distance_matrix" };
    }
    const element = data?.rows?.[0]?.elements?.[0];
    if (!element || element.status !== "OK") {
      return {
        ok: false,
        error: element?.status || data?.status || "maps_no_route",
    ...
    if (!Number.isFinite(seconds) || seconds < 0) {
      return { ok: false, error: "maps_invalid_duration", provider: "google_distance_matrix" };
    }
    return {
      ok: true,
      drive_minutes: Math.max(1, Math.round(seconds / 60)),
      distance_miles: Number.isFinite(meters) ? Math.round((meters / 1609.344) * 10) / 10 : null,
      provider: "google_distance_matrix",
    };
  } catch (err) {
