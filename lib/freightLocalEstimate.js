  ...
  try {
    const res = await fetch("https://routes.googleapis.com/directions/v2:computeRoutes", {
      ...
      body: JSON.stringify({
        ...
        travelMode: "DRIVE",
        // Stable quotes: distance/route time without live traffic.
    ...
    // Prefer staticDuration; with TRAFFIC_UNAWARE duration matches it.
    const durationStr = String(route.staticDuration || route.duration || "");
    ...
    const carMinutes = Math.max(1, Math.round(seconds / 60));
    const driveMinutes = applyLocalTruckDriveMinutes(carMinutes) || carMinutes;

    return attachRouteMapFields(
      {
        ok: true,
        drive_minutes: driveMinutes,
        car_drive_minutes: carMinutes,
...
export function buildManualReviewReasons(
  ...
  if (access.stairs) {
    if (flights >= 2) reasons.push(`${flights} flights of stairs (first free, then $7/flight)`);
  ...
  if (access.needs_more_than_two_people || extraPeople > 0) {
    if (extraPeople >= 2) reasons.push("2 extra people (4 total) requested — estimate only");
  ...
  if (oversize.length) {
    const titles = oversize
      ...
      .join(", ");
    reasons.push(
      `Oversized item — size-based 3-person / ($130/hr) best guess (${titles}: 299+ lb and over 72" H, or 450+ lb). Final may be lower if two people are enough (~$95/hr). Confirm with delivery team (estimate only)`
    );
  }
  ...
  if (isLocalAz && driveMinutes != null && driveMinutes > 75) {
    reasons.push("Long drive from Scottsdale showroom | confirm by phone");
  }
  return reasons;
}
 ...
 * Used for local_az delivery and pickup_az consignor pickup (same pricing rules).
 */
export async function buildLocalEstimateForDestination({
  ...
  if (priced.assembly_confirm_over_60) {
    reviewReasons.push(
      "Disassembly & reassembly over 60 extra minutes | confirm with customer (increases cost)"
    );
  }
  ...
  const stairsOrCrew =
    ...
    Boolean(priced.assembly_confirm_over_60);
  ...
  return {
    ...
    local_estimate: {
      ...
      large_dim_adder: priced.large_dim_adder,
      large_dim_count: priced.large_dim_count,
      heavy_weight_adder: priced.heavy_weight_adder,
      heavy_weight_count: priced.heavy_weight_count,
      ...
      assembly_required: priced.assembly_required,
      assembly_extra_minutes: priced.assembly_extra_minutes,
      assembly_fee: priced.assembly_fee,
      assembly_confirm_over_60: priced.assembly_confirm_over_60,
      assembly_items: priced.assembly_items,
