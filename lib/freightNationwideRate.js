...
function followUpMessage({ miles, measured }) {
  ...
  return (
    ...
    " Every carrier prices freight differently, so ranges are intentionally wide — we confirm the exact quote ourselves with a trusted human partner in the loop (not an automated book-it rate)." +
    ...
    " This is an estimate only. FreightCenter can also be reached at 800-716-7608 if you want a same-day carrier check."
  );
}
...
export async function fetchNationwideLiveRate(submission = {}) {
  ...
  return {
    ...
    follow_up:
      "We will confirm the exact nationwide freight quote with our trusted human partner in the loop — every carrier prices differently. FreightCenter: 800-716-7608 for a same-day check. Warehouse/terminal pickup usually saves about $150–$200.",
  };
}
