...
/** Large submissions can skip AI pricing + PDFs when CONSIGNMENT_HEAVY_MODE=1 (off by default — use more Render RAM instead). */
export const CONSIGNMENT_HEAVY_MODE_ENABLED =
  String(process.env.CONSIGNMENT_HEAVY_MODE || "0").trim() === "1" ||
  String(process.env.CONSIGNMENT_HEAVY_MODE || "").trim().toLowerCase() === "true";

export const HEAVY_CONSIGNMENT_MIN_PHOTOS = Math.max(
  12,
  parseInt(process.env.CONSIGNMENT_HEAVY_PHOTO_COUNT || "25", 10) || 25
);

export const HEAVY_CONSIGNMENT_MIN_ITEMS = Math.max(
  6,
  parseInt(process.env.CONSIGNMENT_HEAVY_ITEM_COUNT || "10", 10) || 10
);

export function isHeavyConsignmentSubmission(itemCount, photoCount) {
  if (!CONSIGNMENT_HEAVY_MODE_ENABLED) return false;
...
export const SUBMIT_FORM_CLIENT_GUIDANCE = {
  ...
  brandTip:
    "Brand or maker is required. Enter the brand if known, or Unknown. This helps prioritize your item for review.",
  individualItemsTip:
    "Submitting each item individually gives you a better chance at approval.",
  reviewNote:
