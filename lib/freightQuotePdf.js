...
import { formatCustomerAssemblyTimeLabel, customerAssemblyTimeLines, isArizonaStateOrZip } from "./freightPalletize.js";
...
function drawTwoColRows(doc, leftRows, rightRows, { valueColorRight = "#222" } = {}) {
  ...
  const labelW = 112;
  const valueW = Math.max(40, colW - labelW - 4);
  const n = Math.max(leftRows.length, rightRows.length);
  let y = doc.y;
  for (let i = 0; i < n; i++) {
    const L = leftRows[i];
    const R = rightRows[i];
    const leftLabel = L ? String(L[0] ?? "") : "";
    const rightLabel = R ? String(R[0] ?? "") : "";
    const leftVal = L ? String(L[1] ?? "—") : "";
    const rightVal = R ? String(R[1] ?? "—") : "";
    // Measure wrapped label + value height so long copy cannot overlap the next row.
    doc.font("Helvetica").fontSize(8);
    const leftLabelH = L ? doc.heightOfString(leftLabel, { width: labelW }) : 0;
    const rightLabelH = R ? doc.heightOfString(rightLabel, { width: labelW }) : 0;
    doc.font("Helvetica-Bold").fontSize(8);
    const leftValH = L ? doc.heightOfString(leftVal, { width: valueW }) : 0;
    const rightValH = R ? doc.heightOfString(rightVal, { width: valueW }) : 0;
    const rowH =
      Math.max(11, leftLabelH, rightLabelH, leftValH, rightValH) + 3;
    if (y + rowH > pageBottom(doc)) {
      doc.addPage();
      y = MARGIN;
    }
    if (L) {
      doc.font("Helvetica").fontSize(8).fillColor(MUTED).text(leftLabel, MARGIN, y, {
        width: labelW,
      });
      doc
        .font("Helvetica-Bold")
        .fontSize(8)
        .fillColor("#222")
        .text(leftVal, MARGIN + labelW + 4, y, {
          width: valueW,
        });
    }
    if (R) {
      ...
      const isYes =
        rightVal === "Yes" ||
        rightVal.startsWith("Yes —") ||
        rightVal.startsWith("Yes-") ||
        /^\d+\s*minutes?/i.test(rightVal) ||
        /Additional\s+\d+/i.test(rightVal) ||
        /Total\s+\d+/i.test(rightVal);
      doc.font("Helvetica").fontSize(8).fillColor(MUTED).text(rightLabel, x, y, {
        width: labelW,
      });
      doc
        .font("Helvetica-Bold")
...
export async function generateFreightQuotePdf(submission, ctx = {}, options = {}) {
  ...
  if (isLocal && Number(localEstimate?.assembly_extra_minutes) > 0) {
    const lines = customerAssemblyTimeLines(localEstimate.assembly_extra_minutes);
    if (lines) {
      requestLeft.push([lines.extraTitle, lines.extraLabel]);
      requestLeft.push([lines.totalTitle, lines.totalLabel]);
    }
  } else if (isLocal && localEstimate?.assembly_required) {
    requestLeft.push(["Disassembly & Reassembly", "Within 15 min (no extra charge)"]);
  }
    ...
    if (!items.length) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        doc
          ...
          .fillColor(NAVY)
          .text(
            `${row.index}. ${customerFacingTitle(row, "Item")}`,
            textX,
            y0,
            { width: textW }
          );
