...
import { isArizonaStateOrZip } from "./freightPalletize.js";
    ...
    access.needs_more_than_two_people
      ? `Yes — ${
          Number(access.extra_people) >= 2
            ? "2 extra people / two trucks"
            : Number(access.extra_people) === 1
              ? "1 extra person ($130/hr)"
              : "crew TBD"
    ...
    (() => {
      ...
      if (req === "yes") {
        ...
        if (over === "yes") {
          ...
          return `Yes · +${mins} min beyond 15`;
        }
        if (over === "idk") return "Yes · over 15 min: idk";
        return "Yes (within 15 min)";
      }
...
export async function generateFreightQuotePdf(submission, ctx = {}, options = {}) {
  ...
  if (isLocal && Number(localEstimate?.assembly_fee) > 0) {
    ...
    requestLeft.push([
      ...
      `${localEstimate.assembly_extra_minutes || 0} Minutes for Disassembly & Reassembly (${money(
        localEstimate.assembly_fee
      )})`,
    ]);
