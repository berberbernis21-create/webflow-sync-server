...
export async function generateFreightQuotePdf(submission, ctx = {}, options = {}) {
  ...
  // Internal clipboard only — never expose hourly rates on the customer summary PDF.
  if (isLocal && audience === "internal") {
    requestLeft.push(["Rate", "$95/hr round-trip"]);
  }
  ...
  const headline =
    audience === "customer"
      ? `Your request summary  ·  ${pathLabel}  ·  ${estimate}`
