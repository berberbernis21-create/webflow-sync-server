      ...
      if (req === "yes") {
        const over = String(access.assembly_over_15 || "").toLowerCase();
                if (over === "yes") {
                  ...
                  return `Yes · +${mins} min above free 15`;
                }
                if (over === "idk") return "Yes · over free 15 min: idk";
                return "Yes (within free 15 min)";
              }
...
export async function generateFreightQuotePdf(submission, ctx = {}, options = {}) {
  ...
  if (isLocal && Number(localEstimate?.assembly_fee) > 0) {
    ...
    requestLeft.push([
      ...
      `${localEstimate.assembly_extra_minutes || 0} Minutes for Disassembly & Reassembly`,
    ]);
