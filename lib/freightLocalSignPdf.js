...
export async function generateLocalSignPdf(submission, ctx = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Header: showroom (left) · LF logo (center) · truck (right)
    const headerH = 58;
    const logoSize = 54;
    const truckW = 96;
    const truckH = 52;
    ...
    if (storeBuf) {
      try {
        doc.image(storeBuf, MARGIN, headerMidY - storeH / 2, {
          width: storeW,
          height: storeH,
          fit: [storeW, storeH],
          align: "center",
    ...
    const logoX = MARGIN + (CONTENT_WIDTH - logoSize) / 2;
    if (logoBuf) {
      try {
        doc.image(logoBuf, logoX, headerMidY - logoSize / 2, {
          width: logoSize,
          height: logoSize,
        ...
        doc
          ...
          .fontSize(11)
          .fillColor(NAVY)
          .text("LOST + FOUND", logoX - 10, headerMidY - 6, { width: logoSize + 20, align: "center" });
      }
    } else {
      doc
        .font("Helvetica-Bold")
    ...
    const truckX = MARGIN + CONTENT_WIDTH - truckW;
    drawDeliveryTruckIcon(doc, truckX, headerMidY - truckH / 2, truckW, truckH);
    y += headerH + 6;

    // Day-of blank fill-ins
    ...
    const disclosureText = [
      ...
      "Installation, assembly beyond basic placement, electrical, plumbing, gas, HVAC, wall anchoring, and any utility or fixture work are NOT included and are NOT the responsibility or liability of Lost & Found.",
