...
export async function generateLocalSignPdf(submission, ctx = {}) {
  ...
  const truckBuf = readBrand("cruz-pro-line-truck.png");

  // Never print internal hourly rates on this customer-facing clipboard form.
  const rateNote =
    Number(submission.access?.extra_people) >= 1 || localEstimate?.oversize_confirm
      ? "Stairs, oversized items, or extra crew may change pricing — confirm before scheduling"
      : "Confirm pricing before scheduling. Stairs or special handling may change the final price.";
  ...
  return new Promise((resolve, reject) => {
    ...
    // Header branding: logo · delivery truck · storefront
    const headerH = 54;
    const truckW = 72;
    const truckH = 50;
    const storeW = 86;
    ...
    if (logoBuf) {
      try {
        doc.image(logoBuf, MARGIN, y + 2, { height: 48, fit: [48, 48] });
      } catch {
        doc.font("Helvetica-Bold").fontSize(12).fillColor(NAVY).text("LOST + FOUND", MARGIN, y + 12);
      }
    } else {
      doc.font("Helvetica-Bold").fontSize(12).fillColor(NAVY).text("LOST + FOUND", MARGIN, y + 12);
    }
    if (truckBuf) {
      try {
        ...
        doc.roundedRect(truckX - 4, y - 1, truckW + 8, truckH + 2, 6).fill("#f7f4ee");
        doc.image(truckBuf, truckX, y, {
          width: truckW,
          height: truckH,
          fit: [truckW, truckH],
          align: "center",
    ...
    if (storeBuf) {
      try {
        doc.image(storeBuf, MARGIN + CONTENT_WIDTH - storeW, y + 2, {
          width: storeW,
          height: storeH,
          fit: [storeW, storeH],
          align: "center",
    ...
    // Blank fill-ins for day-of / shop use (below logo + truck)
    const fillColW = (CONTENT_WIDTH - 12) / 2;
    drawLineField(doc, "DELIVERY COMPANY", MARGIN, y, fillColW, "");
    drawLineField(doc, "CONFIRMED RATE (IF REQUIRED)", MARGIN + fillColW + 12, y, fillColW, "");
    y += 22;
