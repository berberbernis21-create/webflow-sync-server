 ...
 * @param {{ valueSize?: number, labelSize?: number, valuePad?: number }} [opts]
 */
function drawLineField(doc, label, x, y, width, value = "", opts = {}) {
  ...
  doc
    ...
    .lineWidth(1)
    .stroke();
  doc.font("Helvetica").fontSize(labelSize).fillColor(MUTED).text(String(label || ""), x, lineY + 3, {
    width,
    lineBreak: false,
  });
...
export async function generateLocalSignPdf(submission, ctx = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Team fill-ins — large write-in / read-in fields
    const fillColW = (CONTENT_WIDTH - 14) / 2;
    const fillOpts = { valueSize: 14, labelSize: 9, valuePad: 18 };
    const fillPanelH = 52;
    doc.roundedRect(MARGIN, y, CONTENT_WIDTH, fillPanelH, 4).lineWidth(1.1).strokeColor(BORDER).stroke();
    drawLineField(doc, "DELIVERY COMPANY", MARGIN + 12, y + 8, fillColW - 14, "", fillOpts);
    drawLineField(doc, "CONFIRMED RATE", MARGIN + fillColW + 14, y + 8, fillColW - 14, "$95/hr", fillOpts);
    y += fillPanelH + 3;
    doc
      .font("Helvetica")
      .fontSize(8)
    ...
    const infoOpts = { valueSize: 12, labelSize: 8, valuePad: 15 };
    drawLineField(doc, "NAME", MARGIN, y, colW, submission.customer_name || "", infoOpts);
    drawLineField(doc, "PHONE", MARGIN + colW + 14, y, colW, formatPhone(submission.customer_phone), infoOpts);
    y += FIELD_ROW + 2;
    ...
    const driveText =
      oneWay != null
        ...
        : miles != null
          ? `${miles} mi`
          : "";
    drawLineField(doc, "DRIVE / DISTANCE", MARGIN + colW + 14, y, colW, driveText, infoOpts);
    y += FIELD_ROW + 2;
    drawLineField(doc, "DATE OF DELIVERY", MARGIN, y, colW, "", infoOpts);
    drawLineField(doc, "TIME WINDOW", MARGIN + colW + 14, y, colW, "", infoOpts);
    y += FIELD_ROW + 4;
    ...
    // Items full width (no route map)
    const pageBottom = () => doc.page.height - MARGIN - 12;
    const reserveForFooter = 96;
    let textWidth = CONTENT_WIDTH - 36;

    doc.font("Helvetica-Bold").fontSize(10).fillColor(NAVY).text("ITEMS", MARGIN, y);
    y = doc.y + 2;
    doc.moveTo(MARGIN, y).lineTo(MARGIN + CONTENT_WIDTH, y).strokeColor(NAVY).lineWidth(1).stroke();
    y += 4;

    doc.font("Helvetica-Bold").fontSize(8).fillColor(MUTED).text("QTY", MARGIN, y, { width: 32 });
    doc
      .font("Helvetica-Bold")
      .fontSize(8)
      .fillColor(MUTED)
      .text("DESCRIPTION / ITEMS FROM LOST + FOUND", MARGIN + 34, y, { width: CONTENT_WIDTH - 34 });
    let itemsY = y + 12;

    function startItemsContinuationPage() {
      doc.addPage();
      itemsY = MARGIN;
      textWidth = CONTENT_WIDTH - 36;
      doc.font("Helvetica-Bold").fontSize(10).fillColor(NAVY).text("ITEMS (continued)", MARGIN, itemsY);
      itemsY = doc.y + 3;
      doc.moveTo(MARGIN, itemsY).lineTo(MARGIN + CONTENT_WIDTH, itemsY).strokeColor(NAVY).lineWidth(1).stroke();
      itemsY += 5;
      doc.font("Helvetica-Bold").fontSize(8).fillColor(MUTED).text("QTY", MARGIN, itemsY, { width: 32 });
      doc
        .font("Helvetica-Bold")
        .fontSize(8)
        .fillColor(MUTED)
        .text("DESCRIPTION / ITEMS FROM LOST + FOUND", MARGIN + 34, itemsY, {
    ...
    if (!items.length) {
      doc.font("Helvetica").fontSize(9).fillColor(INK).text("No items listed", MARGIN + 34, itemsY);
      itemsY += 13;
    } else {
      for (let i = 0; i < items.length; i++) {
        ...
        const rowH = Math.max(13, doc.heightOfString(bits, { width: textWidth }) + 2);
        ensureItemRowSpace(rowH);
        doc.font("Helvetica-Bold").fontSize(10).fillColor(INK).text(String(qty), MARGIN, itemsY, {
          width: 32,
          lineBreak: false,
        });
        doc.font("Helvetica").fontSize(9.5).fillColor(INK).text(bits, MARGIN + 34, itemsY, {
          width: textWidth,
        });
    ...
    // IMPORTANT — a bit larger for readability; keep compact enough for one page.
    const signatureBlockH = 58;
    const disclosureBodyEstimate = 56;
    const neededTail = 12 + disclosureBodyEstimate + signatureBlockH + 8;
    ...
    doc
      ...
      .fontSize(8.5)
      .fillColor(NAVY)
      .text("IMPORTANT — PLEASE READ BEFORE SIGNING", MARGIN + 8, disclosureTop + 5, {
        width: CONTENT_WIDTH - 16,
      });

    let dy = disclosureTop + 17;
    ...
    disclosurePlain(
      "Preliminary estimate only — final price must be confirmed and may change based on access, stairs, item size/weight, and conditions on the day of service. Third-party movers (e.g. Cruz Pro Line) are paid directly (cash, check, or Zelle)."
    );
    disclosurePlain("Full furniture installation, ", { continued: true });
    ...
    disclosurePlain(
      "By signing, customer agrees to pay all fees and understands this is not a final locked rate."
    );

    const disclosureBottom = dy + 3;
    doc
      .roundedRect(MARGIN, disclosureTop, CONTENT_WIDTH, Math.max(48, disclosureBottom - disclosureTop), 4)
      .lineWidth(1)
      .strokeColor(BORDER)
      .stroke();
    y = Math.max(disclosureBottom, disclosureTop + 48) + 6;
    ...
    drawLineField(doc, "DATE", MARGIN + sigW + nameW + 16, y, dateW, "", infoOpts);
    y += FIELD_ROW + 2;
    ...
    doc
      ...
      .text(
        ...
        } · Print for delivery clipboard / customer signature`,
        MARGIN,
        footerY,
