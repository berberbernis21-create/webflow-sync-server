...
export async function generateLocalSignPdf(submission, ctx = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // IMPORTANT — larger, sits under items; signature pinned to page bottom below.
    if (y + disclosureReserve + signatureBlockH + footerReserve > pageBottom()) {
      doc.addPage();
      y = MARGIN;
    }
    ...
    // Pin signature + footer to the very bottom of the page (no FQ id).
    const absoluteBottom = doc.page.height - MARGIN;
    const footerY = absoluteBottom - 10;
    let sigY = footerY - signatureBlockH;
    if (disclosureBottom + 6 > sigY) {
      doc.addPage();
      sigY = absoluteBottom - signatureBlockH - 10;
    }
