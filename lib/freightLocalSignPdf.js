...
export async function generateLocalSignPdf(submission, ctx = {}) {
  ...
  const rateNote = localEstimate?.oversize_confirm
    ...
    : Number(submission.access?.extra_people) >= 1
      ...
      : Number(localEstimate?.assembly_fee) > 0
        ? `Additional Disassembly & Reassembly Time | ${localEstimate.assembly_extra_minutes || 0} Minutes`
  ...
  const estimateLine =
    assemblyFeeAmt > 0 && assemblyMinsAmt > 0
      ? `Local Arizona delivery  ·  Estimate ${estimate}  ·  +${assemblyMinsAmt} min disassembly & reassembly`
      : `Local Arizona delivery  ·  Estimate ${estimate}`;
        ...
        const assemblyBit =
          String(row.assembly_required || "").toLowerCase() === "yes"
            ? String(row.assembly_over_15 || "").toLowerCase() === "yes"
              ? `+${Math.max(0, Math.floor(Number(row.assembly_extra_minutes) || 0))}m reassembly`
