...
export async function generateLocalSignPdf(submission, ctx = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const reserveForFooter = 118; // disclosure + signature block approx when cramped beside map
    ...
    // Disclosures + signatures: keep together; move to next page if needed.
    const disclosureTitleH = 14;
    const disclosureBodyEstimate = 95;
    const signatureBlockH = 78;
    const neededTail = disclosureTitleH + disclosureBodyEstimate + signatureBlockH + 16;
    ...
    doc
      ...
      .fontSize(8)
      .fillColor(NAVY)
      .text("IMPORTANT — PLEASE READ BEFORE SIGNING / INITIALING", MARGIN + 8, disclosureTop + 6, {
    ...
    disclosurePlain(
      "This is a preliminary estimate only. Final pricing must be confirmed before scheduling and may change based on access, stairs, crew size, item size/weight, distance, wait time, labor, disassembly & reassembly time, special handling, and conditions on the day of service."
    );
    disclosurePlain(
    ...
    disclosurePlain(
      "Estimated disassembly & reassembly time from the online questionnaire is included in this estimate when answered. Full furniture installation, ",
      { continued: true }
    );
    ...
    disclosurePlain(
      "By signing and initialing, customer agrees to pay all fees and understands the price is subject to change depending on conditions — this is not a final locked rate."
    );

    const disclosureBottom = dy + 4;
    doc
      .roundedRect(MARGIN, disclosureTop, CONTENT_WIDTH, Math.max(72, disclosureBottom - disclosureTop), 4)
      .lineWidth(1)
      .strokeColor(BORDER)
      .stroke();
    y = Math.max(disclosureBottom, disclosureTop + 72) + 10;
