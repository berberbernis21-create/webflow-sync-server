...
export async function generateLocalSignPdf(submission, ctx = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // IMPORTANT block stays the prior readable size (do not enlarge).
    const signatureBlockH = 68;
    const disclosureBodyEstimate = 52;
    ...
    disclosurePlain(
      "Preliminary estimate only — final price must be confirmed and may change based on access, stairs, item size/weight, distance, labor, and conditions on the day of service. Third-party movers (e.g. Cruz Pro Line) are paid directly (cash, check, or Zelle)."
    );
    ...
    drawLineField(doc, "CUSTOMER SIGNATURE", MARGIN, y, colW + 40, "", infoOpts);
    drawLineField(doc, "DATE", MARGIN + colW + 52, y, colW - 40, "", infoOpts);
    y += FIELD_ROW + 2;
    drawLineField(doc, "PRINT NAME", MARGIN, y, colW + 40, submission.customer_name || "", infoOpts);
    drawLineField(doc, "CUSTOMER INITIALS", MARGIN + colW + 52, y, colW - 40, "", infoOpts);
    y += FIELD_ROW + 2;
