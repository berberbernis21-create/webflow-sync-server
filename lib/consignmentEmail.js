...
const PRICING_UNAVAILABLE_REASONS = {
  ...
  budget_exceeded:
    "Comparable analysis in progress or unavailable (submission time budget exceeded).",
...
function pricingUnavailableHeadline(pricing) {
  if (String(pricing?.reason || "").trim() === "budget_exceeded") {
    return "Comparable analysis in progress or unavailable";
  }
...
function buildPricingSectionText(pricing) {
  if (!pricing?.available || !pricing.analysis) {
    ...
    const detail = pricingUnavailableDetail(pricing);
    const reason = String(pricing?.reason || "").trim();
    if (reason === "budget_exceeded") return headline;
...
export function buildCustomerConfirmationEmail(submission, items, groupedPhotos, { submittedAt } = {}) {
  ...
  const html = [
    ...
    `<p style="margin:0 0 6px;font-family:${EMAIL_FONT};font-size:26px;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;color:#ffffff;line-height:1.2;">Lost &amp; Found</p>`,
    `<p style="margin:0 0 8px;font-family:${EMAIL_FONT};font-size:14px;font-weight:600;letter-spacing:0.04em;color:${EMAIL_COLOR_ACCENT};line-height:1.3;">Resale Interiors</p>`,
    ...
    `<p style="margin:0 0 14px;font-family:${EMAIL_FONT};font-size:15px;line-height:1.6;color:${EMAIL_COLOR_TEXT};">Our team is reviewing your submission and will get back to you within 1-2 business days.</p>`,
