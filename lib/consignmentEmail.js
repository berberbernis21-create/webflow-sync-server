...
function findPricingForItem(pricingResults, itemNumber) {
  ...
  return pricingResults.find((p) => p.itemNumber === itemNumber) || null;
}

function buildPricingSectionHtml(pricing) {
  if (!pricing?.available || !pricing.analysis) {
    return `<div style="margin-top:16px;padding:12px;background:#f9f7f4;border-radius:6px;border:1px solid #e8e2d9;"><p style="margin:0;font-size:13px;color:#777;font-style:italic;">Comparable analysis unavailable</p></div>`;
  }
...
export function buildCustomerConfirmationEmail(submission, items, groupedPhotos, { submittedAt } = {}) {
  ...
  const html = [
    ...
    `<p style="margin:0 0 24px;font-family:${EMAIL_FONT};font-size:15px;line-height:1.6;color:${EMAIL_COLOR_TEXT};">A copy of your <strong style="color:${EMAIL_COLOR_HEADING};">submission summary</strong> and our <strong style="color:${EMAIL_COLOR_HEADING};">consignment reference guide</strong> is attached as a PDF for your records.</p>`,
  ...
  const text = [
    ...
    "",
    "A copy of your submission summary and our consignment reference guide is attached as a PDF for your records.",
    "",
    "Submission Summary:",
...
/** Send customer confirmation via Resend (optional PDF attachment). */
export async function sendCustomerConfirmationEmail(
  submission,
  items,
  groupedPhotos,
  { submittedAt, customerPdfBuffer, customerPdfFilename } = {}
) {
  ...
  const attachments = [];
  if (customerPdfBuffer?.length && customerPdfFilename) {
    attachments.push({
