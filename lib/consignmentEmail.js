...
export function buildCustomerConfirmationEmail(submission, items, groupedPhotos, { submittedAt } = {}) {
  ...
  const html = [
    ...
    `<p style="margin:0 0 14px;font-family:${EMAIL_FONT};font-size:15px;line-height:1.6;color:${email.colorText};">Pricing is provided post-acceptance, intake, and in-person review via the consignment contract that will be sent to you.</p>`,
  ...
  const text = [
    ...
    "",
    "Pricing is provided post-acceptance, intake, and in-person review via the consignment contract that will be sent to you.",
    "",
...
export function buildConsignmentEmail({
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    itemHtml += buildInternalEmailPricingHtml(pricing);
  ...
  const html = [
    ...
    `<p style="margin:0 0 16px;font-size:13px;color:#666;">See pricing in the attached PDF.</p>`,
