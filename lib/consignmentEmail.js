...
export function buildConsignmentEmail({
  ...
  const html = [
    ...
    `<p style="margin:0 0 14px;font-size:14px;color:#555;">New ${escapeHtml(verticalLabel.toLowerCase())} · ${escapeHtml(displayValue(consignorName))} · ${escapeHtml(itemsLabel)} item(s) · ${uploadedPhotoCount} photo(s)</p>`,
    ...
    `<p style="margin:0 0 4px;font-size:13px;color:#666;">Consignor contact, research links, and full rationale are in the PDF.</p>`,
    `<h3 style="margin:18px 0 0;color:${accentColor};font-size:15px;">Items — pricing snapshot</h3>`,
    inlineSections.join(""),
    `</div>`,
  ].join("");
  ...
  const textBlocks = [
    ...
    `${displayValue(consignorName)} · ${itemsLabel} item(s) · ${uploadedPhotoCount} photo(s)`,
    "",
    `Internal only. ${INTERNAL_EMAIL_PRICING_NOTICE}`,
    "Consignor contact and full research detail are in the attached PDF.",
  ];
