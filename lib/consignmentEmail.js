...
import { getConsignmentBrand, getItemDetailFields } from "./consignmentBrand.js";
import {
  displayValue,
  resolveItemNumber,
...
export function buildConsignmentEmail({
  ...
  const summaryRows = [
    ...
    ["Email", displayValue(consignorEmail || body.customerEmail)],
  ...
  for (const [label, value] of summaryRows) {
    summaryHtml += `<tr><td style="padding:6px 12px 6px 0;font-weight:600;vertical-align:top;white-space:nowrap;">${escapeHtml(label)}</td><td style="padding:6px 0;">${escapeHtml(value)}</td></tr>`;
  }
  summaryHtml += `</table>`;
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const parentSubmissionNumber = item?._splitChild ? item.splitFromItemNumber : itemNumber;
    const uploadedForItem = sourcePhotoGroups.get(parentSubmissionNumber) || [];
    ...
    if (item?._splitChild) {
      itemHtml += `<p style="margin:0 0 12px;font-size:13px;color:#8a3b12;font-weight:600;">Split piece ${item.splitPieceIndex} of ${item.splitPieceCount} from submission Item #${item.splitFromItemNumber} (confidence ${escapeHtml(String(item.splitConfidence || "").toUpperCase())}) — analyzed separately, not blended.</p>`;
    }
    // Customer-provided details only (research lives in the pricing box below).
    ...
    itemHtml += `<li><strong>Photos uploaded (submission line):</strong> ${uploadedForItem.length}</li>`;
    itemHtml += `<li><strong>Photos attached (this piece):</strong> ${photos.length}</li>`;
    itemHtml += `</ul>`;
    ...
    if (photos.length) {
      ...
      for (let p = 0; p < photos.length; p++) {
        ...
        // Short /api/consignment-lens/:token link:
        // - desktop → 302 straight into Lens (same as uploadbyurl)
        // - iPhone Outlook → bridge page with a tap-to-open button
        const lensUrl = file.googleLensUrl || file.googleLensDirectUrl;
        ...
        try {
          ...
          itemHtml += `<table style="border-collapse:collapse;margin:8px 0 4px;width:100%;"><tr>`;
          itemHtml += `<td style="padding:0 8px 0 0;font-size:13px;color:#555;vertical-align:middle;">${escapeHtml(filename)}</td>`;
  ...
  textBlocks.push("", "Summary:", ...summaryRows.map(([k, v]) => `  ${k}: ${v}`), "", "Items:");
