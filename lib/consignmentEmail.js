...
export function buildConsignmentEmail({
  ...
  const itemsLabel = splitExpanded
    ? `${itemCount} analysis items (from ${submittedItemCount} submission line${submittedItemCount === 1 ? "" : "s"}; split when confidently distinct)`
    : String(itemCount);
  ...
  const summaryHtml = buildConsignorProfileTableHtml({
    ...
    if (photos.length) {
      ...
      for (let p = 0; p < photos.length; p++) {
        ...
        try {
          ...
          itemHtml += `<td style="padding:0;text-align:right;vertical-align:middle;white-space:nowrap;">${buildGoogleItemSearchButtonHtml(item, { photoIndex, lensUrl: file.googleLensUrl })}</td>`;
          itemHtml += `</tr></table>`;
  ...
  const html = [
    ...
    `<h3 style="margin:0 0 12px;color:${accentColor};">Consignor profile</h3>`,
    summaryHtml,
