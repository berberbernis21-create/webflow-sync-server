...
function nationwideInternalPlaybookHtml(submission = {}) {
  ...
  const oversized = hasOversizedPalletItems(submission.items || []);
  ...
  return [
    ...
    oversized
      ? `<p style="margin:0 0 10px;font-size:13px;line-height:1.5;color:#9c2f2f;font-weight:700;">⚠ Oversized vs 48×40 — solve pallet fit first (see step 1).</p>`
      : "",
    `<ol style="margin:0 0 12px;padding-left:18px;font-size:14px;line-height:1.55;color:#222;">`,
    oversized
      ? `<li style="margin:0 0 8px;"><strong>Before FreightCenter or Luke:</strong> This item is larger than a standard <strong>48″×40″</strong> pallet. Figure out whether we can stand it up differently, rotate it, or otherwise maneuver it to fit a standard pallet — that can cut the freight cost a lot. Only then get quotes using the best realistic pallet entry.</li>`
      : "",
    `<li style="margin:0 0 8px;"><strong>${
      oversized ? "Then get" : "Quickly get"
    } our own quote</strong> on <a href="https://www.freightcenter.com" style="color:${NAVY};font-weight:700;">freightcenter.com</a> using the freight-ready pallet entry below (usually <strong>48×40</strong>${
      oversized ? " after you confirm the best fit" : ""
    }). Login + steps: <strong>${escapeHtml(FREIGHTCENTER_SOP_PATH)}</strong>.</li>`,
...
function nationwideInternalPlaybookText(submission = {}) {
  ...
  return [
    ...
    oversized
      ? "1) BEFORE FreightCenter or Luke: item is larger than a standard 48x40 pallet. Figure out whether we can stand it up differently, rotate it, or otherwise maneuver it to fit a standard pallet — that can cut freight a lot. Only then quote with the best realistic pallet entry."
      : null,
    `${oversized ? "2" : "1"}) Quickly get our own quote on https://www.freightcenter.com using the 48x40 (or best-fit) pallet entry. SOP/login: ${FREIGHTCENTER_SOP_PATH}`,
    `${oversized ? "3" : "2"}) Forward this email to Luke Rogers <${FREIGHTCENTER_LUKE_EMAIL}> and ask for options: cheapest, best/quality, warehouse pickup, and other good alternatives. CC the customer on that email, or forward Luke's reply to the customer afterward.`,
    `${oversized ? "4" : "3"}) Always stick with what Luke says — his confirmed options are the source of truth for the customer quote.`,
