...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  const addr =
    ...
    [submission.street, submission.unit, submission.city, submission.state, submission.zip]
      .filter(Boolean)
      .join(", ");
  const firstName = String(submission.customer_name || "there").trim().split(/\s+/)[0] || "there";
  ...
  // —— Customer: clean thank-you + estimate note + contact / follow-up ——
  const estimateLine =
    isLocal && localEstimate?.estimated_price != null
      ? `Your preliminary local delivery estimate is <strong>${escapeHtml(
          money(localEstimate.estimated_price)
        )}</strong>${
          route?.drive_minutes || localEstimate.drive_minutes
            ? ` based on about <strong>${escapeHtml(
                route?.drive_minutes ?? localEstimate.drive_minutes
              )} minutes</strong> one-way from our Scottsdale showroom.`
            : "."
        }`
      : isLocal
  ...
  const customerHtml = wrapEmail(
    [
      `<h2 style="margin:0 0 12px;color:${NAVY};font-size:22px;">Thank you, ${escapeHtml(
        firstName
      )}</h2>`,
      `<p style="margin:0 0 14px;font-size:15px;color:#333;">${estimateLine}</p>`,
      `<p style="margin:0 0 14px;font-size:14px;color:#444;">This is a <strong>preliminary estimate only</strong> — not a booking and not a final quote. Access, labor, item handling, carrier pricing, and scheduling can change the final amount. Our team will follow up to confirm if you need firm rates.</p>`,

      (submission.items || []).length
        ? `<p style="margin:0 0 6px;font-size:13px;font-weight:800;color:${NAVY};text-transform:uppercase;letter-spacing:0.04em;">What you submitted</p>${customerItemsHtml(
            submission.items
      ...
      `<p style="margin:0 0 8px;font-size:14px;font-weight:800;color:${NAVY};">Questions or ready to confirm?</p>`,
      `<p style="margin:0;font-size:14px;line-height:1.6;color:#333;">Feel free to email us at <a href="mailto:${LOST_FOUND_EMAIL}" style="color:${NAVY};font-weight:700;">${LOST_FOUND_EMAIL}</a> or call <a href="tel:${LOST_FOUND_PHONE.replace(
        /\D/g,
        ""
      )}" style="color:${NAVY};font-weight:700;">${LOST_FOUND_PHONE}</a>.<br/>We will follow up to confirm details and rates if you need them.</p>`,
      `</div>`,
  ...
  const customerText = [
    ...
    isLocal && localEstimate?.estimated_price != null
      ? `Your preliminary local delivery estimate is ${money(localEstimate.estimated_price)}${
          route?.drive_minutes || localEstimate.drive_minutes
            ? ` (about ${route?.drive_minutes ?? localEstimate.drive_minutes} minutes one-way from Scottsdale).`
            : "."
        }`
      : isLocal
    ...
    "",
    "This is a preliminary estimate only — not a booking and not a final quote. We will follow up to confirm if you need firm rates.",
    "",
    "What you submitted:",
    ...
    `Delivery: ${addr}`,
    "",
    `Questions? Email ${LOST_FOUND_EMAIL} or call ${LOST_FOUND_PHONE}.`,
    "Feel free to reach out anytime — we will follow up to confirm details and rates if you need them.",
    "",
    `Reference: ${requestId}`,
