...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  const customerHtml = wrapEmail(
    [
      ...
      isLocal
        ? ""
        : `<p style="margin:18px 0 12px;font-size:14px;color:#444;">Nationwide ranges are wide because every carrier prices freight differently. We confirm the exact quote ourselves with a trusted <strong>human partner in the loop</strong> — not an automated book-it rate. White Glove / Room of Choice raise cost significantly; warehouse/terminal pickup usually saves about $150–$200 (we will mention this even if you did not select it); no liftgate at delivery and store loading at our end also help.</p>
           <p style="margin:0 0 12px;font-size:14px;color:#444;"><strong>FreightCenter</strong>: <a href="tel:${FREIGHTCENTER_PHONE.replace(/\D/g, "")}" style="color:${NAVY};font-weight:700;">${FREIGHTCENTER_PHONE}</a> — optional same-day carrier check while we confirm your exact quote.</p>`,
  ...
  const customerText = [
    ...
    !isLocal
      ? "Nationwide ranges are wide — every carrier prices differently. We confirm the exact quote with a trusted human partner in the loop. Warehouse/terminal pickup usually saves about $150–$200. FreightCenter: 800-716-7608 for a same-day check."
      : null,
    "",
    "Your item(s):",
