...
export function buildFreightQuoteEmails(submission, ctx = {}) {
    ...
    : isLocal
      ...
      : nationwideRangeText
        ? [
            ...
            `<p style="margin:0 0 6px;font-size:28px;line-height:1.15;font-weight:800;color:${NAVY};">${escapeHtml(
              nationwideRangeText
            )}</p>`,
            `<p style="margin:4px 0 8px;font-size:14px;line-height:1.45;color:#c62828;font-weight:700;text-align:left;">Our team will review your request and follow up with confirmed rates and the best available options for you.</p>`,
          ].join("")
  ...
  const nationwideExplainHtml = !isLocal
    ? [
        nationwideMiles != null
          ? `<p style="margin:0 0 8px;font-size:14px;color:#444;">About <strong>${nationwideMiles} miles</strong> from our Scottsdale showroom.</p>`
          : "",
        ...
        `<p style="margin:0 0 10px;font-size:14px;line-height:1.55;color:#333;"><strong>Freight savings tip:</strong> Picking up from a nearby freight terminal or warehouse instead of choosing residential delivery can significantly reduce the final shipping cost. Available pickup options will be included with your confirmed rates.</p>`,
        outOfAz
          ? [
              `<p style="margin:0 0 4px;font-size:14px;line-height:1.55;color:#333;"><strong>Out-of-state purchase savings:</strong> When you purchase the item online and it is shipped outside Arizona, no Arizona sales tax is charged on the item purchase at checkout. That is approximately an 8% savings compared with an Arizona purchase.</p>`,
              `<p style="margin:0 0 10px;font-size:12px;line-height:1.45;color:${MUTED};">This applies to the merchandise purchase. Freight and other service charges are separate.</p>`,
            ].join("")
          : "",
      ].join("")
    : "";
  ...
  const nationwideBuyTipHtml = !isLocal && outOfAz
    ? `<p style="margin:8px 0 0;font-size:13px;line-height:1.5;color:#333;">Purchase online to secure the item and receive the out-of-state no-sales-tax benefit on the merchandise purchase.</p>`
    : "";
  const buyItemsHtml =
  ...
  const buyPolicyHtml = !isLocal
    ? [
        ...
        `<p style="margin:0 0 ${buyNowHtml || buyableItems.length ? "12px" : "0"};font-size:14px;line-height:1.55;color:#333;">Questions? Reach Lost &amp; Found at <a href="mailto:${LOST_FOUND_EMAIL}" style="color:${INK};font-weight:700;">${LOST_FOUND_EMAIL}</a> or <a href="tel:${LOST_FOUND_PHONE.replace(
          /\D/g,
          ""
        )}" style="color:${INK};font-weight:700;">${LOST_FOUND_PHONE}</a>.</p>`,
        !buyItemsHtml && buyNowHtml
          ? `${buyNowHtml}${nationwideBuyTipHtml}`
          : "",
        `</div>`,
      ].join("")
    : "";
