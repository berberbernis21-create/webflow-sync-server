...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  const assemblyTimeLabel = formatCustomerAssemblyTimeLabel(assemblyMinsAmt);
  const assemblyBreakdownHtml =
    assemblyFeeAmt > 0 && assemblyMinsAmt > 0
      ? `<p style="margin:0 0 8px;font-size:14px;line-height:1.45;color:#333;text-align:center;"><strong>Additional Disassembly &amp; Reassembly Time | ${assemblyTimeLabel}</strong></p>`
  ...
  const assemblyBreakdownText =
    assemblyFeeAmt > 0 && assemblyMinsAmt > 0
      ? `Additional Disassembly & Reassembly Time | ${assemblyTimeLabel}`
      : isLocal && Boolean(localEstimate?.assembly_required)
