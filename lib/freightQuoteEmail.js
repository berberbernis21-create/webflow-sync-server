        ...
        (() => {
          ...
          if (req === "yes") {
            ...
            if (over === "yes") {
              ...
              return `Disassembly & reassembly: yes · +${mins} min beyond 15`;
            }
            if (over === "idk") return "Disassembly & reassembly: yes · over 15 min: idk";
            return "Disassembly & reassembly: yes (within 15 min)";
          }
          return null;
        })(),
      ]
...
export function buildFreightQuoteEmails(submission, ctx = {}) {
    ...
    isLocal && localEstimate?.estimated_price != null
      ? [
          ...
          Number(localEstimate.stair_fee) > 0
            ...
            : null,
          Number(localEstimate.assembly_fee) > 0
            ? [
                "Additional Disassembly & Reassembly Time",
                `${localEstimate.assembly_extra_minutes || 0} Minutes · ${money(localEstimate.assembly_fee)}`,
              ]
            : localEstimate.assembly_required
              ? ["Disassembly & Reassembly", "Yes — within 15 min (no extra charge)"]
              : null,
  ...
  if (isLocal && Boolean(localEstimate?.assembly_confirm_over_60)) {
    callOutRows.unshift([
      "Confirm disassembly & reassembly time",
      `Over 60 extra minutes — confirm with customer (increases cost). Estimate includes ${
        localEstimate.assembly_extra_minutes || 0
      } Minutes · ${money(localEstimate.assembly_fee || 0)}.`,
    ]);
  } else if (isLocal && Number(localEstimate?.assembly_fee) > 0) {
    callOutRows.push([
      "Additional Disassembly & Reassembly Time",
      `${localEstimate.assembly_extra_minutes || 0} Minutes · ${money(localEstimate.assembly_fee)}`,
    ]);
  } else if (isLocal && Boolean(localEstimate?.assembly_required)) {
    callOutRows.push(["Disassembly & Reassembly", "Required — within 15 min (no extra charge)"]);
  }
  ...
  const assemblyBreakdownHtml =
    assemblyFeeAmt > 0 && assemblyMinsAmt > 0
      ? [
          `<p style="margin:0 0 4px;font-size:14px;line-height:1.45;color:#333;text-align:center;"><strong>Additional Disassembly &amp; Reassembly Time | ${assemblyMinsAmt} Minutes</strong></p>`,
          `<p style="margin:0 0 8px;font-size:14px;line-height:1.45;color:#333;text-align:center;">${assemblyTimeLabel} + ${assemblyMinsAmt} Minutes for Disassembly &amp; Reassembly · +${escapeHtml(
            money(assemblyFeeAmt)
          )} (rounded up)</p>`,
        ].join("")
      : isLocal && Boolean(localEstimate?.assembly_required)
        ? `<p style="margin:0 0 8px;font-size:13px;line-height:1.45;color:#444;text-align:center;">Disassembly &amp; reassembly needed (within 15 minutes — no extra charge).</p>`
        : "";
  const assemblyBreakdownText =
    assemblyFeeAmt > 0 && assemblyMinsAmt > 0
      ? `Additional Disassembly & Reassembly Time | ${assemblyMinsAmt} Minutes\n${assemblyTimeLabel} + ${assemblyMinsAmt} Minutes for Disassembly & Reassembly · +${money(
          assemblyFeeAmt
        )} (rounded up)`
      : isLocal && Boolean(localEstimate?.assembly_required)
        ? "Disassembly & reassembly needed (within 15 minutes — no extra charge)."
        : "";
