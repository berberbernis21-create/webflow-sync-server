  ...
  const driveExplainHtml =
    isLocal && oneWayMinutes != null
      ? `<p style="margin:0 0 10px;font-size:14px;color:#444;"><strong>Drive time to you:</strong> <strong>${oneWayMinutes} min</strong> one way · <strong>${roundTripMinutes} min</strong> round trip${
        ...
        }. Includes a little extra time for a loaded delivery truck (not live traffic). ${includedWorkHtml}</p>${
  ...
  const driveExplainText =
    isLocal && oneWayMinutes != null
      ? `Drive time to you: ${oneWayMinutes} min one way · ${roundTripMinutes} min round trip${
        ...
        }. Includes a little extra time for a loaded delivery truck (not live traffic). ${includedWorkText}${
          Number(localEstimate?.multi_item_adder) > 0
