...
const BRAND = {
  ...
  showroomExterior: `${PUBLIC_API_BASE}/brand/showroom-exterior.png`,
  showroomInterior: `${PUBLIC_API_BASE}/brand/showroom-interior.png`,
  showroomLiving: `${PUBLIC_API_BASE}/brand/showroom-living.png`,
...
function compactTableHtml(rows) {
  ...
  return [
    ...
    ...cleaned.map(([k, v], i) => {
      ...
      return `<tr style="background:${bg};"><td style="padding:6px 8px;color:${MUTED};vertical-align:top;width:42%;border-bottom:1px solid ${BORDER};">${escapeHtml(
        k
      )}</td><td style="padding:6px 8px;color:#222;vertical-align:top;border-bottom:1px solid ${BORDER};font-weight:600;">${escapeHtml(
        String(v)
      )}</td></tr>`;
    }),
    `</table>`,
...
/** Only yes / notable access answers for a short internal email. */
function accessHighlightRows(access = {}, { isPickup = false } = {}) {
  const rows = [];
  rows.push(["Type", access.residential ? "Residential" : access.commercial ? "Commercial" : "-"]);
  if (access.stairs) {
    rows.push([
      "Stairs",
  ...
  return items
    .map((row) => {
      ...
      const bits = [
        ...
        .join(" | ");
      return [
        `<div style="margin:0 0 10px;padding:10px 12px;border:1px solid ${BORDER};border-radius:6px;background:#fff;">`,
        ...
        row.product_url
          ? `<p style="margin:4px 0 0;font-size:12px;"><a href="${escapeHtml(
              row.product_url
            )}" style="color:${NAVY};">View listing</a></p>`
          : "",
        `</div>`,
      ].join("");
    })
...
const SOCIAL = {
  ...
  luxurySite: "https://www.lostandfoundhandbags.com/",
...
function socialLinkButton(href, label, { bg = "#ffffff", color = NAVY, border = BORDER } = {}) {
  return `<a href="${escapeHtml(href)}" style="display:inline-block;margin:0 8px 8px 0;padding:10px 14px;border:1px solid ${border};border-radius:6px;background:${bg};color:${color};font-size:12px;font-weight:800;text-decoration:none;letter-spacing:0.02em;">${escapeHtml(
    label
  )}</a>`;
}
...
function socialFollowHtml({ isPickup }) {
  ...
  return [
    `<div style="margin:22px 0 0;padding:18px;background:${CREAM};border:1px solid ${BORDER};border-radius:10px;">`,
    `<p style="margin:0 0 6px;font-size:11px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;color:${INK};">Follow Lost &amp; Found</p>`,
    `<p style="margin:0 0 14px;font-size:14px;line-height:1.55;color:#333;">${escapeHtml(intro)}</p>`,
    `<p style="margin:0 0 10px;font-size:13px;font-weight:800;color:${INK};">Lost &amp; Found Resale</p>`,
    `<div>${socialLinkButton(SOCIAL.resaleFacebook, "Facebook", { bg: "#ffffff", color: INK, border: "#222" })}</div>`,
    `<p style="margin:16px 0 8px;font-size:13px;font-weight:800;color:${INK};">Also check out our luxury division</p>`,
    `<p style="margin:0 0 10px;font-size:13px;line-height:1.5;color:#444;">Designer handbags and accessories at Lost &amp; Found Handbags.</p>`,
    `<div>`,
    socialLinkButton(SOCIAL.luxurySite, "Shop Luxury", { bg: "#ffffff", color: INK, border: "#222" }),
    socialLinkButton(SOCIAL.luxuryFacebook, "Luxury Facebook", { bg: "#3b5998", color: "#ffffff", border: "#3b5998" }),
    socialLinkButton(SOCIAL.luxuryInstagram, "Luxury Instagram", { bg: "#c13584", color: "#ffffff", border: "#c13584" }),
    `</div>`,
    `</div>`,
  ].join("");
}
...
function socialFollowText({ isPickup }) {
  ...
  return [
    ...
    "Also check out our luxury division:",
    `Shop: ${SOCIAL.luxurySite}`,
...
function showroomStripHtml() {
  return [
    ...
    `<p style="margin:0 0 8px;font-size:11px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;color:${INK};">Visit our Scottsdale showroom</p>`,
    `<table role="presentation" cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse;">`,
    `<tr>`,
    `<td style="width:50%;padding:0 4px 0 0;"><img src="${BRAND.showroomExterior}" alt="Lost + Found showroom exterior" width="300" style="display:block;width:100%;height:auto;border-radius:6px;border:1px solid ${BORDER};" /></td>`,
    `<td style="width:50%;padding:0 0 0 4px;"><img src="${BRAND.showroomInterior}" alt="Lost + Found showroom interior" width="300" style="display:block;width:100%;height:auto;border-radius:6px;border:1px solid ${BORDER};" /></td>`,
    `</tr>`,
    `</table>`,
    `<p style="margin:8px 0 0;font-size:12px;color:${MUTED};">15530 N Greenway Hayden Loop Ste 100, Scottsdale | Mon-Sat 10-5, Sun 12-4</p>`,
    `</div>`,
  ].join("");
}
...
export function buildFreightQuoteEmails(submission, ctx = {}) {
    ...
    isLocal && localEstimate?.estimated_price != null
      ...
      : isLocal
        ...
        : [
            ...
            ["Note", "Confirm with FreightCenter | floor $350"],
          ];

  const requestCompactRows = [
    ["Mode", modeLabel],
    ["Path", pathLabel],
    ["Type", submission.destination_type || "-"],
    ["Address", addr],
  ...
  const accessCompactRows = accessHighlightRows(submission.access, { isPickup });
  ...
  const internalSnapshotHtml = [
    ...
    mapRoute?.map_image_url
      ? `<div style="margin:0 0 14px;"><a href="${escapeHtml(
          ...
          mapRoute.map_image_url
        )}" alt="${isPickup ? "Pickup route" : "Delivery route"}" width="640" style="display:block;width:100%;max-width:640px;height:auto;border:1px solid ${BORDER};border-radius:8px;" /></a></div>`
      : "",
  ].join("");
  ...
  const twoColHtml = [
    ...
    `<p style="margin:0 0 8px;padding-bottom:4px;border-bottom:2px solid ${NAVY};color:${NAVY};font-size:13px;font-weight:800;">Access highlights</p>`,
    compactTableHtml(accessCompactRows),
    `</td>`,
    `</tr>`,
    `</table>`,
  ].join("");
  ...
  const internalHtml = wrapEmail(
    [
      ...
      !isLocal
        ? `<p style="margin:14px 0 0;font-size:11px;color:${MUTED};">Confirm nationwide pricing / Shipment ID with FreightCenter (${FREIGHTCENTER_PHONE}).</p>`
        : "",
    ].join("")
  );
  ...
  const customerHtml = wrapEmail(
    [
      ...
      isLocal
        ? ""
        : `<p style="margin:18px 0 12px;font-size:14px;color:#444;">Final pricing depends on carrier and service level | it can be lower than this range; white-glove can push it higher. We will follow up after partner review.</p>
