...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  const localInternalNextStepsHtml = isLocal
    ? [
        ...
        `<p style="margin:0;font-size:15px;line-height:1.5;font-weight:700;color:${INK};">Confirm with delivery team and let client know.${
          submission.delivery_path === "local_az"
            ? " Print the <strong>Sign Form</strong> PDF (attached) for customer signature &amp; initials on delivery day."
            : ""
        }</p>`,
        `</div>`,
      ].join("")
  ...
  const localInternalNextStepsText = isLocal
    ? [
        ...
        submission.delivery_path === "local_az"
          ...
          : "Confirm with delivery team and let client know.",
        "",
      ].join("\n")
    : "";
...
function applyPdfNotice({ html, text, attached, downloadUrl, filename, signFormAttached = false }) {
  ...
  if (signFormAttached) {
    noticeHtml +=
      " <strong>Sign Form PDF</strong> is also attached — one-page branded delivery estimate for print, with route map, customer signature &amp; initials (not for pickup).";
    noticeText +=
