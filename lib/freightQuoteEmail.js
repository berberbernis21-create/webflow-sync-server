...
function nationwideInternalPlaybookHtml(submission = {}) {
  ...
  return [
    ...
    `<p style="margin:0 0 14px;font-size:14px;line-height:1.55;color:#222;"><strong>Forward this email to our FreightCenter contact</strong> and request the best available carrier options, including the lowest rate, best overall service, and terminal-pickup options. Review the recommendations internally before sending confirmed options to the customer.</p>`,
    palletFitHtml,
...
function nationwideInternalPlaybookText(submission = {}) {
  ...
  return [
    ...
    "Forward this email to our FreightCenter contact and request the best available carrier options, including the lowest rate, best overall service, and terminal-pickup options. Review the recommendations internally before sending confirmed options to the customer.",
    "",
    palletFit,
    "",
    "PACKING DETAILS",
    ...
    .map((row) => {
      ...
      const dimBits =
        row.width != null && row.depth != null && row.height != null
          ...
          : "";
      const titleLine = dimBits ? `${title} · ${dimBits}` : title;
      const meta = [
      ...
      const buyCell = url
        ? `<td valign="middle" align="right" style="padding:10px 12px 10px 8px;white-space:nowrap;"><a href="${escapeHtml(
            url
          )}" style="display:inline-block;padding:10px 14px;background:${NAVY};color:#ffffff;text-decoration:none;font-weight:800;font-size:13px;border-radius:8px;">Buy Now</a></td>`
        : "";
      return [
...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  function localRateConfirmHtml({ strong = false } = {}) {
    ...
    return [
      ...
      `<p style="margin:0 0 16px;font-size:14px;line-height:1.5;color:#333;">This <strong>preliminary estimate</strong> is based on the information you submitted. A PDF of your submitted details is attached for your records. Review the information for accuracy, then contact the delivery partner or Lost &amp; Found Resale to confirm availability and <strong>final pricing</strong>.</p>`,
      ...
      `<a href="tel:+1${cruzTel}" style="text-decoration:none;"><img src="${BRAND.cruzLogo}" alt="${CRUZ_PRO_LINE_NAME}" width="56" height="56" style="display:block;width:56px;height:56px;border-radius:8px;object-fit:cover;border:1px solid #e5e1d8;" /></a>`,
      `</td>`,
      `<td style="vertical-align:middle;padding-left:12px;text-align:left;">`,
      `<p style="margin:0 0 1px;font-size:10px;font-weight:800;letter-spacing:0.06em;text-transform:uppercase;color:#8a6d12;text-align:left;">Delivery partner</p>`,
      `<p style="margin:0;font-size:14px;font-weight:800;color:${NAVY};line-height:1.25;text-align:left;"><a href="tel:+1${cruzTel}" style="color:${NAVY};text-decoration:none;">${CRUZ_PRO_LINE_NAME}</a></p>`,
      ...
      `<table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>`,
      `<td style="vertical-align:middle;width:64px;">`,
      `<a href="tel:+1${shopTel}" style="text-decoration:none;"><img src="${BRAND.storePhoto}" alt="Lost &amp; Found Scottsdale showroom" width="56" height="56" style="display:block;width:56px;height:56px;border-radius:8px;object-fit:cover;border:1px solid #e5e1d8;" /></a>`,
      `</td>`,
      `<td style="vertical-align:middle;padding-left:12px;text-align:left;">`,
      `<p style="margin:0 0 1px;font-size:10px;font-weight:800;letter-spacing:0.06em;text-transform:uppercase;color:#8a6d12;text-align:left;">Call the shop</p>`,
  ...
  const estimateHtml = localPriceDisplay
    ...
    : isLocal
      ...
      : nationwideRangeText
        ? [
            `<p style="margin:0 0 4px;font-size:15px;line-height:1.4;color:#333;">Your preliminary nationwide freight range</p>`,
            `<p style="margin:0 0 8px;font-size:28px;line-height:1.15;font-weight:800;color:${NAVY};">${escapeHtml(
              nationwideRangeText
            )}</p>`,
  ...
  const driveExplainHtml =
    isLocal && oneWayMinutes != null
      ? [
          ...
          `<p style="margin:0 0 8px;font-size:12px;line-height:1.45;color:${MUTED};text-align:center;">Drive time includes additional time for a loaded delivery truck. Distance and time are based on the mapped route, not live traffic.</p>`,
  ...
  const driveExplainText =
    isLocal && oneWayMinutes != null
      ? [
          ...
          "Drive time includes additional time for a loaded delivery truck. Distance and time are based on the mapped route, not live traffic.",
          Number(localEstimate?.multi_item_adder) > 0
  ...
  const confirmOnceHtml = isLocal
    ...
    : `<p style="margin:8px 0 12px;font-size:13px;line-height:1.5;color:#444;text-align:center;">Review the attached PDF of your answers and make sure they are accurate when you call to confirm pricing.</p>`;
  const confirmOnceText = isLocal
    ...
    : "Review the attached PDF of your answers and make sure they are accurate when you call to confirm pricing.";
  ...
  const buyItemsHtml =
    !isPickup && (submission.items || []).length
      ? [
          `<div style="margin:10px 0 8px;">`,
          `<p style="margin:0 0 8px;font-size:13px;font-weight:800;color:${NAVY};">Buy your item(s)</p>`,
      ...
      : isPickup && (submission.items || []).length
        ? [
            `<div style="margin:10px 0 8px;">`,
            `<p style="margin:0 0 8px;font-size:13px;font-weight:800;color:${NAVY};">Your item(s)</p>`,
            customerItemsHtml(submission.items || [], { showBuyLinks: false }),
            `<p style="margin:6px 0 0;font-size:12px;color:${MUTED};">Pickup: ${escapeHtml(addr)}</p>`,
            `</div>`,
          ].join("")
        : "";

  // Nationwide only: short purchase note (no duplicate confirm-pricing / PDF coaching).
  const buyPolicyHtml = !isLocal
    ? [
        ...
        `<p style="margin:0 0 ${buyNowHtml || buyableItems.length ? "12px" : "0"};font-size:14px;line-height:1.55;color:#333;">We do <strong>not hold items</strong>. Purchase now and we can confirm delivery and freight shortly. If you need rates ASAP, please reach out to us at <a href="mailto:${LOST_FOUND_EMAIL}" style="color:${INK};font-weight:700;">${LOST_FOUND_EMAIL}</a>, <a href="tel:${LOST_FOUND_PHONE.replace(
          /\D/g,
          ""
        )}" style="color:${INK};font-weight:700;">${LOST_FOUND_PHONE}</a>, or reply here.</p>`,
        !buyItemsHtml && buyNowHtml ? buyNowHtml : "",
        `</div>`,
      ].join("")
    : "";
  ...
  const customerHtml = wrapEmail(
    [
      `<h2 style="margin:0 0 10px;color:${NAVY};font-size:26px;line-height:1.25;font-weight:800;">${greetingHtml}</h2>`,
      estimateHtml,
