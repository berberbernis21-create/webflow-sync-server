    ...
    isLocal && localEstimate?.estimated_price != null
      ? [
          ...
          [
            "Pricing formula",
            isPickup
              ? "Pickup: $95 through 15 min, then +$15/8 per extra minute, round up to $5"
              : "$95 through 15 min, then +$15/8 per extra minute, round up to $5",
          ],
  ...
  const driveExplainHtml =
    isLocal && oneWayMinutes != null
      ? `<p style="margin:0 0 10px;font-size:14px;color:#444;"><strong>${oneWayMinutes} min</strong> one way · <strong>${roundTripMinutes} min</strong> round trip${
        ...
        } (showroom → ${isPickup ? "pickup" : "you"} → showroom). Includes loading, unloading, and standard labor.</p>`
      : isLocal
        ? `<p style="margin:0 0 10px;font-size:14px;color:#444;">Round-trip from our Scottsdale showroom. Includes loading, unloading, and standard labor.</p>`
  ...
  const driveExplainText =
    isLocal && oneWayMinutes != null
      ? `${oneWayMinutes} min one way · ${roundTripMinutes} min round trip${
        ...
        } (showroom → ${isPickup ? "pickup" : "you"} → showroom). Includes loading, unloading, and standard labor.`
      : isLocal
        ? "Round-trip from our Scottsdale showroom. Includes loading, unloading, and standard labor."
        : `${
            nationwideMiles != null
