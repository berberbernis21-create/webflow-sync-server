  ...
  const estimateRows =
    isLocal && localEstimate?.estimated_price != null
      ? [
          ...
          [
            "Pricing",
            "$95.00/hour round-trip (in and out), rounded up to $5",
          ],
          isPickup ? ["Direction", "Round-trip pickup — showroom out and back with the item"] : null,
        ].filter(Boolean)
      : isLocal
  ...
  const driveExplainHtml =
    isLocal && oneWayMinutes != null
      ? `<p style="margin:0 0 10px;font-size:14px;color:#444;"><strong>${oneWayMinutes} min</strong> one way · <strong>${roundTripMinutes} min</strong> round trip${
        ...
        }. Price is round-trip (in and out), not one way. Includes loading, unloading, and standard labor.</p>`
      : isLocal
        ? `<p style="margin:0 0 10px;font-size:14px;color:#444;">Round-trip from our Scottsdale showroom (in and out, not one way). Includes loading, unloading, and standard labor.</p>`
  ...
  const driveExplainText =
    isLocal && oneWayMinutes != null
      ? `${oneWayMinutes} min one way · ${roundTripMinutes} min round trip${
        ...
        }. Price is round-trip (in and out), not one way. Includes loading, unloading, and standard labor.`
      : isLocal
        ? "Round-trip from our Scottsdale showroom (in and out, not one way). Includes loading, unloading, and standard labor."
        : `${
            nationwideMiles != null
