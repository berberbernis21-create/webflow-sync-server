...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  function localRateConfirmHtml({ strong = false } = {}) {
    ...
    const intro = isPickup
      ? `This <strong>preliminary estimate</strong> is based on the information you submitted. A PDF of your submitted details is attached for your records. Contact <strong>Ivan</strong> directly to confirm availability and <strong>final pricing</strong>, then let the shop know the confirmed pickup date by calling or emailing <a href="mailto:${LOST_FOUND_EMAIL}" style="color:${NAVY};font-weight:700;">${LOST_FOUND_EMAIL}</a>.`
    ...
    const partnerKicker = isPickup ? "Confirm pricing with Ivan" : "Delivery partner";
    const partnerName = isPickup ? "Ivan" : CRUZ_PRO_LINE_NAME;
    const partnerSub = isPickup
      ? `<p style="margin:2px 0 0;font-size:12px;line-height:1.3;color:${MUTED};text-align:left;">${CRUZ_PRO_LINE_NAME}</p>`
      : "";
  ...
  function localRateConfirmText() {
    if (isPickup) {
      return [
        "Next steps",
        `This preliminary estimate is based on the information you submitted. A PDF of your submitted details is attached for your records. Contact Ivan directly to confirm availability and final pricing, then let the shop know the confirmed pickup date by calling or emailing ${LOST_FOUND_EMAIL}.`,
        `Ivan (${CRUZ_PRO_LINE_NAME}): ${CRUZ_PRO_LINE_PHONE}`,
