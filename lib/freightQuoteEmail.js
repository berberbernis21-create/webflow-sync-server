...
/** Only yes / notable access answers for a short internal email. Type lives on Request. */
...
function socialFollowHtml({ isPickup }) {
  ...
  return [
    ...
    `<p style="margin:0 0 10px;font-size:13px;line-height:1.5;color:#444;">Designer handbags and accessories.</p>`,
    `<div>`,
...
export async function sendFreightQuoteEmails(submission, ctx) {
  ...
  let pdfBuffer = null;
  let pdfFilename = buildFreightQuotePdfFilename(submission, ctx);
  try {
    pdfBuffer = await generateFreightQuotePdf(submission, ctx);
