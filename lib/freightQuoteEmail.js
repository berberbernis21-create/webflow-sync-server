...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  if (preferredDaysWindows) {
    callOutRows.push(["Preferred days / windows", preferredDaysWindows]);
  }
  ...
  function buildBuyItemsBlock({ includeTip = false, includeAddress = false, margin = "0 0 16px" } = {}) {
    ...
    const preferredScheduleHtml =
      includeAddress && preferredSchedule
        ? `<p style="margin:6px 0 0;font-size:12px;color:${MUTED};"><strong style="color:${INK};">Preferred days / windows:</strong> ${escapeHtml(
            preferredSchedule
          )}</p>`
        : "";
    if (isPickup) {
