...
export async function sendFreightQuoteEmails(submission, ctx) {
  ...
  if (isLocalDelivery) {
    const t0 = Date.now();
    // Fast path: build + attach sign PDF from the submitted payload (no listing re-lookup).
    const [signPdfBuffer, builtQuick] = await Promise.all([
      safeGenerateLocalSignPdf(submission, ctx),
      Promise.resolve(buildFreightQuoteEmails(submission, ctx)),
    ]);
    ...
    const internal = await sendInternalFreightEmail({
      built: builtQuick,
      pdfBuffer: null,
      pdfFilename: buildFreightQuotePdfFilename(submission, ctx),
      signPdfBuffer,
      signPdfFilename,
      ctx,
    });
