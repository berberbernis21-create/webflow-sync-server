...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  const nationwideNextStepsHtml = isLocal
    ...
    : [
        ...
        } (your choice). <strong>Pickup loading:</strong> Lost &amp; Found will determine if a liftgate is needed when we send your final quote — <strong>for now we assume yes</strong>. Hours: <strong>10am–5pm, Monday–Saturday</strong>.</p>`,
        `</div>`,
      ].join("");
  ...
  const nationwideNextStepsText = isLocal
    ...
    : [
        ...
        `Liftgate at delivery: ${
          submission.access?.liftgate_delivery ? "Yes" : "No"
        } (your choice). Pickup loading: Lost & Found will determine if a liftgate is needed when we send your final quote — for now we assume yes. Hours: 10am–5pm, Monday–Saturday.`,
      ]
        .filter(Boolean)
        .join("\n");
