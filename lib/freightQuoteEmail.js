...
export function buildFreightQuoteEmails(submission, ctx = {}) {
      ...
      ? [
          ...
          Number(localEstimate.assembly_fee) > 0
            ...
            : localEstimate.assembly_required
              ? ["Disassembly & Reassembly", "Yes — within free 15 min with wrapping/prep (no extra charge)"]
              : null,
  ...
  if (isLocal && Boolean(localEstimate?.assembly_confirm_over_60)) {
    ...
    callOutRows.push(["Disassembly & Reassembly", "Required — within free 15 min with wrapping/prep (no extra charge)"]);
  }
  ...
  const assemblyBreakdownHtml =
    assemblyFeeAmt > 0 && assemblyMinsAmt > 0
      ? [
          ...
          `<p style="margin:0 0 8px;font-size:14px;line-height:1.45;color:#333;text-align:center;">${assemblyTimeLabel} + ${assemblyMinsAmt} Minutes for Disassembly &amp; Reassembly</p>`,
        ].join("")
      : isLocal && Boolean(localEstimate?.assembly_required)
        ? `<p style="margin:0 0 8px;font-size:13px;line-height:1.45;color:#444;text-align:center;">Disassembly &amp; reassembly needed (within free 15 minutes with wrapping/prep — no extra charge).</p>`
        : "";
  const assemblyBreakdownText =
    assemblyFeeAmt > 0 && assemblyMinsAmt > 0
      ? `Additional Disassembly & Reassembly Time | ${assemblyMinsAmt} Minutes\n${assemblyTimeLabel} + ${assemblyMinsAmt} Minutes for Disassembly & Reassembly`
      : isLocal && Boolean(localEstimate?.assembly_required)
        ? "Disassembly & reassembly needed (within free 15 minutes with wrapping/prep — no extra charge)."
        : "";
