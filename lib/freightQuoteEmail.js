...
function nationwideInternalPlaybookHtml(submission = {}) {
  ...
  return [
    ...
    `<p style="margin:0 0 8px;font-size:13px;font-weight:800;color:${NAVY};">Nationwide — FreightCenter action</p>`,
    `<ol style="margin:0 0 12px;padding-left:18px;font-size:13px;line-height:1.55;color:#222;">`,
    `<li style="margin:0 0 6px;">Open the SOP for FreightCenter login + quoting steps: <strong>${escapeHtml(
      FREIGHTCENTER_SOP_PATH
    )}</strong> (login info is in that PDF).</li>`,
    `<li style="margin:0 0 6px;">Go to <a href="https://www.freightcenter.com" style="color:${NAVY};font-weight:700;">freightcenter.com</a>, pull rates using the freight-ready <strong>48×40</strong> (or oversized) pallet entries below.</li>`,
    `<li style="margin:0 0 6px;">Email <a href="mailto:${FREIGHTCENTER_LUKE_EMAIL}" style="color:${NAVY};font-weight:700;">${FREIGHTCENTER_LUKE_EMAIL}</a> (Luke Rogers) with this packing summary, how it will be stacked, and what the customer selected. You can forward / paste this block. FreightCenter phone: ${escapeHtml(
      FREIGHTCENTER_PHONE
    )}.</li>`,
    `</ol>`,
    `<p style="margin:0 0 6px;font-size:12px;font-weight:800;color:${INK};">Packing / stacking for Luke</p>`,
    ...
    `<p style="margin:0 0 6px;font-size:12px;font-weight:800;color:${INK};">From Luke (FreightCenter) — quoting notes</p>`,
    `<p style="margin:0 0 8px;font-size:12px;line-height:1.55;color:#333;">Carriers that generally <strong>do not charge an extra fee</strong> to mark freight non-stackable (subject to height limits): <strong>Estes</strong>; <strong>AAA Cooper (ACT)</strong> under 52&quot;; <strong>XPO</strong> under 70&quot;; <strong>ABF</strong>; <strong>Oak Harbor</strong>; <strong>CCFS</strong> under 60&quot;; <strong>FedEx Freight</strong>; <strong>R+L</strong> under 70&quot;. If a carrier is <em>not</em> on this list and Non-Stackable is not selected, they will likely stack other freight on top when space allows.</p>`,
    `<p style="margin:0;font-size:12px;line-height:1.55;color:#333;">Carrier quality: cheaper carriers often mean slower communication (email 24–48 hrs vs a live person at a good terminal in under ~10 minutes), more billing adjustments on small weight differences, and less attention to detail. Luke’s guidance: ~95% of the time you get what you pay for — prefer a higher-quality carrier when service issues would hurt.</p>`,
    `</div>`,
  ].join("");
}
...
function nationwideInternalPlaybookText(submission = {}) {
  ...
  return [
    "NATIONWIDE — FREIGHTCENTER ACTION",
    `1) SOP + login: ${FREIGHTCENTER_SOP_PATH}`,
    "2) Quote at https://www.freightcenter.com using the 48x40 (or oversized) pallet entries.",
    `3) Email Luke Rogers <${FREIGHTCENTER_LUKE_EMAIL}> with packing/stacking + customer access. Phone: ${FREIGHTCENTER_PHONE}.`,
    "",
    "PACKING / STACKING FOR LUKE",
    packing || "(no items)",
    "",
    "CUSTOMER SAID",
    [
    ...
    "",
    "FROM LUKE — NON-STACKABLE (no extra fee, height limits): Estes; AAA Cooper under 52\"; XPO under 70\"; ABF; Oak Harbor; CCFS under 60\"; FedEx Freight; R+L under 70\". Other carriers will likely stack on top if Non-Stackable is not selected.",
    "FROM LUKE — CARRIER QUALITY: cheaper = slower communication + more billing adjustments; ~95% of the time you get what you pay for.",
  ].join("\n");
}
    ...
    isLocal && localEstimate?.estimated_price != null
      ...
      : isLocal
        ...
        : [
            ...
            [
              "Note",
              `Get quote on freightcenter.com · email Luke ${FREIGHTCENTER_LUKE_EMAIL} · SOP login in Dropbox · floor $350`,
            ],
          ];
