...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  const assemblyBreakdownHtml =
    assemblyFeeAmt > 0 && assemblyLines
      ? [
          `<p style="margin:0 0 4px;font-size:14px;line-height:1.45;color:#333;text-align:center;"><strong>Additional Disassembly &amp; Reassembly Time | ${assemblyLines.extraLabel}</strong></p>`,
          `<p style="margin:0 0 8px;font-size:14px;line-height:1.45;color:${NAVY};text-align:center;"><strong>Total Disassembly &amp; Reassembly Time | ${assemblyLines.totalLabel}</strong></p>`,
        ].join("")
  ...
  const assemblyBreakdownText =
    assemblyFeeAmt > 0 && assemblyLines
      ? `Additional Disassembly & Reassembly Time | ${assemblyLines.extraLabel}\nTotal Disassembly & Reassembly Time | ${assemblyLines.totalLabel}`
      : isLocal && Boolean(localEstimate?.assembly_required)
