      ...
      return [
        ...
        p
          ? `<br/>Pallet entry: ${escapeHtml(dims(p.width, p.depth, p.height))} · ${escapeHtml(
              p.weight
            )} lb · Class ${escapeHtml(p.freight_class)} · Non-stackable: ${yn(p.non_stackable)}`
...
function itemsText(items = []) {
  ...
  for (const row of items) {
    ...
    lines.push(
      ...
      p
        ? `Pallet: ${dims(p.width, p.depth, p.height)} · ${p.weight} lb · Class ${p.freight_class} · Non-stackable: ${yn(p.non_stackable)}`
        : "Pallet: incomplete",
