...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  const estimateAmountLabel = extraCrew
    ? "Your approximate two-person base"
    : isPickup
  ...
  const estimateHtml = localPriceDisplay
    ? [
        ...
        extraCrew
          ? `<p style="margin:0 0 10px;font-size:14px;line-height:1.55;color:#444;">Amount above is the standard two-person base only — we will confirm if more crew is needed.</p>`
          : "",
      ].join("")
    : isLocal
  ...
  const customerText = [
    ...
    localPriceDisplay
      ? `${estimateAmountLabel}\n${localPriceDisplay}${
          extraCrew
            ? "\nAmount above is the standard two-person base only — we will confirm if more crew is needed."
            : ""
        }`
      : isLocal
