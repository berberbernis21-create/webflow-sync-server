...
function socialLinkButton(href, label, { bg = NAVY, color = "#ffffff" } = {}) {
  return `<a href="${escapeHtml(href)}" style="display:inline-block;margin:0 8px 8px 0;padding:10px 14px;border-radius:6px;background:${bg};color:${color};font-size:12px;font-weight:800;text-decoration:none;letter-spacing:0.02em;">${escapeHtml(
    label
  )}</a>`;
}
...
function socialFollowHtml({ isPickup }) {
  ...
  return [
    `<div style="margin:22px 0 0;padding:18px 18px 14px;background:#07127c;border-radius:10px;color:#ffffff;">`,
    `<p style="margin:0 0 6px;font-size:11px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;color:#c9d0ff;">Follow Lost &amp; Found</p>`,
    `<p style="margin:0 0 14px;font-size:14px;line-height:1.55;color:#e8ebff;">${escapeHtml(intro)}</p>`,
    `<p style="margin:0 0 10px;font-size:13px;font-weight:800;color:#ffffff;">Lost &amp; Found Resale</p>`,
    `<div>${socialLinkButton(SOCIAL.resaleFacebook, "Facebook")}</div>`,
    `<p style="margin:16px 0 8px;font-size:13px;font-weight:800;color:#ffffff;">Also check out our luxury division</p>`,
    `<p style="margin:0 0 10px;font-size:13px;line-height:1.5;color:#e8ebff;">Designer handbags and accessories at Lost &amp; Found Handbags.</p>`,
    `<div>`,
    socialLinkButton(SOCIAL.luxurySite, "Shop Luxury", { bg: "#ffffff", color: NAVY }),
    socialLinkButton(SOCIAL.luxuryFacebook, "Luxury Facebook", { bg: "#3b5998" }),
    socialLinkButton(SOCIAL.luxuryInstagram, "Luxury Instagram", { bg: "#c13584" }),
    `</div>`,
    `</div>`,
  ].join("");
}
