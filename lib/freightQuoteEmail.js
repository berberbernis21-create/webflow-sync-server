...
export function buildFreightQuoteEmails(submission, ctx = {}) {
    ...
    isLocal && localEstimate?.estimated_price != null
      ? [
          ...
          [
            "Pricing formula",
            isPickup
              ? "Pickup: $95 through 17 min, then +$15/8 per extra minute, round up to $5"
              : "$95 through 17 min, then +$15/8 per extra minute, round up to $5",
          ],
          isPickup ? ["Direction", "Scottsdale showroom → consignor address → showroom (round trip)"] : null,
        ].filter(Boolean)
      : isLocal
  ...
  const driveExplainHtml =
    isLocal && oneWayMinutes != null
      ? `<p style="margin:0 0 14px;font-size:14px;color:#444;">This estimate considers <strong>${oneWayMinutes} minutes one way (${roundTripMinutes} minutes round trip)</strong>${
          ...
          isPickup
            ? "from our Scottsdale showroom to your pickup location and back, loading the item at your location, securing it for transport, bringing it back to our showroom, unloading, and the expected time and labor."
      ...
      : isLocal
        ? `<p style="margin:0 0 14px;font-size:14px;color:#444;">This estimate considers ${
            isPickup
              ? "the drive from our Scottsdale showroom to your pickup location and back, loading the item at your location, securing it for transport, bringing it back to our showroom, unloading, and the expected time and labor."
              : "the drive from our Scottsdale showroom to your location and back, wrapping and securing for transport, loading the item, unloading, and the expected time and labor."
          }</p>`
  ...
  const driveExplainText =
    isLocal && oneWayMinutes != null
      ...
      : isLocal
        ? isPickup
          ? "This estimate considers the drive from our Scottsdale showroom to your pickup location and back, loading the item at your location, securing it for transport, bringing it back to our showroom, unloading, and the expected time and labor."
  ...
  const buyPolicyHtml = isPickup
    ? [
        ...
        `<p style="margin:0;font-size:14px;line-height:1.6;color:#333;">This is a preliminary consignor pickup estimate. Our team will review access details and confirm scheduling and final pricing. Reply to this email or contact us at <a href="mailto:${LOST_FOUND_EMAIL}" style="color:${NAVY};font-weight:700;">${LOST_FOUND_EMAIL}</a> / <a href="tel:${LOST_FOUND_PHONE.replace(
          /\D/g,
          ""
  ...
  const buyPolicyText = isPickup
    ? [
        ...
        "This is a preliminary consignor pickup estimate. Our team will review access details and confirm scheduling and final pricing. Reply to this email or contact info@lostandfoundresale.com / 480-588-7006.",
      ].join("\n")
    : [
        "Ready to purchase?",
  ...
  const customerHtml = wrapEmail(
    [
      ...
      isLocal
        ? ""
        : `<p style="margin:0 0 12px;font-size:14px;color:#444;"><strong>This is an estimate only.</strong> Final pricing depends on the carrier and service level — it can be less than this range, and white-glove service can push it higher. We will follow up with better rates after we review your request and confirm an exact quote with our freight partners.</p>
           <p style="margin:0 0 12px;font-size:14px;color:#444;"><strong>FreightCenter</strong> can also be reached at <a href="tel:${FREIGHTCENTER_PHONE.replace(/\D/g, "")}" style="color:${NAVY};font-weight:700;">${FREIGHTCENTER_PHONE}</a> — you can get a quote from them immediately.</p>`,
