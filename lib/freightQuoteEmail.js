      ...
      : isLocal
        ...
        : (() => {
            ...
            if (hasOversizedPalletItems(submission.items || [])) {
              confirmBits.push("oversized item / pallet fit");
            } else if (hasSmallerThanStandardPalletItems(submission.items || [])) {
              confirmBits.push("smaller pallet option");
            }
            if (submission.access?.liftgate_delivery) confirmBits.push("liftgate at delivery");
            confirmBits.push("liftgate at pickup (assume yes until confirmed)");
            if (submission.access?.white_glove) confirmBits.push("White Glove");
            if (submission.access?.warehouse_pickup) confirmBits.push("warehouse pickup");
            const confirmLine = confirmBits.length
              ? ` Confirm if needed: ${confirmBits.join(", ")}.`
              : "";
            return [
              [
                "Range",
              ...
              [
                "Note",
                `Please refer to next steps below.${confirmLine}`,
              ],
            ];
          })();
