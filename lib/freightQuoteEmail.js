...
export function buildFreightQuoteEmails(submission, ctx = {}) {
    ...
    ? [
        ...
        `<p style="margin:0;font-size:15px;line-height:1.5;font-weight:700;color:${INK};">${
          isPickup
            ...
            : `Confirm with delivery team and let client know. Print the <strong>delivery document</strong> PDF (attached) for the customer to sign and initial on delivery day.`
        }</p>`,
        `</div>`,
      ].join("")
    : "";
  ...
  const localInternalNextStepsText = isLocal
    ? [
        ...
        isPickup
          ? "Be on the lookout for final confirmation of when the item(s) are coming in."
          : "Confirm with delivery team and let client know. Print the delivery document PDF (attached) for the customer to sign and initial on delivery day.",
        "",
      ]
        .filter(Boolean)
        .join("\n")
...
function applyPdfNotice({ html, text, attached, downloadUrl, filename, signFormAttached = false }) {
  ...
  if (signFormAttached && !attached && !downloadUrl) {
    ...
    noticeText =
      "Delivery document PDF attached — print for the customer to sign and initial on delivery day (includes route map).";
  ...
  if (signFormAttached && (attached || downloadUrl)) {
    ...
    noticeText +=
      "\nDelivery document PDF also attached — print for the customer to sign and initial on delivery day (includes route map).";
  }
