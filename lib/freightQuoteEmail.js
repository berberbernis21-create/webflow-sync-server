...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  const confirmDueSizeHtml = oversizeConfirm
    ? `<p style="margin:0 0 10px;padding:12px 14px;background:#fff1f0;border-left:4px solid #c0392b;font-size:14px;line-height:1.55;color:#333;"><strong>Needs confirmation due to item size.</strong> At least one item is 299+ lb and over 72&quot; high, or over 550 lb. Confirm pricing before scheduling. <strong>This is an estimate only.</strong></p>`
    : "";
  ...
  const customerText = [
    ...
    localPriceDisplay
      ? `${estimateAmountLabel}\nPreliminary Estimate: ${localPriceDisplay}${
          oversizeConfirm
            ? "\nNeeds confirmation due to item size (299+ lb and over 72\" H, or over 550 lb). Confirm pricing before scheduling. ESTIMATE ONLY."
            : ""
        }${
          longHaulConfirm
