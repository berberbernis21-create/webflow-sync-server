...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  const confirmDueSizeHtml = oversizeConfirm
    ? `<p style="margin:0 0 10px;padding:12px 14px;background:#fff1f0;border-left:4px solid #c0392b;font-size:14px;line-height:1.55;color:#333;"><strong>Needs confirmation due to item size.</strong> At least one item is 299+ lb and over 72&quot; high and 72&quot; wide. Some large items are easier to move than others — this was our best guess based on the dimensions and weight you entered (priced like a 3-person / $130/hr job). Please contact us before scheduling. <strong>This is an estimate only.</strong></p>`
    : "";

  const confirmDueDistanceHtml = longHaulConfirm
    ? `<p style="margin:0 0 10px;padding:12px 14px;background:#fff1f0;border-left:4px solid #c0392b;font-size:14px;line-height:1.55;color:#333;"><strong>Needs confirmation — out of town (100+ mile round trip).</strong> Rates can fluctuate with distance, traffic, and scheduling. This was our best guess based on the route information provided. Please contact us to confirm before scheduling. <strong>This is an estimate only.</strong></p>`
    : "";
  ...
  const estimateHtml = localPriceDisplay
    ? [
        ...
        Number(localEstimate?.stair_fee) > 0
          ? `<p style="margin:0 0 8px;font-size:13px;color:#444;">Includes stair fee of ${escapeHtml(
              money(localEstimate.stair_fee)
            )} (first flight free, then $75 per flight).</p>`
          : "",
        confirmDueSizeHtml,
        confirmDueDistanceHtml,
        estimateOnlySpotlight
          ? `<p style="margin:0 0 10px;padding:12px 14px;background:#fff8e8;border-left:4px solid #c9a227;font-size:15px;line-height:1.55;color:#333;"><strong>ESTIMATE ONLY — please confirm.</strong> Extra crew, stairs, oversized pieces, and longer distances can change the final price. Call the shop or Cruz Pro Line before you rely on this number.</p>`
      ...
      ? `<p style="margin:0 0 10px;font-size:14px;color:#444;"><strong>${oneWayMinutes} min</strong> one way · <strong>${roundTripMinutes} min</strong> round trip${
          ...
          Number(localEstimate?.multi_item_adder) > 0
            ? `<p style="margin:0 0 10px;font-size:14px;color:#444;">Includes multi-item handling for <strong>${escapeHtml(
              ...
              )}</strong> items (+${escapeHtml(money(localEstimate.multi_item_adder))}).</p>`
            : ""
        }${
          localEstimate?.rate_label
            ? `<p style="margin:0 0 10px;font-size:13px;color:#555;">Rate used: ${escapeHtml(
                localEstimate.rate_label
              )}.</p>`
            : ""
        }`
  ...
  const driveExplainText =
    isLocal && oneWayMinutes != null
      ? `${oneWayMinutes} min one way · ${roundTripMinutes} min round trip${
          ...
          Number(localEstimate?.multi_item_adder) > 0
            ? `\nIncludes multi-item handling for ${localEstimate.item_count} items (+${money(
                localEstimate.multi_item_adder
              )}).`
            : ""
        }${localEstimate?.rate_label ? `\nRate used: ${localEstimate.rate_label}.` : ""}`
      : isLocal
        ? includedWorkText
        : `${
            nationwideMiles != null
  ...
  const stackConfirmItems = (submission.items || []).filter(
    (it) =>
  ...
  const customerText = [
    ...
    localPriceDisplay
      ? `${estimateAmountLabel}\n${localPriceDisplay} (Estimate)${
          oversizeConfirm
            ? "\nNeeds confirmation due to item size (299+ lb and over 72\" H × 72\" W). Some large items are easier to move than others — best guess priced as 3-person / $130/hr. ESTIMATE ONLY."
            : ""
        }${
          longHaulConfirm
            ? "\nNeeds confirmation — out of town (100+ mile round trip). Rates can fluctuate with distance. ESTIMATE ONLY — please contact us to confirm."
            : ""
        }${
          estimateOnlySpotlight
            ? "\nESTIMATE ONLY — please confirm with the shop or Cruz Pro Line before relying on this number."
            : ""
        }`
      : isLocal
