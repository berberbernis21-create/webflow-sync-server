...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  const confirmDueSizeHtml = oversizeConfirm
    ? `<p style="margin:0 0 10px;padding:12px 14px;background:#fff8e8;border-left:4px solid #c9a227;font-size:14px;line-height:1.55;color:#333;"><strong>Size-based 3-person estimate.</strong> This quote assumes three people may be needed because at least one item is 299+ lb and over 72&quot; high, or 450+ lb — even if you did not select extra crew. Final rates are often lower if the delivery team confirms two people are enough (typically closer to our $95/hour rate). <strong>Please confirm with the delivery team before scheduling. This is an estimate only.</strong></p>`
    : "";
  ...
  const customerText = [
    ...
    localPriceDisplay
      ? `${estimateAmountLabel}\nPreliminary Estimate: ${localPriceDisplay}${
          ...
          oversizeConfirm
            ? "\nSize-based 3-person estimate. This quote assumes three people may be needed due to item size/weight (299+ lb and over 72\" H, or 450+ lb) — even if you did not select extra crew. Final rates are often lower if two people are enough (typically closer to $95/hour). Confirm with the delivery team before scheduling. ESTIMATE ONLY."
            : ""
        }${
          longHaulConfirm
