...
import {
  ...
  formatCustomerAssemblyTimeLabel,
  customerAssemblyTimeLines,
...
import {
  ...
  customerCtaLabel,
  listingLooksSold,
  stripNoLongerAvailableSuffix,
  titleLooksLikeProductUrl,
  humanizeProductUrlTitle,
} from "./listingTitleDisplay.js";

function itemDisplayTitle(row, fallback = "Item") {
  return customerFacingTitle(row, fallback);
}

function itemCleanTitle(row, fallback = "Item") {
  const raw = String(row?.title || "").trim();
  if (titleLooksLikeProductUrl(raw)) {
    return humanizeProductUrlTitle(raw, fallback);
  }
  const cleaned = stripNoLongerAvailableSuffix(raw || fallback) || fallback;
  return titleLooksLikeProductUrl(cleaned)
    ? humanizeProductUrlTitle(row?.product_url || cleaned, fallback)
    : cleaned;
}
...
function internalItemsHtml(items = [], { showPrice = true } = {}) {
  return items
    .map((row) => {
      const p = row.pallet;
      const title = escapeHtml(itemDisplayTitle(row, `Item ${row.index}`));
      const bits = [
...
function buyNowCtaHtml(items = []) {
  const rows = (items || []).filter((row) => customerShopUrl(row) || String(row.product_url || "").trim());
  if (!rows.length) return "";
  if (rows.length === 1) {
    const row = rows[0];
    const url = customerShopUrl(row) || SHOP_ALL_URL;
    return `<a href="${escapeHtml(url)}" style="display:inline-block;padding:10px 16px;background:${NAVY};color:#ffffff;text-decoration:none;font-weight:800;font-size:13px;letter-spacing:0.02em;border-radius:8px;white-space:nowrap;">${escapeHtml(
      customerCtaLabel(row)
    )}</a>`;
  }
  return rows
    .map((row, i) => {
      const url = customerShopUrl(row) || shopSearchUrl(itemDisplayTitle(row, ""));
      return `<a href="${escapeHtml(url)}" style="display:inline-block;margin:${i ? "6px" : "0"} 0 0;padding:10px 14px;background:${NAVY};color:#ffffff;text-decoration:none;font-weight:800;font-size:13px;border-radius:8px;white-space:nowrap;">${escapeHtml(
        customerCtaLabel(row)
      )}${rows.length > 1 ? ` · ${i + 1}` : ""}</a>`;
    })
    .join("<br/>");
...
function customerItemsHtml(items = [], { showBuyLinks = true } = {}) {
  return (items || [])
    .map((row) => {
      const clean = itemCleanTitle(row, `Item ${row.index}`);
      const dimBits =
        row.width != null && row.depth != null && row.height != null
          ? `${escapeHtml(row.width)}X${escapeHtml(row.depth)}X${escapeHtml(row.height)}H`
          : "";
      // Title on its own line; dims + weight below (strip trailing dims from title so they are not repeated).
      const titleOnly = dimBits
        ? clean
            .replace(
              /\s*[-–—]?\s*\d+(?:\.\d+)?\s*[xX×]\s*\d+(?:\.\d+)?(?:\s*[xX×]\s*\d+(?:\.\d+)?)?\s*H?\s*$/i,
              ""
            )
            .replace(/\s+/g, " ")
            .trim() || clean
        : clean;
      const cleanTitle = escapeHtml(titleOnly);
      const meta = [
        dimBits || null,
        ...
        (() => {
          ...
          if (!req) return null;
          if (req === "idk") return "Disassembly & reassembly: Not sure";
          ...
          if (req === "yes") {
            ...
            if (over === "yes") {
              ...
              return mins > 0
                ? `Disassembly & reassembly: ${formatCustomerAssemblyTimeLabel(mins).toLowerCase()}`
                : "Disassembly & reassembly: yes";
            }
            if (over === "idk") return "Disassembly & reassembly: Not sure";
            return "Disassembly & reassembly: within 15 minutes";
          }
          return null;
        })(),
      ]
        .filter(Boolean)
        .join(" · ");
      const sold = listingLooksSold(row);
      const shopUrl = customerShopUrl(row);
      const url = showBuyLinks ? shopUrl || shopSearchUrl(clean) : "";
      const ctaLabel = customerCtaLabel(row);
      const img = row.image_url ? String(row.image_url) : "";
      const imgBlock = img
        ? url
          ? `<a href="${escapeHtml(url)}" style="display:block;line-height:0;text-decoration:none;"><img src="${escapeHtml(
              img
            )}" alt="${cleanTitle}" width="72" height="72" style="display:block;width:72px;height:72px;object-fit:cover;border-radius:8px;border:1px solid ${BORDER};background:#fff;" /></a>`
          : `<img src="${escapeHtml(
              img
            )}" alt="${cleanTitle}" width="72" height="72" style="display:block;width:72px;height:72px;object-fit:cover;border-radius:8px;border:1px solid ${BORDER};background:#fff;" />`
        : "";
      const soldBlock = sold
        ? `<div style="margin:4px 0 0;color:#9c2f2f;font-size:12px;font-weight:800;letter-spacing:0.04em;text-align:left;">(SOLD)</div><div style="margin:2px 0 0;text-align:left;"><a href="${escapeHtml(
            SHOP_ALL_URL
          )}" style="color:${NAVY};font-size:12px;font-weight:800;text-decoration:underline;">Shop All</a></div>`
        : "";
      const buyCell = url
        ? `<td valign="middle" align="right" style="padding:10px 12px 10px 8px;white-space:nowrap;"><a href="${escapeHtml(
            url
          )}" style="display:inline-block;padding:10px 14px;background:${NAVY};color:#ffffff;text-decoration:none;font-weight:800;font-size:13px;border-radius:8px;">${escapeHtml(
            ctaLabel
          )}</a></td>`
        : "";
      return [
        ...
        `<div style="margin:0 0 2px;text-align:left;"><strong style="color:${INK};font-size:14px;line-height:1.3;">${cleanTitle}</strong></div>`,
        meta
...
function customerItemsText(items = [], { showBuyLinks = true } = {}) {
  return (items || [])
    .map((row) => {
      ...
      const dimBits =
        row.width != null && row.depth != null && row.height != null
          ? `${row.width}X${row.depth}X${row.height}H`
          : "";
      const titleOnly = dimBits
        ? clean
            .replace(
      ...
      const bits = [
        ...
        .join(" ");
      const shopUrl =
        customerShopUrl(row) || shopSearchUrl(itemDisplayTitle(row, "").replace(/\s*\(SOLD\)\s*$/i, "").trim());
      return showBuyLinks
        ? `${bits}\n  ${customerCtaLabel(row)}: ${shopUrl}`
        : bits;
    })
    .join("\n");
}
...
/**
 * Optionally fill missing product_url / image_url / real title from listing lookup.
 * Never invent shop links for consignor pickups or fuzzy title matches
 * (e.g. "Bed" matching a random live listing).
 * When the title field is a pasted product URL, always prefer the listing name.
 */
async function enrichSubmissionItemMedia(submission) {
  ...
  const enriched = await Promise.all(
    items.map(async (item) => {
      ...
      const urlRaw = String(item?.product_url || item?.productUrl || "").trim();
      const titleIsUrl = titleLooksLikeProductUrl(titleRaw);
      const lookupKey = titleIsUrl ? titleRaw : urlRaw || titleRaw;
      if (!lookupKey) return item;

      const source = String(item?.source || "").toLowerCase();
      // Manual entries without a URL should stay manual — do not invent a listing.
      if (source === "manual" && !urlRaw && !titleIsUrl) return item;

      // Already has a real title + media — nothing to do.
      if (
        !titleIsUrl &&
        item?.image_url &&
        item?.product_url &&
        !titleLooksLikeProductUrl(titleRaw)
      ) {
        return item;
      }
      ...
      try {
        const res = await fetch(
          `${PUBLIC_API_BASE}/api/listing?name=${encodeURIComponent(lookupKey)}`,
          { headers: { Accept: "application/json" } }
        );
        if (!res.ok) {
          // Display fallback only: keep URL in product_url, humanize title for emails.
          if (titleIsUrl) {
            return {
              ...item,
              title: humanizeProductUrlTitle(titleRaw, titleRaw),
              product_url: urlRaw || titleRaw,
            };
          }
          return item;
        }
        ...
        const listingTitle = stripNoLongerAvailableSuffix(
          String(listing.title || data.title || "").trim()
        );
        const titlesMatch =
          normalizeListingTitle(titleRaw) &&
          normalizeListingTitle(titleRaw) === normalizeListingTitle(listingTitle);
        // URL / slug lookups are authoritative — always take the listing name.
        const acceptListing = titleIsUrl || Boolean(urlRaw) || titlesMatch;
        if (!acceptListing || !listingTitle) {
          if (titleIsUrl) {
            return {
              ...item,
              title: humanizeProductUrlTitle(titleRaw, titleRaw),
        ...
        return {
          ...item,
          title: listingTitle || (titleIsUrl ? humanizeProductUrlTitle(titleRaw) : item.title),
          image_url: image_url || item.image_url || "",
          product_url: product_url || item.product_url || "",
          sold:
            item.sold === true ||
            listing.sold === true ||
            listingLooksSold({ title: listingTitle, sold: listing.sold }),
        };
      } catch {
        if (titleIsUrl) {
          return {
...
function itemsText(items = []) {
  ...
  for (const row of items) {
    ...
    lines.push(
      "",
      `--- Item ${row.index}: ${itemDisplayTitle(row)} ---`,
...
export function buildFreightQuoteEmails(submission, ctx = {}) {
    ...
    isLocal && localEstimate?.estimated_price != null
      ? [
          ...
          Number(localEstimate.stair_fee) > 0
            ...
            : null,
          ...(Number(localEstimate.assembly_extra_minutes) > 0
            ? (() => {
                const lines = customerAssemblyTimeLines(localEstimate.assembly_extra_minutes);
                return lines
                  ? [
                      [lines.extraTitle, lines.extraLabel],
                      [lines.totalTitle, lines.totalLabel],
                    ]
                  : [];
              })()
            : localEstimate.assembly_required
              ? [["Disassembly & Reassembly", "Yes — within 15 min with wrapping/prep (no extra charge)"]]
              : []),
          localEstimate.assembly_confirm_over_60
            ? [
                "Confirm disassembly & reassembly time",
                "Over 60 extra minutes — confirm with customer (increases cost)",
              ]
            : null,
        ].filter(Boolean)
      : isLocal
  ...
  if (isLocal && Boolean(localEstimate?.oversize_confirm)) {
    ...
    callOutRows.unshift([
      "Confirm crew size",
      n >= 2
        ? "Confirm number of people — customer selected 2 extra people (4 total)"
        : "Confirm number of people required — customer selected 1 extra person (3 total)",
    ]);
  }
  const preferredDaysWindows = String(submission.access?.preferred_days_windows || "").trim();
  if (preferredDaysWindows) {
    // First access note for the team when the customer provided windows.
    callOutRows.unshift(["Preferred days / windows", preferredDaysWindows]);
  }
  if (isLocal && Boolean(localEstimate?.assembly_confirm_over_60)) {
    const lines = customerAssemblyTimeLines(localEstimate.assembly_extra_minutes);
    callOutRows.unshift([
      "Confirm disassembly & reassembly time",
      lines
        ? `Over 60 extra minutes — confirm with customer. ${lines.extraTitle} ${lines.extraLabel} · ${lines.totalTitle} ${lines.totalLabel}`
        : "Over 60 extra minutes — confirm with customer (increases cost).",
    ]);
  } else if (isLocal && Number(localEstimate?.assembly_extra_minutes) > 0) {
    // Access answers already show Additional/Total — skip duplicate fee math here.
  } else if (isLocal && Boolean(localEstimate?.assembly_required)) {
    callOutRows.push(["Disassembly & Reassembly", "Required — within 15 min with wrapping/prep (no extra charge)"]);
  }
  ...
  const estimateAmountLabel = oversizeConfirm
    ? "Preliminary estimate (size-based 3-person; final may be lower)"
    : longHaulConfirm
  ...
  const localPriceDisplay =
    isLocal && localEstimate?.estimated_price != null
      ...
      : null;
  const assemblyFeeAmt = isLocal ? Number(localEstimate?.assembly_fee) || 0 : 0;
  const assemblyMinsAmt = isLocal ? Number(localEstimate?.assembly_extra_minutes) || 0 : 0;
  const assemblyLines = customerAssemblyTimeLines(assemblyMinsAmt);
  const assemblyBreakdownHtml =
    assemblyFeeAmt > 0 && assemblyLines
      ? [
          `<p style="margin:0 0 4px;font-size:14px;line-height:1.45;color:#333;text-align:center;"><strong>${assemblyLines.extraTitle} | ${assemblyLines.extraLabel}</strong></p>`,
          `<p style="margin:0 0 8px;font-size:15px;line-height:1.45;color:${NAVY};text-align:center;"><strong>${assemblyLines.totalTitle} | ${assemblyLines.totalLabel}</strong></p>`,
        ].join("")
      : isLocal && Boolean(localEstimate?.assembly_required)
        ? `<p style="margin:0 0 8px;font-size:13px;line-height:1.45;color:#444;text-align:center;">Disassembly &amp; reassembly needed (within 15 minutes with wrapping/prep — no extra charge).</p>`
        : "";
  const assemblyBreakdownText =
    assemblyFeeAmt > 0 && assemblyLines
      ? `${assemblyLines.extraTitle} | ${assemblyLines.extraLabel}\n${assemblyLines.totalTitle} | ${assemblyLines.totalLabel}`
      : isLocal && Boolean(localEstimate?.assembly_required)
        ? "Disassembly & reassembly needed (within 15 minutes with wrapping/prep — no extra charge)."
        : "";
  const assemblyConfirmHtml = localEstimate?.assembly_confirm_over_60
    ? `<p style="margin:0 0 10px;padding:10px 12px;background:#fff5f5;border-left:4px solid #c62828;border-radius:4px;font-size:13px;line-height:1.45;color:#9c2f2f;font-weight:700;text-align:left;">Confirm disassembly &amp; reassembly time with the customer — over 60 extra minutes was entered and increases cost.</p>`
    : "";
  const assemblyConfirmText = localEstimate?.assembly_confirm_over_60
    ? "Confirm disassembly & reassembly time with the customer — over 60 extra minutes was entered and increases cost."
    : "";
  ...
  const confirmDueSizeHtml = oversizeConfirm
    ? `<p style="margin:0 0 10px;padding:12px 14px;background:#fff8e8;border-left:4px solid #c9a227;font-size:14px;line-height:1.55;color:#333;"><strong>Size-based 3-person estimate.</strong> This quote assumes three people may be needed because at least one item is 299+ lb and over 72&quot; high, or 450+ lb — even if you did not select extra crew. Final rates are often lower if the delivery team confirms two people are enough (typically closer to our ($95/hr) rate). <strong>Please confirm with the delivery team before scheduling. This is an estimate only.</strong></p>`
    : "";
  ...
  const timingNoteHtml = isLocal
    ? isPickup
      ? `<p style="margin:0 0 12px;padding:12px 14px;background:#f4f7fb;border-left:4px solid #1f4e79;font-size:14px;line-height:1.55;color:#333;"><strong>Typical timing:</strong> Same-day pickup is possible; most pickups are next-day, and we always do our best to accommodate requested pickup windows.</p>`
      : `<p style="margin:0 0 12px;padding:12px 14px;background:#f4f7fb;border-left:4px solid #1f4e79;font-size:14px;line-height:1.55;color:#333;"><strong>Typical timing:</strong> Same-day delivery is possible; most items are next-day after purchase, and we always do our best to accommodate requested delivery windows.</p>`
    : `<p style="margin:0 0 12px;padding:12px 14px;background:#f4f7fb;border-left:4px solid #1f4e79;font-size:14px;line-height:1.55;color:#333;"><strong>Typical timing:</strong> Nationwide freight is typically 3–5 business days after booking.</p>`;

  const timingNoteText = isLocal
    ? isPickup
      ? "Typical timing: Same-day pickup is possible; most pickups are next-day, and we always do our best to accommodate requested pickup windows."
      : "Typical timing: Same-day delivery is possible; most items are next-day after purchase, and we always do our best to accommodate requested delivery windows."
    : "Typical timing: Nationwide freight is typically 3–5 business days after booking.";

  // Shared customer hero: label + large bold amount (local, pickup, and nationwide).
  const estimateHtml = localPriceDisplay
    ? [
        `<p style="margin:0 0 4px;font-size:15px;line-height:1.4;color:#333;text-align:center;">${escapeHtml(
          estimateAmountLabel
        )}</p>`,
        `<p style="margin:0 0 4px;font-size:28px;line-height:1.15;font-weight:800;color:${NAVY};text-align:center;">Preliminary Estimate: ${escapeHtml(
          localPriceDisplay
        )}</p>`,
        `<p style="margin:0 0 12px;font-size:16px;line-height:1.3;font-weight:800;color:${NAVY};text-align:center;">${
          oversizeConfirm || extraPeople === 1 || extraCrew
            ? "Billed at 130/hr"
            : extraPeople >= 2
              ? "Billed at 95/hr · two trucks"
              : "Billed at 95/hr"
        }</p>`,
        assemblyBreakdownHtml,
        assemblyConfirmHtml,
        confirmDueSizeHtml,
  ...
  const driveExplainHtml =
    isLocal && oneWayMinutes != null
      ? [
          ...
          `<p style="margin:0 0 8px;font-size:12px;line-height:1.45;color:${MUTED};text-align:center;">Distance is based on the mapped route. Drive time includes local truck travel time and is not live traffic.</p>`,
          Number(localEstimate?.multi_item_adder) > 0
            ...
            : "",
          Number(localEstimate?.large_dim_adder) > 0
            ? `<p style="margin:0 0 8px;font-size:13px;color:#444;text-align:center;">Includes large-item handling (+$10 each) for <strong>${escapeHtml(
                localEstimate.large_dim_count
              )}</strong> piece${Number(localEstimate.large_dim_count) === 1 ? "" : "s"} over 60&quot; in any dimension.</p>`
            : "",
          Number(localEstimate?.heavy_weight_adder) > 0
            ? `<p style="margin:0 0 8px;font-size:13px;color:#444;text-align:center;">Includes heavy-item handling (+$10 each) for <strong>${escapeHtml(
                localEstimate.heavy_weight_count
              )}</strong> piece${Number(localEstimate.heavy_weight_count) === 1 ? "" : "s"} over 300 lb.</p>`
            : "",
        ].join("")
      : isLocal
        ? ""
  ...
  const driveExplainText =
    isLocal && oneWayMinutes != null
      ? [
          ...
          "Distance is based on the mapped route. Drive time includes local truck travel time and is not live traffic.",
          Number(localEstimate?.multi_item_adder) > 0
            ? `Includes multi-item handling for ${localEstimate.item_count} items.`
            : null,
          Number(localEstimate?.large_dim_adder) > 0
            ? `Includes large-item handling (+$10 each) for ${localEstimate.large_dim_count} piece${
                Number(localEstimate.large_dim_count) === 1 ? "" : "s"
              } over 60" in any dimension.`
            : null,
          Number(localEstimate?.heavy_weight_adder) > 0
            ? `Includes heavy-item handling (+$10 each) for ${localEstimate.heavy_weight_count} piece${
                Number(localEstimate.heavy_weight_count) === 1 ? "" : "s"
              } over 300 lb.`
            : null,
        ]
          .filter(Boolean)
  ...
  function buildBuyItemsBlock({ includeTip = false, includeAddress = false, margin = "0 0 16px" } = {}) {
    const preferredSchedule = String(submission.access?.preferred_days_windows || "").trim();
    const preferredScheduleHtml =
      includeAddress && preferredSchedule
        ? `<p style="margin:10px 0 0;padding:10px 12px;background:#fff7f7;border:1px solid #d5b4b4;border-radius:8px;font-size:14px;line-height:1.5;color:#9c2f2f;"><strong>Preferred days / windows:</strong> <strong>${escapeHtml(
            preferredSchedule
          )}</strong></p>`
        : "";
    if (isPickup) {
  ...
  const localInternalNextStepsHtml = isLocal
    ? [
        ...
        oversizeConfirm
          ? `<p style="margin:0 0 8px;font-size:15px;line-height:1.5;font-weight:800;color:${INK};">Confirm with the delivery team that this requires 3 people — this estimate assumed that.</p>`
          : "",
        localEstimate?.assembly_confirm_over_60
          ? `<p style="margin:0 0 8px;font-size:15px;line-height:1.5;font-weight:800;color:${INK};">Confirm disassembly &amp; reassembly time with the customer — over 60 extra minutes was entered and increases cost.</p>`
          : "",
        `<p style="margin:0;font-size:15px;line-height:1.5;font-weight:700;color:${INK};">${
          isPickup
            ...
            : `Confirm with delivery team and let client know. Print the <strong>delivery document</strong> PDF (attached) for the customer to sign and initial.`
        }</p>`,
        `</div>`,
      ].join("")
    : "";
  ...
  const localInternalNextStepsText = isLocal
    ? [
        "Next steps",
        oversizeConfirm
          ? "Confirm with the delivery team that this requires 3 people — this estimate assumed that."
          : null,
        isPickup
          ? "Be on the lookout for final confirmation of when the item(s) are coming in."
          : "Confirm with delivery team and let client know. Print the delivery document PDF (attached) for the customer to sign and initial.",
        "",
      ]
        .filter(Boolean)
        .join("\n")
  ...
  const customerText = [
    ...
    localPriceDisplay
      ? `${estimateAmountLabel}\nPreliminary Estimate: ${localPriceDisplay}\n${
          ...
          assemblyBreakdownText ? `\n${assemblyBreakdownText}` : ""
        }${assemblyConfirmText ? `\n${assemblyConfirmText}` : ""}${
          oversizeConfirm
            ? "\nSize-based 3-person estimate. This quote assumes three people may be needed due to item size/weight (299+ lb and over 72\" H, or 450+ lb) — even if you did not select extra crew. Final rates are often lower if two people are enough (typically closer to ($95/hr)). Confirm with the delivery team before scheduling. ESTIMATE ONLY."
            : ""
        }${
          longHaulConfirm
    ...
    String(submission.access?.preferred_days_windows || "").trim()
      ? `PREFERRED DAYS / WINDOWS: ${String(submission.access.preferred_days_windows).trim()}`
      : null,
    "",
    buyPolicyText || null,
...
function applyPdfNotice({ html, text, attached, downloadUrl, filename, signFormAttached = false }) {
  ...
  // Local delivery: internal team gets the delivery document only (no separate summary PDF).
  if (signFormAttached && !attached && !downloadUrl) {
    noticeHtml =
      "<strong>Delivery document PDF</strong> is attached — one-page delivery estimate for print, with route map, for the customer to sign and initial.";
    noticeText =
      "Delivery document PDF attached — print for the customer to sign and initial (includes route map).";
  ...
  if (signFormAttached && (attached || downloadUrl)) {
    noticeHtml +=
      " <strong>Delivery document PDF</strong> is also attached — one-page branded delivery estimate for print, with route map, for the customer to sign and initial.";
    noticeText +=
      "\nDelivery document PDF also attached — print for the customer to sign and initial (includes route map).";
  }
...
async function sendInternalFreightEmail({
  built,
  pdfBuffer,
  pdfFilename,
  signPdfBuffer,
...
async function sendCustomerFreightEmail({ built, customerPdfBuffer, customerPdfFilename }) {
  let customer;
...
async function safeGenerateFreightPdf(submission, ctx, audience) {
  try {
    const buf = await generateFreightQuotePdf(submission, ctx, { audience });
    return buf?.length ? buf : null;
  } catch (pdfErr) {
    console.error(
      `[freight-quote] ${audience} PDF generation failed (continuing):`,
...
async function safeGenerateLocalSignPdf(submission, ctx) {
  try {
    const buf = await generateLocalSignPdf(submission, ctx);
    return buf?.length ? buf : null;
  } catch (pdfErr) {
    console.error(
...
/**
 * Send internal email as soon as its PDF(s) are ready.
 * Local AZ: sign PDF + internal first (do not wait on listing enrich / customer PDF).
 * Nationwide: enrich, build both PDFs in parallel, internal first, then customer.
 */
export async function sendFreightQuoteEmails(submission, ctx) {
  const isLocalDelivery = submission.delivery_path === "local_az";
  const enrichPromise = enrichSubmissionItemMedia(submission);

  if (isLocalDelivery) {
    const t0 = Date.now();
    // Sign PDF can use the raw payload. Item cards need listing enrich so URL
    // pastes get the real title + thumbnail (same card as Find Item).
    const [signPdfBuffer, enrichedSubmission] = await Promise.all([
      safeGenerateLocalSignPdf(submission, ctx),
      enrichPromise,
    ]);
    const built = buildFreightQuoteEmails(enrichedSubmission, ctx);
    const signPdfFilename = buildLocalSignPdfFilename(submission, ctx);

    const internal = await sendInternalFreightEmail({
      built,
      pdfBuffer: null,
      pdfFilename: buildFreightQuotePdfFilename(enrichedSubmission, ctx),
      signPdfBuffer,
      signPdfFilename,
      ctx,
    });
    console.log("[freight-quote] internal email sent (local fast path)", {
      requestId: ctx?.requestId || "",
      ms: Date.now() - t0,
      sign_pdf: Boolean(signPdfBuffer),
    });

    const customerPdfFilename = buildFreightQuotePdfFilename(enrichedSubmission, ctx, {
      audience: "customer",
    });
    const customerPdfBuffer = await safeGenerateFreightPdf(
      enrichedSubmission,
      ctx,
      "customer"
    );
    const customer = await sendCustomerFreightEmail({
      built,
      customerPdfBuffer,
      customerPdfFilename,
    });
  ...
  const enrichedSubmission = await enrichPromise;
  const built = buildFreightQuoteEmails(enrichedSubmission, ctx);
  const pdfFilename = buildFreightQuotePdfFilename(enrichedSubmission, ctx);
  const customerPdfFilename = buildFreightQuotePdfFilename(enrichedSubmission, ctx, {
    audience: "customer",
  });

  const [pdfBuffer, customerPdfBuffer] = await Promise.all([
    safeGenerateFreightPdf(enrichedSubmission, ctx, "internal"),
    safeGenerateFreightPdf(enrichedSubmission, ctx, "customer"),
  ]);
  ...
  const internal = await sendInternalFreightEmail({
    ...
    pdfFilename,
    signPdfBuffer: null,
    signPdfFilename: buildLocalSignPdfFilename(enrichedSubmission, ctx),
    ctx,
  });
