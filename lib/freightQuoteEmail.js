...
const BRAND = {
  ...
  storePhoto: `${PUBLIC_API_BASE}/brand/showroom-exterior.png`,
...
function nationwideInternalPlaybookHtml(submission = {}) {
  ...
  return [
    ...
    `<p style="margin:0 0 14px;font-size:14px;line-height:1.55;color:#222;"><strong>Forward this email to our Freight Center contact</strong> and ask for options: cheapest, best/quality, and warehouse pickup. Stick with their recommendation when quoting the customer.</p>`,
    palletFitHtml,
...
function nationwideInternalPlaybookText(submission = {}) {
  ...
  return [
    ...
    "Forward this email to our Freight Center contact and ask for options: cheapest, best/quality, and warehouse pickup. Stick with their recommendation when quoting the customer.",
    "",
    palletFit,
    "",
    "PACKING DETAILS",
...
function buyNowCtaHtml(items = []) {
  ...
  if (withUrl.length === 1) {
    return [
      `<a href="${escapeHtml(withUrl[0].product_url)}" style="display:block;width:100%;box-sizing:border-box;padding:14px 18px;background:${NAVY};color:#ffffff;text-decoration:none;font-weight:800;font-size:15px;letter-spacing:0.03em;border-radius:8px;text-align:center;">Buy Now</a>`,
      `<p style="margin:8px 0 0;font-size:12px;line-height:1.4;color:${MUTED};text-align:center;">Opens the product page</p>`,
    ].join("");
  }
  return withUrl
    .map(
      (row, i) =>
        `<a href="${escapeHtml(row.product_url)}" style="display:block;width:100%;box-sizing:border-box;margin:${i ? "8px" : "0"} 0 0;padding:13px 14px;background:${NAVY};color:#ffffff;text-decoration:none;font-weight:800;font-size:14px;border-radius:8px;text-align:center;">Buy Now${
  ...
  return (items || [])
    .map((row) => {
      ...
      const meta = [
        ...
        .filter(Boolean)
        .join(" · ");
      const url = showBuyLinks && row.product_url ? String(row.product_url) : "";
      const img = row.image_url ? String(row.image_url) : "";
      const imgBlock = img
        ? url
          ? `<a href="${escapeHtml(url)}" style="display:block;line-height:0;text-decoration:none;"><img src="${escapeHtml(
  ...
  return (items || [])
    .map((row) => {
      const bits = [
        ...
        .filter(Boolean)
        .join(" ");
      return showBuyLinks && row.product_url ? `${bits}\n  Buy: ${row.product_url}` : bits;
    })
    .join("\n");
}
...
function socialFollowHtml({ isPickup }) {
  ...
  return [
    ...
    `<p style="margin:0 0 14px;font-size:14px;line-height:1.55;color:#333;">${escapeHtml(intro)}</p>`,
    `</div>`,

    `<div style="margin:0 12px 12px;padding:14px;background:#ffffff;border:1px solid ${BORDER};border-radius:12px;">`,
    ...
    `<div style="margin:0 12px 16px;padding:14px;background:#ffffff;border:1px solid ${BORDER};border-radius:12px;">`,
...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  if (isLocal && Boolean(localEstimate?.oversize_confirm)) {
    callOutRows.unshift([
      "Confirm crew size",
      "Confirm number of people required — this hit the 3-person / $130/hr oversized threshold",
    ]);
  ...
  const stairsSelected = Boolean(submission.access?.stairs);
  const oversizeConfirm = Boolean(localEstimate?.oversize_confirm);
  const longHaulConfirm = Boolean(localEstimate?.long_haul);
  const estimateOnlySpotlight =
    isLocal && (extraCrew || stairsSelected || oversizeConfirm || longHaulConfirm);
  const estimateAmountLabel = oversizeConfirm
    ? "Best-guess estimate (oversized item — confirm)"
    : longHaulConfirm
      ? "Best-guess estimate (out of town 100+ mi — confirm)"
      : extraPeople >= 2
  ...
  function localRateConfirmHtml({ strong = false } = {}) {
    ...
    return [
      `<div style="margin:14px 0 16px;padding:18px 18px 16px;background:${bg};border:2px solid ${border};border-radius:12px;">`,
      `<p style="margin:0 0 8px;font-size:15px;font-weight:800;color:#8a6d12;letter-spacing:0.03em;text-transform:uppercase;">These are estimates only</p>`,
      `<p style="margin:0 0 16px;font-size:14px;line-height:1.55;color:#333;">This is our <strong>best guess</strong> based on the information you provided — not a locked-in rate. Please call to confirm.</p>`,
      `<table role="presentation" cellpadding="0" cellspacing="0" style="width:100%;border-collapse:separate;border-spacing:0 10px;">`,
      // Cruz Pro Line + call the shop line beside it
      `<tr><td style="padding:12px 14px;background:#ffffff;border:1px solid #e5e1d8;border-radius:10px;">`,
      `<table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>`,
      `<td style="vertical-align:middle;width:78px;">`,
      `<a href="tel:${cruzTel}" style="text-decoration:none;"><img src="${BRAND.cruzLogo}" alt="${CRUZ_PRO_LINE_NAME}" width="72" height="72" style="display:block;width:72px;height:72px;border-radius:10px;object-fit:cover;" /></a>`,
      `</td>`,
      `<td style="vertical-align:middle;padding-left:12px;">`,
      `<p style="margin:0 0 2px;font-size:11px;font-weight:800;letter-spacing:0.06em;text-transform:uppercase;color:#8a6d12;">Delivery partner</p>`,
      `<p style="margin:0;font-size:16px;font-weight:800;color:${NAVY};"><a href="tel:${cruzTel}" style="color:${NAVY};text-decoration:none;">${CRUZ_PRO_LINE_NAME}</a></p>`,
      `<p style="margin:4px 0 0;font-size:18px;font-weight:800;"><a href="tel:${cruzTel}" style="color:${INK};text-decoration:underline;">${CRUZ_PRO_LINE_PHONE}</a></p>`,
      `<p style="margin:8px 0 0;font-size:13px;line-height:1.45;color:#444;">Call the shop: <a href="tel:${shopTel}" style="color:${NAVY};font-weight:800;text-decoration:underline;">${LOST_FOUND_PHONE}</a></p>`,
      `</td></tr></table>`,
      `</td></tr>`,
      // Lost & Found store card with showroom image
      `<tr><td style="padding:12px 14px;background:#ffffff;border:1px solid #e5e1d8;border-radius:10px;">`,
      `<table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>`,
      `<td style="vertical-align:middle;width:96px;">`,
      `<a href="tel:${shopTel}" style="text-decoration:none;"><img src="${BRAND.storePhoto}" alt="Lost &amp; Found Scottsdale showroom" width="88" height="88" style="display:block;width:88px;height:88px;border-radius:10px;object-fit:cover;border:1px solid #e5e1d8;" /></a>`,
      `</td>`,
      `<td style="vertical-align:middle;padding-left:12px;">`,
      `<p style="margin:0 0 2px;font-size:11px;font-weight:800;letter-spacing:0.06em;text-transform:uppercase;color:#8a6d12;">Call the shop</p>`,
      `<p style="margin:0;font-size:16px;font-weight:800;color:${NAVY};"><a href="tel:${shopTel}" style="color:${NAVY};text-decoration:none;">Lost &amp; Found Resale</a></p>`,
      `<p style="margin:4px 0 0;font-size:18px;font-weight:800;"><a href="tel:${shopTel}" style="color:${INK};text-decoration:underline;">${LOST_FOUND_PHONE}</a></p>`,
      `<p style="margin:6px 0 0;font-size:12px;color:#666;">Scottsdale showroom · final rate confirmation</p>`,
      `</td></tr></table>`,
      `</td></tr>`,
      `</table>`,
      `</div>`,
  ...
  function localRateConfirmText() {
    return [
      ...
      "This is our best guess based on the information you provided — not a locked-in rate. Please call to confirm.",
      `Cruz Pro Line: ${CRUZ_PRO_LINE_PHONE}`,
      `Call the shop: ${LOST_FOUND_PHONE}`,
      `Lost & Found Resale: ${LOST_FOUND_PHONE}`,
    ].join("\n");
  }

  const confirmDueSizeHtml = oversizeConfirm
    ? `<p style="margin:0 0 10px;padding:12px 14px;background:#fff1f0;border-left:4px solid #c0392b;font-size:14px;line-height:1.55;color:#333;"><strong>Needs confirmation due to item size.</strong> At least one item is 299+ lb or over 72&quot; high or 72&quot; wide. Please contact us to confirm. <strong>This is an estimate only.</strong></p>`
    : "";

  const confirmDueDistanceHtml = longHaulConfirm
    ? `<p style="margin:0 0 10px;padding:12px 14px;background:#fff1f0;border-left:4px solid #c0392b;font-size:14px;line-height:1.55;color:#333;"><strong>Needs confirmation — out of town (100+ mile round trip).</strong> Please contact us to confirm. <strong>This is an estimate only.</strong></p>`
    : "";

  // Shared customer hero: label + large bold amount (local, pickup, and nationwide).
  const estimateHtml = localPriceDisplay
    ? [
        `<p style="margin:0 0 6px;font-size:15px;line-height:1.45;color:#333;">${escapeHtml(
          estimateAmountLabel
        )}</p>`,
        `<p style="margin:0 0 4px;font-size:28px;line-height:1.2;font-weight:800;color:${NAVY};">${escapeHtml(
          localPriceDisplay
        )} <span style="font-size:16px;font-weight:700;color:#8a6d12;">(Estimate)</span></p>`,
        confirmDueSizeHtml,
        confirmDueDistanceHtml,
        estimateOnlySpotlight
          ? `<p style="margin:0 0 10px;padding:12px 14px;background:#fff8e8;border-left:4px solid #c9a227;font-size:15px;line-height:1.55;color:#333;"><strong>Please confirm.</strong> Access details, crew needs, stairs, oversized pieces, and longer distances can change the final price. Call the shop or Cruz Pro Line before you rely on this number.</p>`
          : `<p style="margin:0 0 10px;font-size:14px;line-height:1.55;color:#444;"><strong>(Estimate)</strong> Best guess based on the information provided — not a final booked rate.</p>`,
      ].join("")
    : isLocal
      ...
      : nationwideRangeText
        ? [
            `<p style="margin:0 0 6px;font-size:15px;line-height:1.45;color:#333;">Your preliminary nationwide freight range</p>`,
            `<p style="margin:0 0 10px;font-size:28px;line-height:1.2;font-weight:800;color:${NAVY};">${escapeHtml(
              nationwideRangeText
            )}</p>`,
            `<p style="margin:0 0 10px;font-size:14px;line-height:1.55;color:#444;">Our team will review your request and email you a confirmed quote.</p>`,
          ].join("")
        : `<p style="margin:0 0 10px;font-size:15px;line-height:1.55;color:#333;">We received your nationwide freight request. Our team will review your request and email you a confirmed quote.</p>`;
  ...
  const driveExplainHtml =
    isLocal && oneWayMinutes != null
      ? `<p style="margin:0 0 10px;font-size:14px;color:#444;"><strong>Drive time to you:</strong> <strong>${oneWayMinutes} min</strong> one way · <strong>${roundTripMinutes} min</strong> round trip${
          distanceMiles != null ? ` · <strong>Distance: ${distanceMiles} mi</strong> one way from our Scottsdale showroom` : ""
        }. Drive time includes a little extra for a loaded delivery truck. Distance and time come from the mapped road route (not live traffic). ${includedWorkHtml}</p>${
          Number(localEstimate?.multi_item_adder) > 0
            ? `<p style="margin:0 0 10px;font-size:14px;color:#444;">Includes multi-item handling for <strong>${escapeHtml(
  ...
  const driveExplainText =
    isLocal && oneWayMinutes != null
      ? `Drive time to you: ${oneWayMinutes} min one way · ${roundTripMinutes} min round trip${
          distanceMiles != null ? ` · Distance: ${distanceMiles} mi one way from our Scottsdale showroom` : ""
        }. Drive time includes a little extra for a loaded delivery truck. Distance and time come from the mapped road route (not live traffic). ${includedWorkText}${
          Number(localEstimate?.multi_item_adder) > 0
  ...
  const buyableItems = (submission.items || []).filter((row) => row.product_url);
  const buyNowAboveMapHtml =
    !isPickup && buyableItems.length
      ? [
          `<div style="margin:16px 0 14px;padding:14px 16px;background:${CREAM};border:1px solid ${BORDER};border-radius:8px;">`,
          `<p style="margin:0 0 10px;font-size:13px;font-weight:800;color:${NAVY};">Buy your item(s)</p>`,
          customerItemsHtml(buyableItems),
          `</div>`,
        ].join("")
      : "";
  ...
  const buyPolicyHtml = isPickup
    ? [
        ...
        `<p style="margin:0;font-size:14px;line-height:1.6;color:#333;">Please call the shop first for final rate confirmation — this is only our best guess based on the information provided.</p>`,
        `</div>`,
      ].join("")
    : isLocal
      ? [
          `<div style="margin:22px 0;padding:16px 14px;background:${CREAM};border:1px solid ${BORDER};border-radius:8px;">`,
          `<p style="margin:0 0 8px;font-size:15px;font-weight:800;color:${INK};">Ready to purchase?</p>`,
          `<p style="margin:0 0 ${buyNowHtml ? "14px" : "0"};font-size:14px;line-height:1.6;color:#333;">We do <strong>not hold items</strong>. Purchase now and we can confirm delivery shortly. Call the shop first for final rate confirmation — this is only our best guess based on the information provided.</p>`,
          buyNowHtml || "",
          `</div>`,
        ].join("")
      : [
          `<div style="margin:22px 0;padding:16px 14px;background:${CREAM};border:1px solid ${BORDER};border-radius:8px;">`,
          `<p style="margin:0 0 8px;font-size:15px;font-weight:800;color:${INK};">Ready to purchase?</p>`,
          `<p style="margin:0 0 ${buyNowHtml ? "14px" : "0"};font-size:14px;line-height:1.6;color:#333;">We do <strong>not hold items</strong>. Purchase now and we can confirm delivery and freight shortly. If you need rates ASAP, please reach out to us at <a href="mailto:${LOST_FOUND_EMAIL}" style="color:${INK};font-weight:700;">${LOST_FOUND_EMAIL}</a>, <a href="tel:${LOST_FOUND_PHONE.replace(
            /\D/g,
            ""
  ...
  const buyPolicyText = isPickup
    ? [
        ...
        "Please call the shop first for final rate confirmation — this is only our best guess based on the information provided.",
      ].join("\n")
    : isLocal
      ? [
          "Ready to purchase?",
          "We do not hold items. Purchase now and we can confirm delivery shortly. Call the shop first for final rate confirmation — this is only our best guess based on the information provided.",
          buyUrlsText || null,
        ]
          .filter(Boolean)
          .join("\n")
      : [
          "Ready to purchase?",
          "We do not hold items. Purchase now and we can confirm delivery and freight shortly. If you need rates ASAP, please reach out to us at info@lostandfoundresale.com, 480-588-7006, or reply here.",
          buyUrlsText || null,
        ]
          .filter(Boolean)
          .join("\n");
  ...
  const stackConfirmHtml = stackConfirmItems.length
    ? [
        ...
        `<p style="margin:0 0 8px;font-size:14px;line-height:1.5;color:#333;">Customer selected quantity 2+ and answered whether pieces can be flipped/stacked. <strong>Do not book final freight until the warehouse confirms packing.</strong></p>`,
        `<ul style="margin:0;padding-left:18px;font-size:13px;line-height:1.5;color:#333;">`,
        ...stackConfirmItems.map((it) => {
          const claim = it.flip_stack_claimed || it.pallet?.flip_stack_claimed || "unanswered";
          return `<li style="margin:0 0 4px;"><strong>${escapeHtml(it.title || "Item")}</strong> · qty ${escapeHtml(
            it.quantity || 1
          )} · customer answer: <strong>${escapeHtml(claim)}</strong>${
  ...
  const stackConfirmText = stackConfirmItems.length
    ? [
        ...
        ...stackConfirmItems.map((it) => {
          const claim = it.flip_stack_claimed || it.pallet?.flip_stack_claimed || "unanswered";
          return `- ${it.title || "Item"} · qty ${it.quantity || 1} · customer answer: ${claim}`;
        }),
        "",
      ].join("\n")
    : "";
  ...
  const nationwideNextStepsHtml = isLocal
    ...
    : [
        ...
        `<p style="margin:0 0 10px;font-size:14px;line-height:1.55;color:#333;">Carrier rates vary — you can also call FreightCenter anytime at <a href="tel:${FREIGHTCENTER_PHONE.replace(/\D/g, "")}" style="color:${NAVY};font-weight:700;">${FREIGHTCENTER_PHONE}</a>.</p>`,
        ...
        } (your choice). <strong>Pickup loading:</strong> Lost &amp; Found will determine if a liftgate is needed when we send your final quote — <strong>for now we assume yes</strong>.</p>`,
  ...
  const nationwideNextStepsText = isLocal
    ...
    : [
        "What happens next",
        "Carrier rates vary — you can also call FreightCenter anytime at 800-716-7608.",
        submission.access?.white_glove
        ...
        `Liftgate at delivery: ${
          submission.access?.liftgate_delivery ? "Yes" : "No"
        } (your choice). Pickup loading: Lost & Found will determine if a liftgate is needed when we send your final quote — for now we assume yes.`,
  ...
  const customerHtml = wrapEmail(
    [
      ...
      estimateHtml,
      isLocal ? localRateConfirmHtml({ strong: estimateOnlySpotlight }) : "",
      driveExplainHtml,
      oversizedHtml,
      buyNowAboveMapHtml,
      ...
      isLocal
        ? `<p style="margin:0 0 14px;font-size:14px;color:#444;"><strong>(Estimate)</strong> Preliminary estimate only — not a booking. Access, labor, stairs, and crew size can change the final amount. Call the shop for final rate confirmation.</p>`
        : `<p style="margin:0 0 14px;font-size:14px;color:#444;">Preliminary estimate only — not a booking. Access and labor can change the final amount.</p>`,
  ...
  const customerText = [
    ...
    "",
    localPriceDisplay
      ? `${estimateAmountLabel}\n${localPriceDisplay} (Estimate)${
          oversizeConfirm
            ? "\nNeeds confirmation due to item size (299+ lb or over 72\" H or over 72\" W). Please contact us to confirm. ESTIMATE ONLY."
            : ""
        }${
          longHaulConfirm
            ? "\nNeeds confirmation — out of town (100+ mile round trip). Please contact us to confirm. ESTIMATE ONLY."
            : ""
        }${
          estimateOnlySpotlight
            ? "\nPlease confirm with the shop or Cruz Pro Line before relying on this number."
            : ""
        }`
      : isLocal
        ? `We received your ${isPickup ? "consignor pickup" : "local delivery"} request and will confirm timing and pricing shortly.`
        : nationwideRangeText
          ? `Your preliminary nationwide freight range\n${nationwideRangeText}\nOur team will review your request and email you a confirmed quote.`
          : "We received your nationwide freight request. Our team will review your request and email you a confirmed quote.",
    ...
    "",
    isLocal
      ? "(Estimate) Preliminary estimate only — not a booking. Call the shop for final rate confirmation."
