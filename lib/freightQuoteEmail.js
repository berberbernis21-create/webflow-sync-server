...
function nationwideInternalPlaybookHtml(submission = {}) {
  ...
  const packingHtml = packingForLukeLines(submission.items)
    .map((block) =>
    ...
    .join("");
  const customerSaid = [
    a.white_glove ? "White Glove requested" : null,
    a.room_of_choice ? "Room of Choice requested" : null,
    a.warehouse_pickup ? "Customer can pick up at warehouse/terminal" : null,
    a.store_loading ? "Store loading at Lost & Found OK" : null,
    `Liftgate at delivery (customer): ${ynLabel(a.liftgate_delivery)}`,
    "Liftgate at pickup: Lost & Found staff to confirm",
    a.stairs ? `Stairs: Yes${a.stair_flights ? ` (${a.stair_flights} flight(s))` : ""}${a.stair_notes ? ` — ${a.stair_notes}` : ""}` : null,
    a.dock ? "Loading dock at delivery" : null,
    a.notes ? `Customer notes: ${a.notes}` : null,
  ]
    .filter(Boolean)
    .join(" · ");

  return [
    `<div style="margin:16px 0;padding:14px 16px;background:#fff8e8;border:1px solid #e6d7a8;border-radius:8px;">`,
    `<p style="margin:0 0 8px;font-size:13px;font-weight:800;color:${NAVY};">Nationwide — next steps</p>`,
    `<ol style="margin:0 0 12px;padding-left:18px;font-size:14px;line-height:1.55;color:#222;">`,
    `<li style="margin:0 0 8px;"><strong>Quickly get our own quote</strong> on <a href="https://www.freightcenter.com" style="color:${NAVY};font-weight:700;">freightcenter.com</a> using the freight-ready pallet entry below (usually <strong>48×40</strong>). Login + steps: <strong>${escapeHtml(
      FREIGHTCENTER_SOP_PATH
    )}</strong>.</li>`,
    `<li style="margin:0 0 8px;"><strong>Forward this email to Luke</strong> at <a href="mailto:${FREIGHTCENTER_LUKE_EMAIL}" style="color:${NAVY};font-weight:700;">${FREIGHTCENTER_LUKE_EMAIL}</a> and ask for <strong>options</strong>: cheapest, best/quality, warehouse pickup, and any other good alternatives. Everything he needs is already in this message. You can <strong>CC the customer</strong> on that email to Luke, or forward Luke’s reply to the customer afterward.</li>`,
    `<li style="margin:0 0 8px;"><strong>Always stick with what Luke says</strong> when you quote the customer — his confirmed options are the source of truth.</li>`,
    `</ol>`,
    `<p style="margin:0 0 12px;font-size:12px;line-height:1.5;color:${MUTED};">FreightCenter: ${escapeHtml(
      FREIGHTCENTER_PHONE
    )} · Use your quick quote for a fast read, then go with Luke’s recommendation for the customer.</p>`,
    `<p style="margin:0 0 6px;font-size:12px;font-weight:800;color:${INK};">Packing / stacking (for our quote + Luke)</p>`,
    packingHtml || `<p style="margin:0 0 10px;font-size:12px;color:${MUTED};">No items listed.</p>`,
    `<p style="margin:0 0 10px;font-size:12px;line-height:1.5;color:#333;"><strong>Customer said:</strong> ${escapeHtml(
      customerSaid || "No special access options selected."
    )}</p>`,
    `<p style="margin:0 0 6px;font-size:12px;font-weight:800;color:${INK};">Quick reminder (from Luke)</p>`,
    `<p style="margin:0 0 8px;font-size:12px;line-height:1.55;color:#333;">Non-stackable usually no extra fee at: Estes; AAA Cooper under 52&quot;; XPO under 70&quot;; ABF; Oak Harbor; CCFS under 60&quot;; FedEx Freight; R+L under 70&quot;. Other carriers will likely stack on top if Non-Stackable is not selected.</p>`,
    `<p style="margin:0;font-size:12px;line-height:1.55;color:#333;">Cheaper carriers often mean slower communication and more billing adjustments — ~95% of the time you get what you pay for. Ask Luke for cheapest <em>and</em> best when you forward.</p>`,
    `</div>`,
  ].join("");
}
...
function nationwideInternalPlaybookText(submission = {}) {
  const a = submission.access || {};
  const packing = packingForLukeLines(submission.items).join("\n\n");
  return [
    "NATIONWIDE — NEXT STEPS",
    `1) Quickly get our own quote on https://www.freightcenter.com using the 48x40 (or oversized) pallet entry. SOP/login: ${FREIGHTCENTER_SOP_PATH}`,
    `2) Forward this email to Luke Rogers <${FREIGHTCENTER_LUKE_EMAIL}> and ask for options: cheapest, best/quality, warehouse pickup, and other good alternatives. CC the customer on that email, or forward Luke's reply to the customer afterward.`,
    `3) Always stick with what Luke says — his confirmed options are the source of truth for the customer quote.`,
    `Use your quick quote for a fast read, then go with Luke's recommendation. Phone: ${FREIGHTCENTER_PHONE}.`,
    "",
    "PACKING / STACKING (FOR OUR QUOTE + LUKE)",
    packing || "(no items)",
    "",
    "CUSTOMER SAID",
    [
      a.white_glove ? "White Glove" : null,
      a.room_of_choice ? "Room of Choice" : null,
      a.warehouse_pickup ? "Warehouse/terminal pickup" : null,
      a.store_loading ? "Store loading OK" : null,
      `Liftgate delivery (customer): ${ynLabel(a.liftgate_delivery)}`,
      "Liftgate pickup: Lost & Found staff to confirm",
      a.notes ? `Notes: ${a.notes}` : null,
    ]
      .filter(Boolean)
      .join(" | ") || "No special options",
    "",
    "REMINDER — NON-STACKABLE (no extra fee, height limits): Estes; AAA Cooper under 52\"; XPO under 70\"; ABF; Oak Harbor; CCFS under 60\"; FedEx Freight; R+L under 70\".",
    "REMINDER — ask Luke for cheapest AND best; cheaper carriers often = slower communication + more billing adjustments.",
  ].join("\n");
}
...
function socialIconButton(href, label, iconUrl, { bg = "#1877F2" } = {}) {
  return [
    `<a href="${escapeHtml(href)}" style="display:inline-block;margin:0 10px 10px 0;text-decoration:none;">`,
    ...
    `</td>`,
    `<td style="padding:8px 16px 8px 0;vertical-align:middle;color:#ffffff;font-size:13px;font-weight:800;letter-spacing:0.02em;white-space:nowrap;">${escapeHtml(
      label
    )}</td>`,
    `</tr>`,
    `</table>`,
...
function socialFollowHtml({ isPickup }) {
  ...
  return [
    ...
    `<a href="${escapeHtml(SOCIAL.luxuryShop)}" style="display:inline-block;margin:0 10px 10px 0;padding:10px 16px;border:1px solid ${NAVY};border-radius:999px;background:#ffffff;color:${NAVY};font-size:13px;font-weight:800;text-decoration:none;">Shop</a>`,
    socialIconButton(SOCIAL.luxuryFacebook, "Facebook", SOCIAL.iconFacebook, { bg: "#1877F2" }),
...
export function buildFreightQuoteEmails(submission, ctx = {}) {
    ...
    isLocal && localEstimate?.estimated_price != null
      ...
      : isLocal
        ...
        : [
            ...
            [
              "Note",
              `1) Quick quote on freightcenter.com  2) Forward to Luke (${FREIGHTCENTER_LUKE_EMAIL}) for cheapest/best/pickup options · floor $350`,
            ],
          ];
  ...
  const customerHtml = wrapEmail(
    [
      ...
      isLocal
        ? ""
        : `<p style="margin:18px 0 12px;font-size:14px;color:#444;"><strong>We will get you a quote.</strong> Our team will review your request and email you full rates. You can also contact <strong>FreightCenter</strong> directly anytime at <a href="tel:${FREIGHTCENTER_PHONE.replace(/\D/g, "")}" style="color:${NAVY};font-weight:700;">${FREIGHTCENTER_PHONE}</a>.</p>
           <p style="margin:0 0 12px;font-size:14px;color:#444;">White Glove / Room of Choice raise cost significantly. Warehouse/terminal pickup near you usually saves about $150–$200 (we will mention this even if you did not select it). A liftgate lowers the pallet when there is no dock — you choose liftgate at delivery; <strong>Lost &amp; Found will determine if a liftgate is needed at pickup</strong>. Pickup/loading hours: <strong>10am–5pm, Monday–Saturday</strong>.</p>`,
  ...
  const customerText = [
    ...
    !isLocal
      ? "We will get you a quote — our team will review your request and email you full rates. You can also contact FreightCenter anytime at 800-716-7608. Warehouse/terminal pickup usually saves about $150–$200. Lost & Found pickup/loading hours: 10am–5pm, Monday–Saturday."
      : null,
    "",
    "Your item(s):",
