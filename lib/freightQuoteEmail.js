...
function customerItemsHtml(items = []) {
  return (items || [])
    .map((row) => {
      ...
      const buyLabel = url
        ? img
          ? `<a href="${escapeHtml(url)}" style="display:inline-block;margin-top:8px;color:${NAVY};font-weight:700;font-size:13px;text-decoration:underline;">View &amp; buy on our site</a>`
          : `<a href="${escapeHtml(url)}" style="display:inline-block;margin-top:6px;padding:8px 12px;border:1px solid ${NAVY};border-radius:6px;color:${NAVY};font-weight:700;font-size:13px;text-decoration:none;">View &amp; buy on our site</a>`
        : "";
      return [
        `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 14px;padding:12px;background:${CREAM};border:1px solid ${BORDER};border-radius:8px;">`,
        `<tr>`,
        imgBlock
          ? `<td valign="top" width="120" style="padding-right:14px;">${imgBlock}</td>`
          : "",
        `<td valign="middle" style="font-size:14px;line-height:1.5;color:#333;">`,
        `<strong style="color:${INK};font-size:15px;">${title}</strong>`,
        meta ? `<br/><span style="color:${MUTED};font-size:13px;">${meta}</span>` : "",
        buyLabel,
        `</td></tr></table>`,
      ].join("");
    })
