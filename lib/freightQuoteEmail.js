...
function customerItemsHtml(items = [], { showBuyLinks = true } = {}) {
  return (items || [])
    .map((row) => {
      ...
      const cleanTitle = escapeHtml(clean);
      const dimBits =
        row.width != null && row.depth != null && row.height != null
          ? `${escapeHtml(row.width)}X${escapeHtml(row.depth)}X${escapeHtml(row.height)}H`
          : "";
      const titleAlreadyHasDims = /\d+\s*[xX×]\s*\d+/.test(clean);
      const titleLineClean = dimBits && !titleAlreadyHasDims ? `${cleanTitle} · ${dimBits}` : cleanTitle;
      const meta = [
      ...
      return [
        ...
        `<div style="margin:0 0 2px;text-align:left;"><strong style="color:${INK};font-size:14px;line-height:1.3;">${titleLineClean}</strong></div>`,
        meta
...
function customerItemsText(items = [], { showBuyLinks = true } = {}) {
  return (items || [])
    .map((row) => {
      const sold = listingLooksSold(row);
      const bits = [
        `- ${itemCleanTitle(row, `Item ${row.index}`)}${sold ? " (SOLD)" : ""}`,
...
export function buildFreightQuoteEmails(submission, ctx = {}) {
  ...
  const estimateHtml = localPriceDisplay
    ? [
        ...
        `<p style="margin:0 0 10px;font-size:15px;line-height:1.3;font-weight:700;color:${MUTED};text-align:center;">${
          Boolean(localEstimate?.oversize_confirm) || Number(localEstimate?.extra_people) === 1
            ? "(Charged $130/hr)"
            : Number(localEstimate?.extra_people) >= 2
              ? "(Charged $95/hr · two trucks)"
              : "(Charged $95/hr)"
        }</p>`,
