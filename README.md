...
**Sold retention (optional)** — **Furniture (ecommerce) only.** After listings have been in a sold state for **3 days** (default), each `/sync-all` run archives them off the live site (`archiveWebflowEcommerceProduct`, same as duplicate cleanup). **Luxury** is never part of retention: sold items stay in the **Recently Sold** category (and hidden from the main grid) via normal sync — no CMS `isArchived` sweep.  
`SOLD_RETENTION_DAYS` — Default `3` (furniture only).  
`SOLD_RETENTION_DISABLE` — Set to `1` or `true` to turn off furniture retention archive.  
`FURNITURE_SOLD_SINCE_FIELD_SLUG` — (optional) DateTime field on furniture products; set on first transition to sold so retention uses an explicit timestamp. `LUXURY_SOLD_SINCE_FIELD_SLUG` — optional; if set, written when luxury items are marked sold (not used for archiving luxury). Otherwise the server uses `soldMarkedAt` in the JSON cache for furniture retention, or starts the clock when a sold listing is first seen during the sweep.

**One-time sold backfill (first `/sync-all` after deploy)** — **Furniture only.** While `DATA_DIR/sold_retention_backfill_2026-04-02.done` does **not** exist, the server archives **sold** furniture listings whose **anchor date** is **on or before** the cutoff (default **2026-04-02** end of day UTC). Anchor = `FURNITURE_SOLD_SINCE_FIELD_SLUG` or `soldMarkedAt` in cache if present, else Webflow ecommerce product `lastUpdated` / `lastPublished` / `createdOn`. Items with **no** parseable anchor are skipped (`sold_retention.backfill_skip_no_anchor`). After the pass, it writes the marker file; **delete that file** to run another backfill.  
...
After the marker exists, only the normal **3-day** furniture rule runs.
...
- **Long-sold archive:** One-time **April 2 (configurable) backfill** archives old **sold furniture** only; **luxury** is unchanged (Recently Sold). After the marker file exists, **furniture** sold listings older than `SOLD_RETENTION_DAYS` (default 3) are ecommerce-archived.
