...
`FURNITURE_SOLD_SINCE_FIELD_SLUG` — (optional) DateTime field slug on **furniture ecommerce** products. Default **`date-sold`**. The sync **always fills** this when marking sold (or when `sold` is true and the date is empty); cleared when back in stock. **Ongoing** 3-day archive uses **only** this Webflow datetime (not `soldMarkedAt` in cache). One-time backfill can still fall back to Webflow `lastUpdated` / etc. if the field is empty. `LUXURY_SOLD_SINCE_FIELD_SLUG` — optional; if set, written when luxury items are marked sold (not used for archiving luxury).
